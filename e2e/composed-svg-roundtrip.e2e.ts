...
const fixtures = [
  { name: 'composition-mixed.svg', objects: 4, images: 2 },
  { name: 'composition-clipped-image.svg', objects: 1, images: 1 },
  { name: 'composition-visible-mask.svg', objects: 5, images: 2 },
] as const;
...
for (const fixture of fixtures) {
  test(`composed SVG picker/export/re-import preserves rendered geometry: ${fixture.name}`, async ({
    ...
    const painted = await captureSvgCanvas(page, inserted.project);
    await svgUndo(page, 0);
    expect((await composedSvgSnapshot(page)).undoCount).toBe(0);
    const raster = inserted.project.scene.objects[0];
    if (fixture.name === 'composition-clipped-image.svg' && raster?.kind === 'raster-image') {
      const empty = await captureSvgCanvas(page, inserted.project);
    ...
    expect(reimported.undoCount).toBe(1);
...
test('Esc during second SVG bitmap hydration cancels the whole file and leaves Undo untouched', async ({
  ...
  expect((await composedSvgSnapshot(page)).undoCount).toBe(1);
  await svgUndo(page, 0);
  expect((await kerfdesk.events()).filter((event) => event.kind.startsWith('serial-'))).toEqual([]);
});
