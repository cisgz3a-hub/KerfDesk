...
import { makePng, u16beBytes } from '../src/ui/import/png-incremental-decoder.test-support';

const PNG16_BASE64 = Buffer.from(
  makePng({
    width: 4,
    height: 2,
    colorType: 0,
    bitDepth: 16,
    rows: [u16beBytes(0x0000, 0x0001, 0x00ff, 0x0100), u16beBytes(0x1234, 0x7fff, 0x8000, 0xffff)],
    filters: [1, 4],
    transparency: Uint8Array.of(0x12, 0x34),
  }),
).toString('base64');

test('real import worker and main fallback preserve the same exact PNG16 field', async ({
  page,
}) => {
  ...
  const result = await page.evaluate(async (pngBase64) => {
    ...
    if (
      ...
      throw new Error('production PNG16 import modules are unavailable');
    }

    const progress: Progress[] = [];
    try {
      const pending = client.prepareReliefHeightfieldPngOffThread(blob, 'exact-u16.png', 100, 5, {
