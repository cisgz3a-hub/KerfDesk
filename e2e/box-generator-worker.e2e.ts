...
test('Box Generator stays editable and cancellable during expensive generation', async ({
  ...
  await dialog.getByRole('spinbutton', { name: 'Finger width (mm)', exact: true }).fill('3');
  await expect(generate).toBeEnabled();
  await dialog
    .getByRole('spinbutton', { name: 'Width (mm)', exact: true })
    .fill(EXPENSIVE_WIDTH_MM);
  await expect(dialog.getByRole('button', { name: 'Cancel preview generation' })).toBeVisible();

  const startedAt = performance.now();
  await dialog.getByRole('spinbutton', { name: 'Depth (mm)', exact: true }).fill('41');
  const editElapsedMs = performance.now() - startedAt;
  expect(editElapsedMs).toBeLessThan(RESPONSIVE_EDIT_LIMIT_MS);
  await expect(dialog.getByRole('spinbutton', { name: 'Depth (mm)', exact: true })).toHaveValue(
    '41',
  );
  ...
  await dialog.getByRole('spinbutton', { name: 'Width (mm)', exact: true }).fill('60');
  await expect(generate).toBeEnabled();
...
test('Box Generator keeps flat and assembled previews available above 20,000 points', async ({
  ...
  await dialog.getByRole('spinbutton', { name: 'Finger width (mm)', exact: true }).fill('3');
  await dialog.getByRole('spinbutton', { name: 'Width (mm)', exact: true }).fill('5000');
  await expect(generate).toBeEnabled();
