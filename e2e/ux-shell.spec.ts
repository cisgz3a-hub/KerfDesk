...
test.describe('workspace shell acceptance', () => {
  ...
  test('keeps a usable, overflow-free canvas at compact and zoom-equivalent sizes', async ({
    page,
  }) => {
  ...
  test('renders a nonblank workspace in real Chromium canvas', async ({ page }) => {
    await page.setViewportSize({ width: 1024, height: 700 });
    await page.goto('/');
    const canvas = page.locator('canvas[aria-label="KerfDesk workspace"]');
