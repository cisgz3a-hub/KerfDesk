/**
 * Image tracing via a Potrace-style bitmap path, polygon, vertex-adjustment,
 * smoothing, and curve-optimization pipeline.
 * Async path runs the same math in a Web Worker (see `trace.worker.ts`).
 */
...
export function traceToSceneObjectAsync(
  ...
  return new Promise((resolve) => {
    ...
    const onMessage = (ev: MessageEvent<{ id: number; subPaths: SubPath[] }>) => {
      if (ev.data.id !== id) return;
      worker.removeEventListener('message', onMessage);
      worker.removeEventListener('error', onError);

      const { subPaths } = ev.data;
