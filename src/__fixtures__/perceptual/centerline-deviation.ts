...
export function measureCenterlineDeviation(
  ...
  const [maxDeviationPx, meanDeviationPx] = directedDistance(tracedLines, truth.centerlines);
  const [maxGapPx, meanGapPx] = directedDistance(truth.centerlines, tracedLines);
