...
describe('schema 9 owned image clip persistence', () => {
  it('writes the clip and its native geometry under schema 9 and reads it without migration', () => {
    ...
    expect(JSON.parse(prepared.json).schemaVersion).toBe(9);
  ...
  it('promotes schema 8 without adding clips or changing its pixels, external mask or output', () => {
    ...
    expect(migrated).toEqual({ kind: 'ok', raw: { ...legacy, schemaVersion: 9 }, steps: [8] });
    ...
    expect(loaded.project.schemaVersion).toBe(9);
