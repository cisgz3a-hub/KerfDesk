...
describe('webSerial byte-range validation', () => {
  it('encodes every generated byte-range code unit as exactly one identical byte', async () => {
    await fc.assert(
      fc.asyncProperty(fc.integer({ min: 0, max: WIRE_BYTE_MAX }), async (byte) => {
        const port = installMockSerial(new MockPort());
        const ref = await webSerial.requestPort();
        if (ref === null) throw new Error('expected port ref');
        const connection = await ref.open({ baudRate: 115200 });

        await connection.write(String.fromCharCode(byte));

        const written = port.writer.write.mock.calls[0]?.[0];
        expect(Array.from(written ?? [])).toEqual([byte]);
      }),
      { numRuns: WIRE_PROPERTY_RUNS },
    );
  });

  it('rejects generated out-of-range code units before writing', async () => {
    await fc.assert(
      fc.asyncProperty(
        fc.integer({ min: WIRE_BYTE_MAX + 1, max: CODE_UNIT_MAX }),
        async (codeUnit) => {
          const port = installMockSerial(new MockPort());
          const ref = await webSerial.requestPort();
          if (ref === null) throw new Error('expected port ref');
          ...
          await expect(connection.write(String.fromCharCode(codeUnit))).rejects.toThrow(
            /single-byte/i,
          );
