...
const IMPORT_PATTERN = /\b(?:from\s*|import\s*\(\s*|import\s+)['"]([^'"]+)['"]/gu;
...
describe('user macro dependency boundary', () => {
  it('recognizes static, dynamic, and side-effect-only imports', () => {
    const source = [
      "import '../../../core/controllers';",
      "import { helper } from './helper';",
      "const module = import('./dynamic-helper');",
    ].join('\n');

    expect([...source.matchAll(IMPORT_PATTERN)].map((match) => match[1])).toEqual([
      '../../../core/controllers',
      './helper',
      './dynamic-helper',
    ]);
