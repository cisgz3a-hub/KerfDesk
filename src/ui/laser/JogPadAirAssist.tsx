...
function airSetupSummaryText(summary: AirAssistDefaultSyncSummary): string {
  ...
  // Those are job settings: a completed Frame no longer matches afterwards, and
  // Start stays greyed out until the job is framed again (controller audit
  // gap-start-9, ADR-372).
  return changes.length === 0
    ? ''
    : `This will ${changes.join(', ')}, then turn manual air on. The job changes, so Frame the job again before Start.`;
}
