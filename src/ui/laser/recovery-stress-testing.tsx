...
export async function connectSimulator(): Promise<GrblSimulator> {
  const simulator = createGrblSimulator(SIM_OPTIONS);
  await act(async () => {
...
export async function harness(): Promise<StressHarness> {
  ...
  useStore.setState({ project: stripedProject() });
...
/** Press Start through the real Frame permit and Job Review, return once the
 * stream has been accepted by the controller. */
