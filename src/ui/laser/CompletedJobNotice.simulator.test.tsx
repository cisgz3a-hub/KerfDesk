...
it('offers Done after an unknown-dwell run settles even though its live estimate was unavailable', async () => {
  const simulator = createGrblSimulator();
  await useLaserStore.getState().connect(simulator.adapter);
  await vi.advanceTimersByTimeAsync(1_200);
  useLaserStore.setState({ detectedControllerKind: null });
  const gcode = COMPLETED_GCODE.replace('M3 S0', 'M3 S0\nG4 P0.2');
  const handoff = laserCountdownTestHandoff({
    gcode,
    retentionKey: 'completed-display-unknown-dwell',
    capability: 'realtime',
  });
  expect(handoff.jobTimingPlan).toMatchObject({
    kind: 'unavailable',
