// CameraPanel — a minimal, non-modal preview panel for Camera Mode (ADR-094).
  ...
  if (!open) {
    return (
      <button
        ...
        style={launcherStyle}
        onClick={toggle}
        title="Open the camera preview (ADR-094)"
      >
        Camera
      </button>
    );
...
function CameraPanelOpen(): JSX.Element {
  ...
  return (
    <div role="dialog" aria-label="Camera preview" style={panelStyle}>
      ...
      <NetworkCameraSection state={networkCamera} onDetect={() => void detectNetworkCamera(camera)} />
      {!isSupported ? (
        <p style={noteStyle}>
