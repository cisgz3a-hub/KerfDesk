...
// alignment (ADR-094). The operator clicks the four bed corners in the live
...
const CORNER_LABELS: ReadonlyArray<string> = ['top-left', 'top-right', 'bottom-right', 'bottom-left'];

type IntrinsicSize = { readonly width: number; readonly height: number };
type ClientRect = { readonly left: number; readonly top: number; readonly width: number; readonly height: number };
...
export function NetworkCameraView(props: { readonly frameUrl: string }): JSX.Element {
  ...
  const handleClick = (event: React.MouseEvent<HTMLImageElement>): void => {
    ...
    const point = clickToIntrinsicPixel(event.clientX, event.clientY, img.getBoundingClientRect(), natural);
  ...
  return (
    <div style={columnStyle}>
      ...
      />
      <AlignmentControls alignment={alignment} onBegin={beginBedAlignment} onReset={resetAlignment} />
    </div>
  );
}
