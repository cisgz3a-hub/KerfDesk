...
export function ArrayDialog(props: {
  ...
  return (
    ...
    >
      <ArrayModeTabs mode={mode} setMode={setMode} />
      {mode === 'grid' ? (
        <GridArrayFields
...
function ArrayModeTabs(props: {
  readonly mode: ArraySpec['kind'];
  readonly setMode: (mode: ArraySpec['kind']) => void;
}): JSX.Element {
  return (
    <div role="tablist" aria-label="Array type" style={tabsStyle}>
      ...
      <ModeButton
        ...
        onClick={() => props.setMode('point-rotation')}
      />
      <ModeButton
        active={props.mode === 'circular'}
        label="Circular"
        onClick={() => props.setMode('circular')}
      />
    </div>
  );
}
