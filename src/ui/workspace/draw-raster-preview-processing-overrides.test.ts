...
for (const algorithm of ['ordered', 'floyd-steinberg', 'jarvis'] as const)
  it(`uses the existing source-space ${algorithm} processor with effective overrides`, () => {
    const raster = {
        ...previewRaster(),
        operationOverride: {
      ...
      sink = previewSink();
    sink.draw(p);
    // This is processing-route equivalence. Directional dithering after a machine
    // row flip is not equivalent to flipping an already dithered source bitmap.
    const reference = buildProcessedRasterBitmap(
      raster,
      previewLayer({ ditherAlgorithm: algorithm, negativeImage: false, minPower: 17, power: 29 }),
      p.device,
    );
    expect(reference.kind).toBe('ok');
    expect(sink.drawn[0]?.pixels?.data).toEqual(
      reference.kind === 'ok' ? reference.rgba : undefined,
    );
  });
