...
import {
  ...
  type Project,
  type SceneObject,
  type Transform as ObjTransform,
  transformedBBox,
} from '../../core/scene';
...
// Cached HTMLImageElement per dataUrl. Decoding a PNG data URL is
// async (Image.onload), but the draw loop is synchronous; we cache
// the resolved element keyed by URL so repeated frames hit memory
// rather than re-decoding. First paint shows nothing until the
// image loads — same one-frame-delay as the SVG import flow.
