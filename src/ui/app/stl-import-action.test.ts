import { afterEach, describe, expect, it, vi } from 'vitest';
import { importStlFiles } from './stl-import-action';

describe('importStlFiles', () => {
  afterEach(() => vi.unstubAllGlobals());

  it('imports through the disclosed legacy fallback when worker construction fails', async () => {
    stubUnavailableWorker();
    const { file, arrayBuffer } = validAsciiStlFile();
    const pushToast = vi.fn();
    const importObject = vi.fn();

    await importStlFiles([file], {
      importObject,
      pushToast,
    });

    expect(arrayBuffer).toHaveBeenCalledOnce();
    expect(importObject).toHaveBeenCalledOnce();
    expect(importObject.mock.calls[0]?.[0]).toMatchObject({
      targetWidthMm: 100,
      targetHeightMm: 100,
      widthAspect: 'preserve',
      reliefSource: {
        kind: 'legacy-mesh',
        intrinsicBounds: { kind: 'finite-float32-v1', maxX: 10, maxY: 10, maxZ: 1 },
      },
    });
    expect(pushToast).toHaveBeenCalledWith(
      expect.stringMatching(/part\.stl.*main thread.*unresponsive/i),
      'warning',
    );
    expect(pushToast).toHaveBeenCalledWith(expect.stringContaining('Imported relief'), 'success');
  });

  it('imports without machine context and discloses that only CNC produces output', async () => {
    stubUnavailableWorker();
    const { file, arrayBuffer } = validAsciiStlFile();
    const pushToast = vi.fn();
    const importObject = vi.fn();

    await importStlFiles([file], {
      importObject,
      pushToast,
    });

    expect(arrayBuffer).toHaveBeenCalledOnce();
    expect(importObject).toHaveBeenCalledOnce();
    expect(pushToast).toHaveBeenCalledWith(
      expect.stringMatching(/stored in either machine mode.*only in CNC mode/i),
      'success',
    );
    expect(pushToast).not.toHaveBeenCalledWith(expect.stringMatching(/needs CNC mode/i), 'error');
  });
});

function stubUnavailableWorker(): void {
  vi.stubGlobal('Worker', function WorkerUnavailable(): never {
    throw new Error('workers blocked');
  });
}

function validAsciiStlFile(): {
  readonly file: File;
  readonly arrayBuffer: ReturnType<typeof vi.fn>;
} {
  const ascii = [
    'solid part',
    'facet normal 0 0 1',
    'outer loop',
    'vertex 0 0 0',
    'vertex 10 0 0',
    'vertex 0 10 1',
    'endloop',
    'endfacet',
    'endsolid part',
  ].join('\n');
  const bytes = new TextEncoder().encode(ascii);
  const arrayBuffer = vi.fn(async () => bytes.buffer as ArrayBuffer);
  return {
    file: { name: 'part.stl', size: bytes.byteLength, arrayBuffer } as unknown as File,
    arrayBuffer,
  };
}
