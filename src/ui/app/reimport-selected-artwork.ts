...
export async function handleReimportSelectedArtwork(args: {
  ...
  try {
    ...
    args.pushToast(`Could not re-import ${args.target.source}: ${messageOf(error)}`, 'error');
    return;
  }
  const file = files[0];
  ...
  if (!file.name.toLowerCase().endsWith(extension)) {
    args.pushToast(
      `Could not re-import: choose a ${extension.toUpperCase()} source file.`,
      'error',
    );
    return;
  }
  ...
  if (extension === '.dxf') {
    await importDxfFiles([file], { importObject: replaceTarget, pushToast: args.pushToast });
    return;
  }
  await importSvgFiles([file], replaceTarget, args.pushToast);
}
