...
describe('createCarvedWoodMaterial', () => {
  ...
  // The mesh is decimated for vertex count while the depth grid stays fine, so
  // the two extents are genuinely different. Centring uses the MESH extent and
  // normalising uses the GRID extent; swapping them slides the shading off the
  // groove it describes.
  it('separates the grid extent from the mesh extent', () => {
    const wood = woodMaterial();
    const shader = standardShader();
    wood.material.onBeforeCompile(shader as never, null as never);

    const gridSize = shader.uniforms.uCarveSizeMm?.value as three.Vector2;
    const meshSize = shader.uniforms.uCarveMeshMm?.value as three.Vector2;
    // 2x2 cells at 1 mm per cell.
    expect([gridSize.x, gridSize.y]).toEqual([2, 2]);
    expect([meshSize.x, meshSize.y]).toEqual([20, 10]);
    wood.dispose();
  });
