...
describe('ReliefInputLevelsControl', () => {
  it('renders only for a heightfield, between Polarity and Gamma, with all mapping cases visible', async () => {
    ...
    try {
      ...
      const polarity = heightfieldRender.host.querySelector(
        'select[aria-label="Relief height-map polarity"]',
      );
      const gamma = heightfieldRender.host.querySelector(
        'input[aria-label="Relief height-map gamma"]',
      );
      expect(group).not.toBeNull();
      expect(polarity?.closest('label')?.nextElementSibling).toBe(group);
      expect(group?.nextElementSibling).toBe(gamma?.closest('label'));
      ...
      expect(group?.textContent).toContain(
        'Equal endpoints produce a flat normalized 0.5 before gamma and polarity',
      );
    } finally {
      await unmount(heightfieldRender.root, heightfieldRender.host);
    }

    const meshRender = await render(meshRelief());
    try {
      expect(meshRender.host.querySelector('[aria-label="Relief input levels"]')).toBeNull();
    } finally {
      await unmount(meshRender.root, meshRender.host);
    }
  });
