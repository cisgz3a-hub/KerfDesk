...
function uniqueWeldId(scene: Scene): string {
  ...
  if (!used.has('welded-paths')) return 'welded-paths';
  for (let index = 2; index <= MAX_WELD_ID_SUFFIX; index += 1) {
    const id = `welded-paths-${index}`;
    if (!used.has(id)) return id;
  }
  return `welded-paths-${crypto.randomUUID()}`;
}

const MAX_WELD_ID_SUFFIX = 1000;
