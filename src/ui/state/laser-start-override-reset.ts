...
//
// The reset is three GRBL realtime bytes, prepended to the SAME write as the
// program's first window. GRBL acts on realtime bytes the moment they arrive
// and never stores them in its receive buffer, so they cost no RX budget and
// cannot reorder behind a queued line; and because they ride the first
// program write, a Start that is refused sends nothing at all. Pausing and
 ...
 * nothing: `bytes` go in front of the first program write, and `accepted`
 * records the reset in the log once that write is on the wire. */
...
/** The bytes to put in front of the first program write: the reset, or ''. */
