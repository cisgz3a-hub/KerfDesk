...
describe('safe-write transport epochs', () => {
  ...
  it('owes exactly one terminal ack for a modal query write like any line', async () => {
    // The ackless mode is deliberately gone (audit 2026-07-17-0146 P3-4): it
    // was inert in production and its unaccounted ok carried an F1 mis-settle
    // risk. $G is a plain owed-ack line — one newline, one owed terminal ack.
    let release: () => void = () => undefined;
    const refs: SafeWriteRefs = {
      connection: deferredConnection((resolve) => {
        release = resolve;
