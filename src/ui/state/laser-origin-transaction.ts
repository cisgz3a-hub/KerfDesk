...
export async function runOriginTransaction(
  ...
  try {
    ...
    // Resolved before the committing set(): a successPatch may await a fresh
    // controller frame — Set Origin waits for the post-G92 position so it never
    // records a location-unknown origin.
