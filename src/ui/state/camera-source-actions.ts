...
type Set = (
  partial:
    | Partial<CameraSourceSlice>
    | ((state: CameraSourceSlice) => Partial<CameraSourceSlice>),
) => void;
...
export function createCameraSourceActions(set: Set, get: Get): CameraSourceActions {
  return {
    ...
    detectMachineCamera: async (bridge) => {
      if (bridge === undefined) {
