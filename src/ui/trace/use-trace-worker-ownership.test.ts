...
it('retains small runtime-failure fallback and fresh-worker recovery without stale death', async () => {
  ...
  crash();
  await flush();
  const recovery = latest();
  expect(OwnedWorker.all).toHaveLength(2);
  crash();
  await flush();
  expect(first.state.settlements).toBe(0);
  expect(recovery.terminated).toBe(0);
  recovery.onerror!();
  await first.done;
  expect(first.state.result).toBeDefined();
  expect(inline).toHaveBeenCalledTimes(1);
  expect(OwnedWorker.all.map((worker) => worker.terminated)).toEqual([1, 1]);
