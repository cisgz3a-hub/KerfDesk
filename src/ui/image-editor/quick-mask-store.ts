...
import { paintStrokeInPlace, snapLineEnd45, type PaintPoint } from '../../core/image-edit';
...
export const useQuickMaskStore = create<QuickMaskState>((set, get) => ({
  ...
  toggle: () => {
    ...
    const { rubylith } = get();
    if (rubylith === null) {
      set({ rubylith: rubylithFromSelection(session.doc, session.selection) });
