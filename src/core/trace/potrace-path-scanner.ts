import type { PotraceTurnPolicy } from './potrace-params';
import type { TraceBitmap } from './potrace-bitmap';
import type { PotracePoint } from './potrace-polygon';

export type { PotraceTurnPolicy };

export type PotracePathScanOptions = {
  readonly turdsize: number;
  readonly turnpolicy: PotraceTurnPolicy;
};

export type PotraceScannedPath = {
  points: PotracePoint[];
  area: number;
  sign: '+' | '-';
  minX: number;
...
function tracePath(
  ...
  while (true) {
    ...
    const left = getValueAt(bitmap, x + (dirX + dirY - 1) / 2, y + (dirY - dirX - 1) / 2);
    const right = getValueAt(bitmap, x + (dirX - dirY - 1) / 2, y + (dirY + dirX - 1) / 2);
...
function xorPath(bitmap: MutableBitmap, path: PotraceScannedPath): void {
  ...
  for (let index = 1; index < path.points.length; index += 1) {
    ...
    for (let currentX = x; currentX < path.maxX; currentX += 1) {
...
export function traceBitmapToPotracePaths(
  ...
  while (true) {
    ...
    if (path.area > options.turdsize) paths.push(path);
  }
