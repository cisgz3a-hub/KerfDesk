...
describe('normalizeClosedPolylinesEvenOddChecked', () => {
  it('resolves a bow-tie into bounded filled roots before offsetting', () => {
    ...
    const result = normalizeClosedPolylinesEvenOddChecked([bowTie]);
    expect(result.kind).toBe('ok');
    if (result.kind !== 'ok') return;
    expect(result.value.length).toBeGreaterThan(0);
    expect(totalArea(result.value)).toBeCloseTo(0.004, 6);
    for (const polyline of result.value) {
      for (const point of polyline.points) {
        expect(point.x).toBeGreaterThanOrEqual(0);
        expect(point.x).toBeLessThanOrEqual(0.2);
        expect(point.y).toBeGreaterThanOrEqual(0);
        expect(point.y).toBeLessThanOrEqual(0.04);
      }
    }
  });
});
