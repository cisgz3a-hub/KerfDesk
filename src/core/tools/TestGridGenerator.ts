...
function orderedByBurnRisk(values: number[], safestFirst: 'low' | 'high'): Array<{ value: number; index: number }> {
  return values
    .map((value, index) => ({ value, index }))
    .sort((a, b) => {
      const delta = safestFirst === 'low'
        ? a.value - b.value
        : b.value - a.value;
...
function validateTestGridOptions(opts: TestGridOptions): void {
  if (!Number.isFinite(opts.originX) || opts.originX < 0 || !Number.isFinite(opts.originY) || opts.originY < 0) {
    throw new Error('Test grid origin must be finite and non-negative.');
  }
  if (!Number.isFinite(opts.cellSizeMm) || opts.cellSizeMm <= 0) {
    throw new Error('Test grid cell size must be positive.');
  }
  if (!Number.isFinite(opts.cellGapMm) || opts.cellGapMm < 0) {
    throw new Error('Test grid cell gap must be non-negative.');
  }
  if (!Number.isFinite(opts.lineIntervalMm) || opts.lineIntervalMm <= 0) {
    throw new Error('Test grid line interval must be positive.');
  }
  if (!Number.isFinite(opts.travelSpeedMmPerMin) || opts.travelSpeedMmPerMin <= 0) {
    throw new Error('Test grid travel speed must be positive.');
  }
  if (opts.powers.length === 0 || opts.powers.some(power => !Number.isFinite(power) || power < 0)) {
    throw new Error('Test grid powers must be finite and non-negative.');
  }
  if (opts.speeds.length === 0 || opts.speeds.some(speed => !Number.isFinite(speed) || speed <= 0)) {
    throw new Error('Test grid speeds must be finite and positive.');
  }
}
...
export function generateTestGrid(opts: TestGridOptions): string {
  validateTestGridOptions(opts);

  const lines: string[] = [];

  lines.push('; LaserForge power/speed test grid');
  lines.push('; Generated by LaserForge Test Grid');
  ...
  const executionPowers = orderedByBurnRisk(powers, 'low');
  const executionSpeeds = orderedByBurnRisk(speeds, 'high');

  for (const powerCell of executionPowers) {
    const power = powerCell.value;
    const clampedPower = Math.min(Math.max(power, 0), maxSpindle);
    const rowY = gridOriginY + powerCell.index * (cellSizeMm + cellGapMm);

    for (const speedCell of executionSpeeds) {
      const speed = speedCell.value;
      const colX = gridOriginX + speedCell.index * (cellSizeMm + cellGapMm);
