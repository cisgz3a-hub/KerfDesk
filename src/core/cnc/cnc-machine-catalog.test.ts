...
describe('CNC_MACHINE_CATALOG', () => {
  ...
  // retained legacy templates, not verified model-specific maxima.
  it('retains the Onefinity template IDs and geometry without claiming controller support', () => {
    ...
    for (const preset of [woodworker, journeyman]) {
      expect(preset?.controllerSupport).toBe('unqualified');
      expect(preset?.controllerNote).toContain('no qualified KerfDesk driver or postprocessor');
      expect(preset?.name).toContain('geometry only');
    }
  });
});
