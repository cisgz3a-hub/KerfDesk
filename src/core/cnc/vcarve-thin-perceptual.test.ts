...
// Perceptual verification (ADR-025 pattern) for ADR-280: V-carve artwork
...
// z(x, y) = −min(distToBoundary, maxDepth) at tan(45°) = 1. Before ADR-280
...
describe('v-carve thin artwork — perceptual (ADR-280)', () => {
  it('a stroke narrower than 2·δ is carved to its analytic shallow groove, not dropped', () => {
    ...
    // Before ADR-280 this was 0.0: the whole stroke was silently dropped.
  ...
  it('a glyph with a thick body and a thin tail carves the tail too (the "Drive" bug)', () => {
    ...
    // The tail's interior INCLUDING the junction zone (the ADR-280 junction
    ...
    // Before ADR-280: 0.0 — the connector never received a single cut.
