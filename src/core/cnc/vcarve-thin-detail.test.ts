...
import * as polygonDifference from '../geometry/polygon-difference';
...
describe('vcarveThinDetailRings', () => {
  ...
  it('a band thinner than 2·δ (empty first ring) becomes detail rings to its centerline', () => {
    ...
    // The stroke body is rescued; its square corner tips remain finer than
    // the fixed detail pitch and must be disclosed rather than area-filtered.
  ...
  it('artwork the first ring covers produces at most corner-wedge detail', () => {
    ...
    // fine steps, and the remaining tips must be disclosed rather than lost.
  ...
  it('reports a representable 0.008 mm² residual instead of suppressing it by area', () => {
    const small = [rect(0, 0, 0.2, 0.04)];
    const detail = vcarveThinDetailRings(small, [], DELTA_MM);
    expect(detail.rings).toEqual([]);
    expect(detail.residualThin).toBe(true);
  });

  it('reports separated small residual roots without a per-component area floor', () => {
    const small = [rect(0, 0, 0.2, 0.04), rect(1, 0, 0.2, 0.04)];
    const detail = vcarveThinDetailRings(small, [], DELTA_MM);
    expect(detail.rings).toEqual([]);
    expect(detail.residualThin).toBe(true);
  });

  it('reports an even-odd bow-tie whose signed lobe areas cancel', () => {
    ...
    const detail = vcarveThinDetailRings([bowTie], [], DELTA_MM);
    expect(detail.capped).toBe(false);
    expect(detail.rings).toEqual([]);
    expect(detail.residualThin).toBe(true);
  });

  it('preserves generated rings and reports a residual geometry-engine failure', () => {
    vi.spyOn(polygonDifference, 'differenceClosedPolylinesChecked').mockReturnValueOnce(
      err({ kind: 'operation-failed', message: 'fixture failure' }),
    );
    const detail = vcarveThinDetailRings([rect(0, 0, 10, 0.5)], [], DELTA_MM);
    expect(detail.rings.length).toBeGreaterThan(0);
    expect(detail.offsetFailed).toBe(true);
