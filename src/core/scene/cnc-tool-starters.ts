import type { CncTool } from './cnc-tool';

// Starter bit library — common hobby-router bits. Names are mm-first with the
// imperial fraction the bit is physically sold by in parens, so an operator can
// match the bit in hand while the app stays metric. Diameters in mm. Existing
// ids are STABLE (referenced by .lf2 files, the default toolId, and tests) —
// only ever append here.
export const DEFAULT_CNC_TOOLS: ReadonlyArray<CncTool> = [
  {
    id: 'em-3175',
    name: '3.175 mm (1/8") end mill',
    kind: 'end-mill',
    diameterMm: 3.175,
    family: 'straight',
  },
  {
    id: 'em-1588',
    name: '1.588 mm (1/16") end mill',
    kind: 'end-mill',
    diameterMm: 1.588,
    family: 'straight',
  },
  {
    id: 'em-6350',
    name: '6.35 mm (1/4") end mill',
    kind: 'end-mill',
    diameterMm: 6.35,
    family: 'straight',
  },
  {
    id: 'em-9525',
    name: '9.525 mm (3/8") end mill',
    kind: 'end-mill',
    diameterMm: 9.525,
    family: 'straight',
  },
  { id: 'em-1000', name: '1 mm end mill', kind: 'end-mill', diameterMm: 1, family: 'straight' },
  { id: 'em-2000', name: '2 mm end mill', kind: 'end-mill', diameterMm: 2, family: 'straight' },
  { id: 'em-3000', name: '3 mm end mill', kind: 'end-mill', diameterMm: 3, family: 'straight' },
  { id: 'em-6000', name: '6 mm end mill', kind: 'end-mill', diameterMm: 6, family: 'straight' },
  {
    id: 'dc-3175',
    name: '3.175 mm (1/8") downcut end mill',
    kind: 'end-mill',
    diameterMm: 3.175,
    family: 'downcut',
  },
  {
    id: 'cp-6350',
    name: '6.35 mm (1/4") compression bit',
    kind: 'end-mill',
    diameterMm: 6.35,
    family: 'compression',
  },
  {
    id: 'bn-3175',
    name: '3.175 mm (1/8") ball nose',
    kind: 'ball-nose',
    diameterMm: 3.175,
    family: 'ball-nose',
  },
  {
    id: 'bn-1588',
    name: '1.588 mm (1/16") ball nose',
    kind: 'ball-nose',
    diameterMm: 1.588,
    family: 'ball-nose',
  },
  {
    id: 'bn-6350',
    name: '6.35 mm (1/4") ball nose',
    kind: 'ball-nose',
    diameterMm: 6.35,
    family: 'ball-nose',
  },
  // An angled bit's CUT diameter is load-bearing, not decoration: it sets the
  // cone-height depth clamp ((D/2)/tan(θ/2)) and the auto ring pitch (D/8). A
  // name that states only the angle invites picking a 12.7 mm cutter for a
  // 3.175 mm bit in hand — 4x the reachable depth and 4x the ring pitch. Every
  // angled entry therefore names its cut diameter.
  {
    id: 'vb-30',
    name: '30° V-bit — 3.175 mm (1/8") cut',
    kind: 'v-bit',
    diameterMm: 3.175,
    tipAngleDeg: 30,
    family: 'v-groove',
  },
  {
    id: 'vb-45',
    name: '45° V-bit — 6.35 mm (1/4") cut',
    kind: 'v-bit',
    diameterMm: 6.35,
    tipAngleDeg: 45,
    family: 'v-groove',
  },
  {
    id: 'vb-60',
    name: '60° V-bit — 6.35 mm (1/4") cut',
    kind: 'v-bit',
    diameterMm: 6.35,
    tipAngleDeg: 60,
    family: 'v-groove',
  },
  {
    id: 'vb-90',
    name: '90° V-bit — 12.7 mm (1/2") cut',
    kind: 'v-bit',
    diameterMm: 12.7,
    tipAngleDeg: 90,
    family: 'v-groove',
  },
  {
    id: 'eng-15',
    name: '15° engraving bit — 3.175 mm (1/8") cut',
    kind: 'engraving',
    diameterMm: 3.175,
    tipAngleDeg: 15,
    family: 'engraving',
  },
  {
    id: 'vb-90-6350-hobby',
    name: '90° V-bit — 6.35 mm (1/4") cut, 3.175 mm (1/8") shank',
    kind: 'v-bit',
    diameterMm: 6.35,
    tipAngleDeg: 90,
    family: 'v-groove',
    shankDiameterMm: 3.175,
    catalogId: 'v90-hobby-0125',
  },
  {
    id: 'vb-90-12700-hobby',
    name: '90° V-bit — 12.7 mm (1/2") cut, 6.35 mm (1/4") shank',
    kind: 'v-bit',
    diameterMm: 12.7,
    tipAngleDeg: 90,
    family: 'v-groove',
    shankDiameterMm: 6.35,
    catalogId: 'v90-hobby-025',
  },
  // The 1/8"-class angled bits that ship with 3018/desktop routers: the cutter
  // is ground to the shank, so cut diameter equals shank. Their absence forced
  // an operator holding one to pick a 6.35 mm or 12.7 mm entry, which lets the
  // depth clamp allow 3.175 mm / 6.35 mm of depth against a bit that bottoms
  // out at 1.588 mm.
  {
    id: 'vb-90-3175',
    name: '90° V-bit — 3.175 mm (1/8") cut, 3.175 mm (1/8") shank',
    kind: 'v-bit',
    diameterMm: 3.175,
    tipAngleDeg: 90,
    family: 'v-groove',
    shankDiameterMm: 3.175,
    catalogId: 'v90-cut-0125',
  },
  {
    id: 'vb-90-3000',
    name: '90° V-bit — 3 mm cut, 3 mm shank',
    kind: 'v-bit',
    diameterMm: 3,
    tipAngleDeg: 90,
    family: 'v-groove',
    shankDiameterMm: 3,
    catalogId: 'v90-cut-3mm',
  },
  {
    id: 'vb-60-3175',
    name: '60° V-bit — 3.175 mm (1/8") cut, 3.175 mm (1/8") shank',
    kind: 'v-bit',
    diameterMm: 3.175,
    tipAngleDeg: 60,
    family: 'v-groove',
    shankDiameterMm: 3.175,
    catalogId: 'v60-cut-0125',
  },
];
