...
function box(spec: ShapeSpec, bounds: Bounds, transform: Transform = IDENTITY_TRANSFORM): ShapeObject {
...
describe('boardFitRegion', () => {
  it('returns the full scene bounds for a rectangle board', () => {
    const region = boardFitRegion(
      box({ kind: 'rect', widthMm: 100, heightMm: 60, cornerRadiusMm: 0 }, {
        minX: 0,
        minY: 0,
        maxX: 100,
        maxY: 60,
  ...
  it('returns the centered inscribed square for a circle board (art stays inside the arc)', () => {
    ...
    const region = boardFitRegion(
      box({ kind: 'ellipse', widthMm: 100, heightMm: 100 }, { minX: 0, minY: 0, maxX: 100, maxY: 100 }),
    );
  ...
  it('honours the box position + scale (scene space)', () => {
    ...
    const region = boardFitRegion(
      box({ kind: 'ellipse', widthMm: 45, heightMm: 45 }, { minX: 0, minY: 0, maxX: 45, maxY: 45 }, scaled),
    );
