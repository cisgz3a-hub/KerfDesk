...
describe('Marlin fan compiled raster scale', () => {
  it.each([1, 255, 1000, 100])(
    'public image output preserves 50 percent at S max %s',
    (maxPowerS) => {
  ...
  it.each([255, 1000, 100])(
    ...
    (maxPowerS) => {
      ...
      const inlineOutput = marlinStrategy.emit({ groups: [group] }, inline);
      expect(inlineOutput).toContain('M3 I S0');
      expect(inlineOutput).toContain(`S${maxPowerS}`);
      expect(inlineOutput).not.toMatch(/^M4\b/m);
    },
  );
