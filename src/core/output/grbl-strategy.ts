...
import { resolveGcodeDialect, type ResolvedGcodeDialect } from './gcode-dialect';
...
function initialLaserMode(dialect: ResolvedGcodeDialect): 'M3' | 'M4' {
  return dialect.laserModeCommand === 'M4' ? 'M4' : 'M3';
}

function preamble(dialect: ResolvedGcodeDialect): string {
  ...
  // cutting move.
  return ['G21', 'G90', `${initialLaserMode(dialect)} S0`].join(LINE_END) + LINE_END;
}

function postamble(laserAlreadyOff: boolean, dialect: ResolvedGcodeDialect): string {
  ...
  // holds either way.
  const lines: string[] = [];
  if (!laserAlreadyOff) lines.push('M5');
  if (dialect.returnToOriginOnEnd) lines.push(travelLine(0, 0, dialect));
  return lines.length > 0 ? lines.join(LINE_END) + LINE_END : '';
}
...
function emitGroup(group: CutGroup, device: DeviceProfile, dialect: ResolvedGcodeDialect): string {
...
function rasterLaserMode(dialect: ResolvedGcodeDialect): 'M3' | 'M4' {
  return dialect.laserModeCommand === 'M3' ? 'M3' : 'M4';
}

function emitRasterGroupHere(group: RasterGroup, dialect: ResolvedGcodeDialect): string {
...
function emitAnyGroup(group: Group, device: DeviceProfile, dialect: ResolvedGcodeDialect): string {
  switch (group.kind) {
    case 'cut':
    ...
    case 'raster':
      return emitRasterGroupHere(group, dialect);
    default:
...
function requiredPowerMode(group: Group, dialect: ResolvedGcodeDialect): 'M3' | 'M4' | null {
  if (group.kind === 'raster') return null;
  if (dialect.laserModeCommand === 'M3' || dialect.laserModeCommand === 'M4') {
    return dialect.laserModeCommand;
  }
  return group.kind === 'fill' ? 'M4' : 'M3';
}

function powerModeTransition(from: 'M3' | 'M4' | 'off', to: 'M3' | 'M4'): string {
  return `${from === 'M3' && to === 'M4' ? 'M5' + LINE_END : ''}${to} S0${LINE_END}`;
}
...
function emitJob(job: Job, device: DeviceProfile): string {
  const dialect = resolveGcodeDialect(device);
  const parts: string[] = [];
  parts.push(preamble(dialect));
  let mode: 'M3' | 'M4' | 'off' = initialLaserMode(dialect);
  let coolant: CoolantMode = 'off';
  for (const group of job.groups) {
    const nextMode = requiredPowerMode(group, dialect);
    if (nextMode !== null && mode !== nextMode) {
      parts.push(powerModeTransition(mode, nextMode));
      mode = nextMode;
    }
