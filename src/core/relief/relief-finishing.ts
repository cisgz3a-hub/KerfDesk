...
// kernel; continuous chords and subcell surface features retain ADR-289's
...
// of sampled rows no greater than that request; when the global heightmap cap
// has coarsened one cell beyond the request, one row is the irreducible sampled
// stride and the requested scallop is not qualified. Flat bits use a fixed
// fraction of their diameter. Rows alternate direction (serpentine).
