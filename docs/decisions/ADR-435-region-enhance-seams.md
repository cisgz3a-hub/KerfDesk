...
brightness path skips leveling and uses it directly. The ramp test now compares the patch with
the full 2x pass (IoU >= 0.99 for Sharp, Smooth and Centerline) instead of a fixed global cut.
