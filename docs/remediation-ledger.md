...
| UA8-GUARD01 | P1 confirmed policy violations: empty-output Preview was disabled despite the `P` route; Electron dirty unload silently refused close; PWA Update was hidden during activity | Preview merged through PR #719: toolbar, Window menu, and `P` enter the same empty Preview state and show the existing explanation. This app-shell slice surfaces Electron's Leave/Stay choice only when the unchanged renderer idle-dirty predicate requests it, defers process cleanup until accepted quit, keeps ready Update reachable in every machine state, and fixes update actions outside the telemetry scroller. Regression-first coverage failed 10/20 UI assertions plus both missing Electron modules before repair; the final focused matrix passes 7 files/41 tests and compact Chrome passes 3/3 for web and desktop actions at 640/390/320 px. Frame remains the sole ordinary Start guard; packaged Electron close UX and OS shutdown remain external qualification |
...
| UA8-PWA01/02/03 | P2 confirmed: service-worker failure is console-only while Offline can claim readiness; compact side panels can consume the canvas; Update shared a hidden-scroll telemetry row | fixed trailing update-action reachability is implemented in the UA8-GUARD01 app-shell slice; nonblocking service-worker readiness/error state and compact side-panel/canvas ownership remain pending |
...
| UA8-DIR07/08/09 | P2 confirmed: repeated PWA Update owned independent reload latches; autosave reporting permanently suppresses later incidents; G-code Save allows duplicate picker/write invocations | DIR07 is implemented in this app-shell slice: one ready-update epoch owns one retryable promise, rapid clicks coalesce, failed skip-wait listeners/timers are released, a single nonblocking toast reports failure, and the visible action retries. DIR08/09 remain pending coalesced retryable owners with tests; existing identity-bound variable advancement is already correct and is not reopened |
...
| UA8-TXT23 | P2 confirmed disclosure gap: generic Date/Time fields emit UTC, so local operators can see a prior date/time with no UTC label | implemented locally: product choice is the computer's local wall clock, matching LightBurn's documented local date and time; `{{date}}`/`{{time}}` print local fields and `{{datetime}}` prints local seconds plus the UTC offset (`2026-09-24T19:05:07-04:00`) instead of UTC milliseconds with `Z`; the one evaluator serves Preview, arrays, SVG export, Save and Start, which keep their separate current-time snapshots (UA8-BATCH-N3); host-zone and fixed-zone regressions pass; hosted integration pending |
