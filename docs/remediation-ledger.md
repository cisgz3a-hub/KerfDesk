...
| RA-314-04 transient numeric drafts and dialog ownership | confirmed defect: Resize/Canvas/Ink drafts collapsed editable text into numbers and same-id source replacement could receive an older commit | implemented: transient strings are separate from last-valid values; blur restores invalid text; valid represented dimensions display immediately; exact session/source ownership gates mutation; OK/Enter uses the last valid value and does not become a new refusal | 4 Image Studio files and 41 tests passed; scoped ESLint, Prettier, and TypeScript passed in independent review | Browser focus/IME behavior, perception, and assistive-technology integration remain unqualified |
...
- The frozen product-source tree passed `pnpm release:check` with exit 0: TypeScript, full source and
  ...
  rather than misrepresenting documentation-only evidence as part of the earlier byte set.
