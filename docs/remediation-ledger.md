...
| UA8-GUARD01 | P1 confirmed policy violations: empty-output Preview was disabled despite the `P` route; dirty unload silently refuses close; PWA Update is hidden during activity | Preview implemented in this slice: toolbar, Window menu, and `P` all enter the same empty Preview state and show the truthful existing explanation; focused command and browser regressions cover all three routes. Dirty-unload and Update ownership remain pending; keep each reachable/nonblocking and keep Frame as the sole ordinary Start guard |
