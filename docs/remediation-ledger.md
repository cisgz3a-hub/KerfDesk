...
| UA8-VAR14 | P1 confirmed: deferred CSV reads lacked picker-request/document/unmount ownership and could overwrite a newer file or project | implemented on `codex/fix-csv-import-ownership-20260902` and published as PR #716: the Import action claims latest-request and exact `projectDocumentEpoch` ownership before opening the picker; stale selection/read success or failure silently no-ops after a newer request, document replacement, or unmount; current read failures receive nonblocking feedback; focused unit 2 files / 13 tests, targeted picker-path E2E 1 test, typecheck, scoped lint/format, diff/file-size checks, and the full product-tree release gate are green; exact-head hosted CI is pending |
