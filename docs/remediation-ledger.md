...
| T3 | MEDIUM | Remove stale unsupported-raster-rotation warning; delegated audit T3; `src/core/preflight/preflight.ts`; `src/core/preflight/preflight-raster.test.ts`; `WORKFLOW.md`; ADR-028 in `DECISIONS.md` | narrowed by the 2026-08-31 current-main audit | `pnpm vitest run src/core/preflight/preflight-raster.test.ts src/core/job/compile-job-raster-rotation.test.ts src/core/job/compile-job-raster-stream.test.ts` — 31/31 passed; stale unsupported-rotation copy search returned no matches; `pnpm typecheck`, focused ESLint, and Prettier passed | Rotation support is real, but a rectangular pass-through raster at 90/270 degrees retained unswapped grid dimensions; square fixtures did not cover it. No physical rotated burn or external reference-CAM comparison was performed |
...
| TP-HF6 | LOW | numeric editing surfaces used inconsistent `Number`/`parseFloat` handling and could silently normalize invalid locale text | narrowed by the 2026-08-31 current-main audit: the shared parser covers the previously remediated surfaces, but `NumericEditsBar` still used `Number(draft)` and committed a blank X/Y/rotation draft as zero | `english-decimal-input.test.ts` green (7/7) plus Job Review invalid-entry test within 248/248; a mounted current-main probe reproduced blank X `20 -> 0` with one undo entry | Locale-aware number entry is not claimed; `NumericEditsBar` needs the same explicit English-decimal validity contract |
