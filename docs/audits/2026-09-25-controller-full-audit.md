...
- grblHAL keeps a G92 offset through a soft reset and a power cycle by default (`gcode.c:787`, `:833-838`; `$384`, `config.h:882-894`), where stock GRBL and FluidNC clear it. Traced after the merge: KerfDesk forgets the origin at the reset, then re-learns it from the WCO in the first report after the banner (grblHAL `report.c:309`), before Frame can run, so User Origin places the job at the offset grblHAL still applies. The origin then shows as unknown rather than G92, and Work Z must be zeroed again. `grblhal-g92-reset.simulator.test.ts` pins it, and the comments that said every reset clears G92 are corrected.
- The CNC track's note for the PC and stand-alone controllers KerfDesk cannot detect was added after the merge: a CNC export on a GRBL-family profile that no connected controller confirmed says which two commands those controllers read differently (ADR-399 decision 5).
