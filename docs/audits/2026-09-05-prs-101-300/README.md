...
- Operations: after integration with the other merged fixes, 124 tests in 17 files passed, including unchanged emitted G-code for the connected-script regression. Independent review passed 13 grouped checks: 25 legacy field/version cases, 32 override-precedence combinations, six real action allocation-collision scenarios, and exact retained CNC pocket output. The same challenges passed again on the integrated tree. These probes use actual action factories, serialization and compilers; separate DOM tests check Inspector values against output.
