...
The ordinary coordinate transforms and Frame-to-Start placement passed independent software tests. The audit also reproduced defects that the existing suites did not catch. Narrow repairs are implemented locally and their focused regression suites pass. Two broader coordinate-model problems remain explicitly open below. This is software and simulated-controller evidence, not physical machine qualification.

Audit branch: `codex/audit-coordinates-origin-20260921`, based on `288ad66baf23e0c75c0a05f6216787577ddc6812` (fresh `origin/main` when work began). The primary checkout at `C:\Users\Asus\LaserForge-2.0` was on `claude/vcarve-stamp-subcell`, `a37082457b76d56b8c9ba92be27db027b5a92cb0`, with substantial existing edits; it was left untouched. Its HEAD diverged from the audit base by 37 local-only and 396 main-only commits. Results for current main must not be attributed to that older working copy.

No machine was connected, moved, homed, framed, fired, probed, or reconfigured. No firmware values, saved browser profiles or user projects were changed. No changes were pushed, merged or deployed.
...
| Native negative GRBL machine coordinates do not map to the ordinary positive bed correctly | A physically valid stock-GRBL position with negative native MPos appears outside the drawn bed; valid jobs can receive false out-of-bed advisories. | **Open.** Needs a shared native-machine-to-bed transform. Does not prove the Falcon uses that firmware convention. |
| Centred-origin contour entry uses a positive-bed clamp | For a 400 mm centred bed, burn X198→188 can receive an X203 entry runway beyond the actual X200 edge. Frame includes this motion and preflight warns. | **Open.** The planner/emitter need placement-aware envelope data for Absolute and translated modes together; a partial clamp patch could alter relative jobs incorrectly. |

## Verification and coverage

The audit's deduplicated inventory contains **126 files / 1,372 passing correctness checks**, plus **3 separate characterisations reproducing the two open defects**. The machine-readable [verification summary](evidence/verification-summary.json) records each unique file, count, run stage and source log; repeated runs are not added together.
...
These cohorts overlap in some existing files and must not be added as unique coverage without deduplication. Tests which deliberately reproduce an open defect are evidence of that defect, not evidence that the behaviour is correct. Initial failing logs are retained alongside final logs under [evidence](evidence/). Full TypeScript checking, ESLint and Prettier for every changed source file, and the ADR-number check passed. This was not a full release/build/hardware qualification run.
...
The repairs are local on the audit branch and ready for review. Publishing them remains a separate requested action. Confirm the A1 Pro profile in the actual app used with the machine; no live saved A1 profile or firmware snapshot was available in the inspected tabs. The two open coordinate-model issues require a coordinated follow-up change; neither should be hidden behind a new Start policy gate. The 4040 contour-entry issue is not enabled by the ordinary Falcon profiles.
