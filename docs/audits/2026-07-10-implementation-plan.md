...
Total implementation tickets: **203** across 18 epics, covering all **183** verified minor-and-above findings (174 from the 14+4 audit sectors plus 9 Codex-only items). The ticket count exceeds the finding count because several findings split into multiple single-concern PRs — e.g. the CNC M0 critical is a 4-PR stack, and refactor-first work is a separate `tidy` ticket. A coverage check confirmed every finding maps to at least one ticket. 67 tickets carry an `ADR: NEEDED` flag, clustered into 58 provisional ADRs below. Polish items sit in per-epic tables and are not counted as tickets.
...
The same underlying fix was flagged by more than one sector in the cases below (a self-audit of the plan surfaced these). **De-duplicate before scheduling** — implement the owning ticket once and drop or rebase the others. Highest-risk first.

| Collision | Severity | Resolution |
|---|---|---|
| **GCO-09 ≡ LAY-08 ≡ TRC-11** | HIGH | All three add per-layer `imageOverscanMm` to the Layer/`.lf2` schema and read it at `compile-job-raster.ts:93`. **Implement once as LAY-08 (Phase 2)** under **ADR-159**. GCO-09 additionally updates `preflight.ts` `maxOutputOverscanMm` — fold that step into LAY-08. GCO-09's `ADR: none` + Phase-0 placement are both wrong for this feature and are corrected in its ticket. |
| **UI-04 ≡ IMP-03** | HIGH | Both create the *same* new `src/ui/app/import-dispatch.ts`, add a `file.import` command on Ctrl+I, and repoint `shortcuts.ts` `FILE_DISPATCH.i`. **Implement once** (they are the same fix from the Import and UI sectors); drop the duplicate. |
| **UI-09 ≡ CNC-09** | HIGH | Both add the CNC-gated `file.import-stl` command, but with *diverging* signatures — CNC-09 refactors `importStlFiles` to `{name,bytes}[]`, UI-09 keeps `File[]`. **Pick one signature** (CNC-09's `{name,bytes}` avoids the `Blob`-has-no-`.name` problem) and implement the command once. |
| **ARC-02 ⊃ CNV-10** | HIGH | Both convert the same core geometry ops (`combineVectorObjects`/`offsetVectorObjects`/`weldVectorObjects`) and the same five `vector-path-actions.ts` call sites from throw-for-control-flow to a result type — but ARC-02 uses ARC-01's canonical `Result<T,E>` while CNV-10 specifies an ad-hoc `{ ok, reason }` shape (the exact thing ARC-01 exists to eliminate). **Fold CNV-10 into ARC-02**, or make CNV-10 consume ARC-01's canonical `Result`. Do not schedule both — they collide on the same signatures with incompatible types. |
| **GCO-06 ≡ PRV-12** | MED | Byte-identical planner fix (the `capJunctionEntries` min-of-both-nominals clamp in `planner.ts` + the `forwardPass` `exitV` clamp), found independently by the G-code sector and Codex M-30. **One PR**; the second is a no-op/merge conflict otherwise. |
| **TRC-04 ≈ TST-03** | MED | Both build a perceptual instrument that runs emitted image-mode raster G-code back through `rasterizeGcodeBurn` and tone/IoU-compares to source luma. **Coordinate into one instrument** (TRC-04's two-file version subsumes TST-03's single test). |
| **UI-01 ≡ LAY-02** | LOW | Keep the Cuts/Layers list visible while an object is selected. One PR (ADR-181/-179 are the same decision). |
| **PRF-09 + PST-04** | LOW | Autosave: PRF-09 is the code move off synchronous localStorage; PST-04 is the WORKFLOW F-C3/ADR documentation. Distinct PRs, shared **ADR-163** — but PRF-09 must **also** update WORKFLOW.md F-C3 (localStorage+toast → IndexedDB+banner) or the doc PST-04 writes goes stale. |
| **ARC-07 vs TRC-07** | LOW | Both delete `src/core/trace/edge-reconnect.ts`. No hazard (second is a no-op rebase); coordinate so one owns it. |
| **PRF-07 ≈ PRV-11** | LOW | Both target `previewCanvasCache` in `draw-raster-preview.ts`: PRF-07 bounds its growth, PRV-11 (ADR-168) documents the pruning contract. Sequence PRV-11's contract before/with PRF-07's rewrite. |
| **ARC-07 ↔ MCH-01** | (handled) | LaserLog: ARC-07 defers `LaserLog.tsx` deletion to MCH-01, which reroutes its warnings first. Already coordinated in both tickets. |

## ADR register (provisional)

67 `ADR: NEEDED` flags across the tickets cluster into the 58 decisions below (several tickets share one ADR — e.g. the three-way GCO-09 ≡ LAY-08 ≡ TRC-11 all map to ADR-159). **Numbers are provisional** — the board stack reserved through ADR-126, so this allocates from **ADR-127**, but parallel branches may also consume numbers. **Allocate serially at each PR and verify against `DECISIONS.md` before writing** (ADR-numbering collisions are a known trap in this repo). Several tickets share one ADR; three ADRs *amend* an existing one.
...
| 159 | Per-layer image-overscan schema (`.lf2`) | LAY-08 ≡ TRC-11 ≡ GCO-09 |
...
- **⚠ Implementation traps (verified against `streamer.ts` during plan self-audit — do NOT skip):**
  1. **M0 detection must be INSIDE `step()`'s send loop, not a top-of-function `queued[0]` guard.** In char-counted mode the loop batches every line that fits the 120-byte window (`streamer.ts:166-176`); retract/`M5`/park/`M0`/`M3`/`G4` are all tiny and would batch together, so a top-only check sends `M0` inside the same chunk. Detect the lone `M0` as the loop considers each line and `break` the batch *before* appending it (flush what was already batched this call, transition to `'tool-change'`, leave `M0` at head).
  2. **Guard the `sentChunks.length === 0` early-return (`streamer.ts:177`).** If `M0` is the first line this call, the naive `break` would hit that early-return and yield the *original* state, dropping the `'tool-change'` transition — return the transitioned state explicitly.
  3. **Scope the swallow to CNC machine jobs.** The streamer also runs *imported* `.nc` G-code and resume tails, which can contain a plain `M0`/`M1`. Swallowing an imported generic `M0` and showing "Load the next bit" misdescribes an ordinary program pause; `M1` is an *optional* stop most senders ignore. Gate the swallow on the job being a KerfDesk-emitted CNC job (or thread a flag from the compile metadata), and word the prompt generically if the label is unknown.
...
- **Tests:** `src/core/controllers/grbl/streamer.test.ts` (exists) — test-first: a queue containing `M0` mid-stream stops at `'tool-change'` with `M0` unsent and nothing in `toSend`; **crucially, an `M0` that is first in a would-be batch still stops (char-counted-mode regression for trap 1/2)**; `continueToolChange` advances past it and resumes sending the following line; a stream with no `M0` is unchanged (regression guard); `'tool-change'` is non-terminal and `step()` sends nothing while in it.
...
- **Risk:** The new union variant does **not** get compiler-enforced coverage — most `StreamerStatus` consumers match via `.includes()`/`=== 'paused'` (e.g. `laser-store-helpers.ts:38-40`), not an exhaustive `switch`/`assertNever`, so missed call sites must be found by **grep**, not `tsc`. Add a defensive `&& status !== 'tool-change'` to the `done` transition (`streamer.ts:219-227`): today it is safe only incidentally (it requires `queued.length === 0`, and `M0` sits at head so the queue is non-empty), but the guard makes the safety explicit. No G-code change. In-flight lines before `M0` must still drain via acks before the boundary is "safe" (UI gates jog on Idle — handled in CNC-03, not here).
...
- **Risk:** `isActiveJob` is load-bearing (jog/probe/origin/canvas guards all funnel through it). Keeping `'tool-change'` inside `isActiveJob` intentionally keeps Start blocked — CNC-03 introduces the *narrower* predicate that re-permits setup motion, so land CNC-03 in the same series before shipping to users. Note: because `isActiveJob` and `jobNeedsRecovery` use `.includes()`/literal comparisons (not exhaustive switches), TypeScript will not flag every consumer that needs the new status — **grep all `StreamerStatus` / `streamer.status` uses** and audit each for whether `'tool-change'` should be included.
...
- **⚠ Scope correction (verified against `resume-program.ts` during self-audit):** this is **not** a "just add a `ModalState` field" one-liner. `applyLine`/`applyWord` currently scan **word-by-word with no per-line context** (`resume-program.ts:80-104`), so detecting "has Z *and* F *and* no X/Y **on this line**" requires a modest restructure to inspect each line's whole word set before folding it into the modal state. Budget for that restructure. Also note: **path3d ramp plunges after the first** are emitted as `G1 X Y Z F` (`cnc-grbl-strategy.ts:337`), so they carry X/Y and won't match the plunge signature — they correctly fall back to `options.plungeMmPerMin`, which is safe (no worse than today's behavior). Only the pure Z-down plunges are reconstructed; state that limitation in the PR.
- **Files:** `src/core/controllers/grbl/resume-program.ts` (modify — includes the per-line word-set restructure, not only a field add).
- **Tests:** `src/core/controllers/grbl/resume-program.test.ts` (exists) — test-first: a program whose last plunge before the resume line is `G1 Z-1.5 F250`, built with `options.plungeMmPerMin=300`, must emit the re-entry `G1 Z... F250` (fails today → passes after). Add a case where the last move before resume is a ramp `G1 X Y Z F` — assert it does **not** hijack the plunge feed (falls back to the option). Keep a case with no prior plunge that still falls back.
...
- **Effort:** M · **Depends on:** none (loosely related to the DEV `.lbdev`-import controllerKind-mapping polish item)
...
- **DUPLICATE — implement once as LAY-08.** This is the same per-layer `imageOverscanMm` `.lf2` schema change as **LAY-08 ≡ TRC-11** (three-way collision — see the cross-sector-duplicate table). Do not build GCO-09 standalone; fold its one distinctive step (updating `preflight.ts` `maxOutputOverscanMm` to read the per-layer value) into LAY-08, which owns the feature in Phase 2. This ticket is retained only to record that preflight-consistency step and the corrected ADR/phase.
...
- **ADR:** NEEDED — **ADR-159** (per-layer image-overscan `.lf2` schema change; the same ADR as LAY-08/TRC-11). *Corrected from the earlier `ADR: none`: an additive `.lf2` schema field is a data-model change the other two sectors correctly flag for an ADR.*
- **Effort:** M (subsumed by LAY-08)  ·  **Depends on:** LAY-08 (owner). **Not a Phase-0 item** — per-layer overscan is a parity feature, scheduled with LAY-08 in Phase 2, not with the G-code-safety epic.
...
- **Approach:** The terminal fix is Windows code signing (electron-updater then verifies the publisher signature automatically) but that is ops/secret-gated and cannot be landed or verified as a code PR here — track it as the follow-on. This ticket adds the interim, app-owned integrity control the audit calls for. Deliver as a short stack (each part its own PR):
  ...
  - **Alternative (smaller, and the SAFER interim — recommended if interposition is uncertain):** switch to notify-only — `autoInstallOnAppQuit=false` and require an explicit operator action to install. This removes the silent-install RCE window with a ~5-line diff in `auto-update.ts`, but ADR-024 §Decision(2) deliberately chose *full* auto-update over notify-only, so this needs the ADR below plus maintainer sign-off. **⚠ Why this may be the better primary, not just a fallback:** the ed25519 gate's guarantee hinges entirely on the verify sitting on electron-updater's *own* download path. electron-updater performs its own unauthenticated `latest.yml` fetch at download time — a TOCTOU gap — so unless the custom logic *fully replaces the provider* (not just races a second fetch), the verify becomes advisory and the silent-install window stays open. Notify-only closes that window unconditionally regardless of the 6.8.9 internals. Prefer it as the interim unless a maintainer confirms robust provider interposition in 6.8.9; keep the ed25519 signature work (step 1) either way, since code signing is the terminal fix and the `.sig` is reusable.
...
- **Effort:** M (the docs writing dominates)  ·  **Depends on:** GCO-08 (LIGHTBURN-STUDY.md restoration) for the two core-comment citations — this ticket can land its WORKFLOW.md + two sector-owned comment edits independently, only the `preview-data.ts`/`grbl-strategy.ts` comment cleanups wait on GCO-08
...
- **Approach:** Decision-first (maintainer choice). Option A (parity): drop the default override in `operationOverrideForTrace` — return `undefined` unless the user explicitly picks a fill style, letting layer mode govern. Option B (keep product behavior): record the trace-time Fill override as an ADR and leave code as-is. If A, keep the Fill-style picker but treat "Scanline" as "no override / inherit" vs an explicit fill choice. It interacts with a `TracedImage`-elimination refactor (a Codex consolidated-audit architectural note, `DECISIONS.md:1080`) that is **not a ticket in this plan** — if that refactor is ever scheduled, coordinate ordering; otherwise Option A stands alone.
...
- **Effort:** S · **Depends on:** none in this plan. (Interacts with a `TracedImage`-elimination refactor that is a Codex architectural note, not a plan ticket — coordinate only if that work is later scheduled.)
...
- **Approach (two PRs):**
  1. **Orphan deletion:** delete the three provably-dead files (`edge-reconnect.ts`, `project-validator-primitives.ts`, `material-library-panel-test-helpers.tsx`). **`LaserLog.tsx` is NOT deleted here — it is owned by MCH-01**, which first reroutes the operator warnings (wrong-baud, banner-mismatch, wake-lock) onto the mounted transcript surface and *then* deletes `LaserLog.tsx`. Deleting it standalone in this ticket would silently drop those warnings on the floor. Sequence: MCH-01 lands, then this PR need not touch `LaserLog.tsx` at all. If MCH-01 is deferred, this ticket must not delete `LaserLog.tsx` either.
...
- **Files:** delete `src/core/trace/edge-reconnect.ts`, `src/io/project/project-validator-primitives.ts`, `src/ui/layers/material-library-panel-test-helpers.tsx` (NOT `LaserLog.tsx` — see MCH-01); modify or delete `src/core/controllers/{marlin,smoothieware,fluidnc,grblhal}/index.ts` + `select-controller-driver.ts` + `controllers/index.ts`.
...
- **ADR:** none.
- **Effort:** S  ·  **Depends on:** ARC-06 (only for the barrel-convention half, to stay consistent) — the three-file orphan deletion depends on nothing; the `LaserLog.tsx` deletion is deferred to and owned by MCH-01.
- **Risk:** Low — the three orphans are provably unreferenced. The one coordination point is `LaserLog.tsx`: it must be deleted by MCH-01 (after rerouting), never here, so its warnings are not lost.
