...
| Stage | Delivers | Gate |
|---|---|---|
| **DS-0** | Governance: ADR-268, PROJECT.md phase + scope amendments, WORKFLOW.md `F-DS` flows | `check:adr-numbers` green |
...
4. **Where depth lives** — recommendation: **per design entity**, materialized to a
   per-object override at Apply. Requires extending `ObjectOperationOverride`; ADR-268
   records it.
