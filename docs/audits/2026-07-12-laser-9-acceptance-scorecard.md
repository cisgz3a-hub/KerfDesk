...
| Materials and migration | 10% | 7.2 | 9.2 | Five licensed external CLB files; five licensed LBRN2 projects; linked snapshots; browser workflow; deterministic import reports | Monitor compatibility with future LightBurn format versions |
...
- External migration acceptance: five pinned CLB fixtures under CC0/MIT and five pinned LBRN2 projects under CERN-OHL-P-2.0 import with retained license texts and SHA-256 provenance. The corpus found and fixed real `Value`-attribute fields, nondeterministic project IDs, and LightBurn 2 shared `VertID`/`PrimID` geometry.
...
The executable procedure, measurement limits, evidence bundle, and independent Labs-removal rules are defined in [`docs/hardware/laser-9-acceptance-protocol.md`](../hardware/laser-9-acceptance-protocol.md).
...
2. No Labs gate is removed until its own matrix row passes; unrelated passing features do not unlock it.
