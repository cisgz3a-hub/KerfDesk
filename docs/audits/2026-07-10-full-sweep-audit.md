# KerfDesk consolidated full-sweep audit — 2026-07-10

> **Authoritative consolidated report.** This document reconciles Claude's 14-sector static audit with an independent Codex source, live-UI, architecture, safety, recovery, Electron, and release-gate audit. Product source was not modified. The only new artifact is this report.

## 1. Document control

| Field | Value |
|---|---|
| Canonical checkout | C:\Users\Asus\LaserForge-2.0 |
| Audited branch | main |
| Audited commit | d0d84c01ae8bafa5350a2012a145c205ea9c9f08 |
| Remote | https://github.com/cisgz3a-hub/KerfDesk.git |
| Working tree at audit start | Clean |
| Remote movement during audit | origin/main advanced to 40bd8194 after the audit began; that rotary/emitter commit is not covered here |
| Claude source report | Hidden worktree admiring-hamilton-666624, branch claude/multi-sector-audit-3447b9, same base commit |
| Claude source SHA-256 | E5679D20B88A624CE1B2351BAA693027414AF1E684C53AEABAE3CFF8AF07B80D |
| Consolidation rule | Deduplicate root causes, preserve disagreements, downgrade unsupported severity, and distinguish static evidence from live or hardware evidence |

Sections 1–14 of this document are the reconciled authority. Appendix A preserves Claude's original 1,977-line report, with line endings normalized, so no detail is lost. Where an Appendix A heading conflicts with the reconciled register, the reconciled register controls.

## 2. Executive verdict

**Yes, KerfDesk is good code.** Its architectural spine is unusually strong for a solo-built CAM and machine-control product: pure-core boundaries, one prepared-output path, deterministic emitters, explicit invariants, a centralized command system, hardened Electron configuration, and a large automated test suite.

The weaknesses are concentrated at integration seams:

- UI entry points that reach the same capability through different state or configuration.
- Controller state that is acknowledged by firmware but not modeled by the application.
- Safety checks that cover most of a motion envelope but not every producer or geometry form.
- Powerful features that are present but buried, compressed, or represented differently across modes.
- Static performance hot paths around preview, 3D simulation, raster work, and queue mutation.
- Documentation and ADR indexes that have fallen behind the implementation.

There is **no confirmed P0 defect** in the audited snapshot. There are nine directly evidenced P1 items plus one P1 beginner-default risk that deserve correction before adding substantial new machine-control surface area. A previously proposed arc-bounds P1 was downgraded to latent hardening after producer tracing.

Claude summarized this as **“the spine is excellent; the edges have drifted.”** The independent audit agrees.

## 3. Evidence legend

| Mark | Meaning |
|---|---|
| SC | Confirmed by tracing the source at the pinned commit |
| TV | Confirmed by an automated test or completed quality gate |
| LB | Confirmed in the live browser application |
| STATIC | Mechanism is visible in code, but runtime magnitude was not measured |
| HW | Requires a controller, camera, spindle, laser, or physical stock to verify |
| REF | Comparison with LightBurn, Easel, Carbide Motion, or another reference product; not independently exercised during this audit |
| DOC | Documentation or architectural-decision drift rather than a runtime defect |

Reference-product differences are treated as parity opportunities unless KerfDesk explicitly adopts “all undocumented divergence is a bug” as product policy.

## 4. Unified scorecard

| Sector | Grade | Consolidated assessment |
|---|---:|---|
| Core architecture and dependency direction | A- | Strong boundaries and deterministic core; state ownership and public-surface governance need tightening |
| Prepared output and G-code strategy | A- | Excellent single pipeline and emitted-text invariants; a few defense-in-depth and special-path gaps remain |
| Machine streaming and controller lifecycle | B- | Strong GRBL state machine, weakened by M0, connection-entry, resume, and physical-completion seams |
| CNC workflow | B- | Real machining depth and good beginner layering, but tool change, zeroing, and restart safety need work |
| Import and file I/O | B+ | Mature hostile-input handling; universal import, embedded images, repeat-import semantics, and large-file UX lag |
| Canvas and content creation | B+ | Broad and coherent, with discoverability and editing-depth gaps rather than structural weakness |
| Layers and cut settings | B | Capable settings model; selection collapse, preset policy, and hidden advanced semantics hurt workflow |
| Camera and registration | B | Ambitious and well-factored math; frame mapping and calibration-resolution contracts are incomplete |
| Trace and raster fidelity | B | Strong trace investment; raster rotation, resampling, and realistic fidelity coverage are weaker |
| UI information architecture | B- | Accessible and mode-aware, but fixed rails, tiny controls, long menus, and buried features limit usability |
| Recovery, persistence, and offline behavior | B- | Thoughtful autosave/checkpoint design with important lifecycle and storage-boundary defects |
| Performance and robustness | B- | Several confirmed static hot loops; no production-scale profile was collected |
| Electron and desktop security | B+ | Hardened shell; adapter contract and packaged-runtime test coverage have drifted |
| Test and CI quality | B+ | Very large passing suite and strong release gate; browser E2E and dialect/fidelity symmetry are missing |
| Documentation and governance | C+ | Rich source material, but pervasive workflow drift and incomplete ADR/index enforcement |

## 5. Authoritative priority register

### M-01 — P1 — CNC M0 tool-change state is not an application barrier

**Sources:** Claude S8-F1; independent P1 finding.  
**Evidence:** src/core/output/cnc-grbl-strategy.ts:145-160; src/core/controllers/grbl/streamer.ts:100-108,153-187; src/ui/laser/JobRunControls.tsx:16-49; src/ui/state/laser-probe-actions.ts:20-29; src/ui/state/laser-origin-actions.ts:36-43,76-82.  
**Status:** SC, TV around individual pieces; HW workflow not exercised.

The emitter retracts, stops the spindle, parks, emits operator instructions, and writes M0. The streamer drops comment lines, does not enter an app-side tool-change state, and may continue filling the controller buffer. Resume only appears for the application's own paused state. Jog, probe, and Z-zero are blocked while a job remains active.

**Impact:** The documented bit-swap and Z re-zero workflow cannot be completed safely inside the application. A physical cycle-start can continue with the wrong tool-length reference.

**Required correction:** Add a structured tool-change boundary to compile/stream metadata. Stop queue filling at that boundary, keep the controller in a safe non-cutting state, name the required bit, temporarily permit guarded jog/probe/Z-zero, and require explicit continuation.

**Regression test:** A multi-tool program must stop before any post-M0 motion, expose the tool identity, permit only the intended setup actions, and resume exactly once after acknowledgement.

### M-02 — P1 — Duplicate Connect surfaces select different controller behavior

**Source:** Independent audit; not found by Claude.  
**Evidence:** src/ui/laser/LaserWindow.tsx:37-41,72-77; src/ui/commands/use-app-commands.ts:251-259; src/ui/state/laser-store.ts:60-65; src/ui/state/laser-connection-actions.ts:50-75; src/ui/commands/laser-command-family.ts:9-48.  
**Status:** SC.

The right rail passes the selected profile's controller kind and baud rate. Menu Connect calls the store without those options, allowing fallback to GRBL and the default baud. Its enablement also does not consistently account for file-only profiles.

**Impact:** Marlin, Smoothieware, custom-baud, and file-only profiles can behave differently depending on which Connect button the operator uses.

**Required correction:** One profile-aware connection command must power every UI entry point and reject non-connectable profiles before opening a transport.

**Regression test:** Exercise both entry points across GRBL, Marlin, Smoothie, custom baud, and file-only device fixtures and assert identical driver/baud selection.

### M-03 — P1 — CNC resume reconstructs plunge motion with a hard-coded F300

**Source:** Independent audit; not found by Claude.  
**Evidence:** src/ui/laser/start-job-flow.ts:147-157,182; src/core/controllers/grbl/resume-program.ts:26-30,140-158; src/ui/layers/CncLayerAdvancedFields.tsx:90-100.  
**Status:** SC.

The recovery flow passes RESUME_PLUNGE_MM_PER_MIN = 300 instead of reconstructing the last safe downward Z feed from the interrupted program.

**Impact:** A job whose intended plunge was F50 can re-enter at F300, risking the bit, stock, and machine.

**Required correction:** Recover the last valid downward Z feed from parsed modal state or map the checkpoint to the compiled CNC group. Refuse CNC resume when a safe feed cannot be established.

**Regression test:** Interrupt and resume fixtures with multiple plunge feeds and assert the re-entry descent never exceeds the interrupted operation's configured plunge rate.

### M-04 — P1 — Tiled CNC export bypasses controller-readiness confirmation and standard provenance

**Source:** Independent audit; not found by Claude.  
**Evidence:** src/ui/app/file-actions.ts:108-121,148,163-211; src/ui/app/save-tiled-gcode.ts:19-26,63-85; src/core/preflight/controller-readiness.ts:101-148; src/io/gcode/gcode-metadata.ts:41-68.  
**Status:** SC.

The tiled path returns before the normal controller-readiness confirmation and metadata composition. It does run prepareOutput and CNC preflight for every tile before writing, so this is **not** a general preflight bypass.

**Impact:** Incorrect controller settings such as $30 or $32 can go unacknowledged, and exported tiles lose normal provenance information.

**Required correction:** Share the standard readiness and metadata layer with normal Save while preserving the all-tiles-preflight-before-any-write invariant.

**Regression test:** Reject or confirm bad readiness once per tiled export, write nothing if any tile fails, and verify metadata in every tile.

### M-05 — P1 — Recovery checkpoint is deleted before physical completion

**Source:** Independent audit; Claude explicitly left recovery unaudited.  
**Evidence:** src/core/recovery/job-checkpoint.ts:39-46; src/ui/app/use-job-checkpoint.ts:30-46; src/ui/state/laser-post-job-settle.ts:21-39,42-79.  
**Status:** SC, TV for the current early-clear behavior.

A streamer “done” means all commands were acknowledged as parsed, not that the motion planner and machine are physically idle. The checkpoint is cleared at “done” while a separate settle path still waits for dwell plus stable Idle reports.

**Impact:** A crash, disconnect, or power loss during final planner drain can leave no recovery checkpoint.

**Required correction:** Preserve the checkpoint through streamer completion and clear it only after stable post-job Idle succeeds.

**Regression test:** Simulate all-lines-acknowledged with non-Idle status and verify the checkpoint remains until the stable-Idle sequence completes.

### M-06 — P1 — Two custom dialogs do not participate in the modal safety contract

**Source:** Claude S11-F5, independently verified.  
**Evidence:** src/ui/library/DesignLibraryDialog.tsx:35; src/ui/relief-viewer/Viewer3DDialogShell.tsx:18; src/ui/kit/Dialog.tsx:26; src/ui/laser/use-job-shortcuts.ts:19.  
**Status:** SC.

Design Library and the 3D viewer do not register modal depth or install the shared focus/Escape behavior. Global job shortcuts therefore remain eligible behind them.

**Impact:** Ctrl+Enter can reach Start while the operator believes a modal dialog owns focus.

**Required correction:** Rebuild both on the shared Dialog contract or extract and use the same modal-registration/focus-trap hook.

**Regression test:** With either dialog open, Start/Pause/Stop shortcuts must be suppressed except for an explicitly allowed emergency-stop policy.

### M-07 — P1 — Camera alignment is not bound to the displayed frame geometry

**Sources:** Claude S7-F1 and S7-F2.  
**Evidence:** src/ui/camera/NetworkCameraView.tsx:32,257; src/core/camera/camera-alignment.ts:10; src/ui/camera/WorkspaceCameraOverlay.tsx:31; src/ui/camera/trace-from-camera.ts:45; src/platform/web/web-camera.ts:68.  
**Status:** SC; HW not verified.

Manual clicks are mapped across the full element rectangle even though the camera image uses contain/letterboxing. Persisted alignment records frame dimensions, but overlay and trace paths do not reject mismatched resolutions; USB capture does not pin a resolution.

**Impact:** Non-4:3 streams or later resolution changes can silently shift registration.

**Required correction:** Map pointer coordinates through the actual rendered image rectangle and make calibration resolution/crop an enforced compatibility key.

**Regression test:** Synthetic 16:9, 4:3, and changed-resolution fixtures must map the same board corners within an explicit tolerance or force recalibration.

### M-08 — P3 — Latent CNC arc text scanning does not include arc extrema

**Source:** Claude S5-F6, independently verified.  
**Evidence:** src/core/invariants/predicates.ts:101; src/core/output/cnc-grbl-strategy.ts:262; src/ui/laser/start-job-readiness.ts:142.  
**Status:** SC as a library limitation. Producer tracing found no normal production CNC compiler path that currently emits arc passes; direct constructed jobs and tests can reach it.

G2/G3 output includes I/J center offsets, but the final text bounds scanner evaluates endpoints rather than the complete arc sweep. The stronger sampled-placement check is not applied in every Absolute-placement case.

**Impact:** If a production compiler begins emitting native arcs, an arc whose endpoints are inside the bed but whose bulge crosses a bed edge or no-go zone can pass the last emitted-text bounds check. This is not a demonstrated current user path.

**Required correction:** Compute arc extrema from modal start, endpoint, center offsets, and direction, and include them in bed/no-go validation.

**Regression test:** Clockwise and counter-clockwise arcs with in-bounds endpoints and out-of-bounds extrema must fail Start and Save.

### M-09 — P1 — Complexity and output-size protection is not shared by all consumers

**Source:** Independent audit, related to Claude S13 performance findings.  
**Evidence:** src/core/job/preparation-complexity.ts:17; src/ui/workspace/draw-preview.ts:116; src/io/gcode/prepare-output.ts:68; src/core/raster/raster-budget.ts:12; src/core/raster/emit-raster.ts:88,390; src/ui/laser/use-frame-action.ts:41.  
**Status:** SC, STATIC; no large-job benchmark was run.

Vector/fill complexity guards are concentrated in Preview. Start, Save, and Frame can still synchronously compile very large vector/fill work. Raster budgets estimate pixels but do not constrain the number of emitted commands or final string size.

**Impact:** Safety-critical actions can freeze the renderer or exhaust memory on valid but extreme jobs.

**Required correction:** Move shared input, compiled-segment, emitted-command, and output-byte budgets into prepareOutput. Use worker/chunked execution where cancellation and progress matter.

**Regression test:** Every consumer must reject the same over-budget project before allocation-heavy emission; near-limit fixtures must remain deterministic.

### M-10 — P1 — CNC Start has no work-zero readiness advisory

**Source:** Claude S8-F2.  
**Evidence:** src/core/output/cnc-grbl-strategy.ts:10-13; src/ui/laser/start-job-readiness.ts:116-123,198-225; src/ui/laser/machine-job-warnings.ts:17-28.  
**Status:** SC; HW not verified.

The emitter assumes Z0 is the stock top, but Start does not confirm or warn that a usable work offset, especially Z, has been established.

**Impact:** A beginner can home the machine and start a job with a work coordinate unrelated to the stock surface.

**Required correction:** Add a CNC pre-carve checklist and a session-scoped work-zero advisory. Keep it a warning if the application cannot prove controller state, but make the assumption explicit.

**Regression test:** CNC Start without a session zero must surface the advisory; laser jobs and explicitly confirmed CNC setups must not be blocked incorrectly.

## 6. Additional P1, P2, and P3 findings

| ID | Finding | Evidence and treatment |
|---|---|---|
| M-11 | Malformed non-finite coordinate words are invisible to the final scanner | src/core/invariants/gcode-words.ts:4-12 and predicates.ts:111-126. Real defense-in-depth hole, but no currently reachable user producer was demonstrated. Reject malformed present words distinctly from absent words. |
| M-12 | Operator warnings are written to an unmounted log | src/ui/laser/LaserLog.tsx:12 and laser-connection-actions.ts:172. Route safety and recovery warnings through mounted status/toast surfaces with persistence and acknowledgement. |
| M-13 | Frame verification state is invisible and signs less than the complete motion story | src/core/job/frame-verification.ts:1-24; src/ui/laser/use-frame-action.ts:51-112; src/core/job/job-bounds.ts:66-171. Frame is a burn-envelope operation, not a preflight bypass; label burn versus full-motion envelope and show Frame verified / Re-frame required. |
| M-14 | PWA Reload can appear in disconnected-but-incomplete job states | src/ui/pwa/PwaUpdatePrompt.tsx:27-29,64-82; src/ui/state/laser-store-helpers.ts:35-40,249-280. Introduce one unsafeToReload predicate covering preserved streamers, recovery state, and unsettled jobs. |
| M-15 | Static performance hot paths cross flagship workflows | src/ui/workspace/Cnc3DPane.tsx:30; use-preview-toolpath.ts:18; src/ui/state/store.ts:414; src/core/controllers/grbl/streamer.ts:162; compile-job-raster.ts:36. Memoize stable selectors, replace repeated queue slice with an index/deque, cache decoded luma, then profile before assigning runtime targets. |
| M-16 | Raster rotation is blocked; burn-grid resampling is nearest-neighbor; realistic-scale perceptual coverage is absent | src/core/preflight/preflight.ts:298; src/core/raster/luma-resample.ts:27; src/core/job/compile-job-raster.ts:60. Existing units cover luma direction, mirroring, and row orientation; the remaining gap is resampling/composition fidelity at realistic scale. |
| M-17 | **P1:** A new CNC layer defaults to a full-depth outside profile with tabs disabled | Stock and layer depth both default to 6.35 mm, profile-outside, with tabs false: src/core/scene/machine.ts:190-230. Missing settings compile through this fallback and preflight accepts it. Default to a shallow/non-releasing operation or require an explicit through-cut/no-tabs acknowledgement. |
| M-18 | Autosave serializes the entire project synchronously to localStorage | src/ui/state/autosave.ts and src/ui/app/use-autosave.ts. Move image/relief payloads to IndexedDB or OPFS and retain lightweight metadata/checkpoints in localStorage. |
| M-19 | Responsive shell compresses the primary workspace | Live at 1366×768, the CNC canvas fell to roughly 292 px until 3D was collapsed. Fixed widths originate in Cnc3DPane.tsx:164-176, CutsLayersPanel.tsx:106-113, and LaserWindow.tsx:239-250. Add breakpoints, resizable/tabbed rails, and sticky Frame/Start/Stop actions. |
| M-20 | Control targets are frequently small | In one live CNC state, 99 buttons were visible and 60 had at least one dimension under 24 px. This is not automatically a WCAG failure, but warrants a target-size and spacing audit. |
| M-21 | Tools and feature entry points are overloaded or hidden | Tools had about 29 entries and scrolled at 720 px. STL is drag-drop only; Design Library is “Lib”; selecting an object collapses Layers and Material Library. Use grouped menus/command search, a visible STL command, and persistent layer context. |
| M-22 | Electron's documented platform contract has drifted | src/ui/app/main.tsx reuses webAdapter with an Electron id while ADR/project prose describes distinct native behavior. Security hardening is strong; supersede the ADR or implement a preload/contextBridge file adapter and add packaged smoke coverage. |
| M-23 | No browser E2E or packaged Electron workflow suite | Manual live validation passed, but there is no Playwright workflow harness and no routine packaged-runtime smoke test. Add import → layer → preview → save and connect/mock-stream golden paths. |
| M-24 | Marlin and Smoothieware property coverage is thin beside GRBL | Reuse the GRBL determinism, bounds, power, and laser-off properties across dialect strategies and add production-composition snapshots. |
| M-25 | Store ownership and subscription boundaries need tightening | Module-level mutable laser lifecycle references conflict with the documented ownership rule; CommandShell subscribes broadly and rebuilds on status changes. Add a store factory/dispose boundary and selector-specific subscriptions. |
| M-26 | ADR and workflow governance has become unreliable | The ADR index omits most later decisions and still foregrounds superseded licensing state. WORKFLOW, PROJECT, help, shortcuts, trace, camera, and CNC prose drift repeatedly. Generate the index and link claims to tests or explicit scope decisions. |
| M-27 | **P2:** Enlarging imported SVG curves scales their flattening error | src/io/svg/flatten-curves.ts:6-12; parse-svg.ts:210-229; src/core/job/compile-job.ts:397-419. Curves are flattened at 0.25 mm during import and later transforms scale those line segments. Preserve curve primitives or flatten adaptively at output. |
| M-28 | **P2:** Geometry-operation failures can be swallowed | src/core/geometry/vector-path-booleans.ts:35-119 and src/ui/state/vector-path-actions.ts:127-199. Core creates useful messages, while several UI paths return unchanged state. Standardize typed results and surface actionable feedback. |
| M-29 | **P2:** Preview converts placement failure into a misleading empty-output diagnosis | src/ui/workspace/use-preview-toolpath.ts:58-69 and preview-overlays.tsx:18-34. Preserve the typed placement failure and show its messages instead of “enable Output.” |
| M-30 | **P2:** Planner ETA can assign a block exit velocity above that block's target | src/core/job/planner.ts:223-263,293-313. This affects estimates rather than controller motion. Clamp a junction to both adjacent target velocities and add a slow-to-fast block test. |
| M-31 | **P3:** Undefined theme token and unshipped help paths | Six components reference an undefined --lf-bg token, and help alerts name repository docs that are not packaged. Correct the token alias and ship/link the full guides or remove the stale paths. |

## 7. Feature and architecture map

| User workflow | Primary implementation | Data/mechanism | Consolidated assessment |
|---|---|---|---|
| Import and create | src/ui/app file actions, drag-drop hooks, src/io/svg, src/io/dxf, image/STL actions | Format-specific parser → SceneObject → project store/history | Strong hostile-input handling; universal import, embedded SVG image support, repeat-import choice, and STL visibility are missing |
| Canvas editing | src/ui/workspace, drawing tools, geometry actions | Pointer/tool state → scene mutations → history snapshot | Coherent and broad; text, node editing, nested groups, snap configuration, and feedback are shallower than the rest |
| Layers and materials | src/ui/layers, material library, project/CNC material state | Object color/layer → laser or CNC operation settings → compiled group | Powerful, but selection hides layer context and advanced overrides/default policies are insufficiently explained |
| Preview and planning | draw-preview, use-preview-toolpath, estimate and 3D/removal grid | prepareOutput result → route statistics, playback, bounds, removal model | Output parity is excellent; recomputation, placement-error copy, color/power visualization, and time realism need work |
| Save and Start | file-actions, emit-gcode, start-job-readiness | prepareOutput → strategy → emitted text → preflight → save or streamer | This is the architectural spine and should remain single-path; tiled export and resume are special-path exceptions to fix |
| Machine control | LaserWindow, laser store/actions, controller drivers | Device profile → transport/driver → streamer → realtime/status handling | Deep and safety-conscious; Connect divergence, invisible log, M0 state, and non-GRBL asymmetry are the main seams |
| CNC | compile-cnc-job, cnc-grbl-strategy, CNC panels and 3D view | Scene/layer settings → tool sections/passes/tabs → Z-aware GRBL | Substantial real CNC capability; work-zero, tool changes, resume feed, tabs, and large-job behavior need priority |
| Camera/registration | core/camera, camera UI, board/jig state | Frame calibration/homography → workspace overlay or captured board | Strong math separation; rendered-frame mapping and calibration compatibility need an explicit contract |
| Trace/raster | core/trace, trace worker, core/raster, compile-job-raster | Image/luma → trace paths or raster samples → layer operation/output | Trace is highly developed; raster transform/resampling/fidelity testing is the weaker half |
| Persistence/recovery | project serializer, autosave, job checkpoint, PWA | .lf2 validation/migration plus browser storage and streamer checkpoint | Thoughtful hostile-input and recovery design; early checkpoint clear, sync storage, and unsafe reload state require fixes |
| Desktop shell | electron/main, platform adapters | Hardened BrowserWindow/custom protocol plus web-oriented renderer adapter | Strong security posture; native capability contract and packaged verification are incomplete |

## 8. What must be preserved

1. **One prepared-output spine.** Preview, Save, Start, and estimates mostly consume the same deterministic compilation result.
2. **Emitted-text safety checks.** Laser-off travel, bounds, CNC plunge/travel, over-depth, and dropped-layer checks validate final output rather than trusting settings alone.
3. **Core purity and dependency direction.** Domain logic is unusually separable and testable for a UI-heavy machine application.
4. **Streamer terminal-state discipline.** Alarm/error/disconnect behavior, oversized-line handling, and acknowledgement tracking are carefully designed.
5. **Mode-gated command architecture.** A centralized command registry feeds menus, toolbars, shortcuts, help metadata, and machine gating.
6. **Hostile-input project loading.** Schema versioning, migrations, bounds, integer-bomb defenses, and path-specific errors make .lf2 loading robust.
7. **Electron hardening.** CSP, custom-protocol traversal protection, context isolation, sandboxing, disabled Node integration, and permission controls are good.
8. **Offline and local-first behavior.** No unnecessary telemetry and practical browser-local recovery are meaningful product advantages.
9. **Advanced placement and fabrication features.** Place Board, registration workflows, multi-operation CNC, relief, tiling, and generators are more ambitious than the interface currently communicates.
10. **Test breadth.** Deterministic, property, perceptual, controller, and UI tests create a strong base for targeted seam regression tests.

## 9. Claude report reconciliation

Claude's audit is valuable and much broader than the independent audit in parity, camera, trace, static performance, help, and detailed sector inventory. Its own adversarial verification also caught several overstatements that were not propagated back into headings or totals.

| Claude item | Reconciled treatment |
|---|---|
| S2-F1 boolean/offset feedback marked Major | Mechanics hold, but silent empty results are documented and tested. Treat as P2 feedback/contract drift, not a major defect. |
| S8-F4 default feeds called a “cut-wrecking” Major | Downgrade to minor workflow guidance. Claude's verifier calculated that suggested material-derived feeds are actually faster/deeper; the proposed fix does not cure the claimed numeric hazard. |
| S7-F4 says normal Replace can destroy registration | Narrow to provenance-blind unlock/repurpose risk. Normal replacement preserves position, size, and lock unless the operator changes them. |
| S14-F2 says inversion, mirroring, and dither mapping could all pass | Existing unit tests cover those classes. Keep the realistic resampling/composition perceptual gap. |
| S5-F2 curve scaling | Strong static fidelity concern: scaling already-flattened curves multiplies chord error. Do not call it a demonstrated visible defect until representative transforms are rendered. |
| S10-F1 says Result discipline “does not exist” | Reword to inconsistent Result/throw handling at geometry and action seams; many tagged-result patterns do exist. |
| Performance described as pathological | Static hot paths are confirmed; runtime magnitude needs profiling on representative projects. |
| LightBurn behavior used as fact | Claude did not run LightBurn. Keep these as REF parity observations unless supported by KerfDesk's adopted divergence policy. |
| Raw total of 157 | Not a unique-defect count. It includes repeated root causes across sectors, including S1-F1=S5-F1, S1-F4=S11-F4, S3-F2=S11-F1, S8-F5=S11-F9, and S9-F4=S14-F2. |

## 10. Consolidated sector backlog

The full low-severity evidence remains in Appendix A. The following grouping is the actionable interpretation.

### Import and file I/O

- Replace three primary picker commands with one universal Import on Ctrl+I; keep type-specific commands as secondary.
- Decide and document whether same-name import means replace, copy, or prompt.
- Either import embedded SVG data images or replace stale “Phase E” copy with an honest limitation.
- Check FileHandle size before reading oversized picker files.
- Reject or report corrupt DXF numeric values rather than coercing geometry to zero.
- Record decisions for Open Recent, .lbrn import, and device-profile mismatch on project open.
- Standardize parse result/error contracts across SVG and DXF.

### Canvas and content creation

- Surface geometry-operation failures consistently.
- Improve text and node editing without creating a parallel geometry model.
- Record shortcut and duplicate-offset differences rather than implying LightBurn identity.
- Preserve layer/material context when selection changes.
- Improve Design Library and Offset discoverability.
- Consolidate duplicated history limits and label history entries accessibly.

### Layers and cut settings

- Reconcile material-preset blocking behavior with ADR-045.
- Keep Layers visible while editing an object.
- Explain or ADR-record per-object operation overrides and sub-layer behavior.
- Provide visible feedback for clamped numeric input.
- Make image overscan a layer setting rather than a fixed 5 mm implementation detail.
- Add a through-cut/no-tabs advisory.

### Preview, Frame, and job planning

- Correct placement-failure diagnosis.
- Add layer-color and optional power shading.
- Distinguish distance-compressed playback from time simulation.
- Persist or deliberately scope Start From / Job Origin settings.
- Show elapsed and remaining time during execution.
- Display Frame verification state and clarify burn versus full motion envelopes.

### Machine control

- Unify connection behavior and make all warnings visible.
- Model M0/tool changes explicitly.
- Decide whether Fire/test pulse, continuous jog, keyboard jog, speed control, and Go to Origin belong in product scope; treat them as parity choices until decided.
- Use driver APIs rather than constructing raw GRBL jog lines in UI state.
- Collapse or tab dense Console/Jog sections at laptop sizes.
- Normalize non-GRBL capability handling across fresh Start and Resume.

### Camera and registration

- Correct letterbox mapping and calibration-resolution binding.
- Add plausibility checks for first-corner capture.
- Make board/jig provenance and unlock/replace semantics explicit.
- Add integrated synthetic camera-pipeline fidelity tests.
- Improve panel placement and capture-flow ergonomics.

### Trace and raster

- Make auto-Sketch promotion visible and reversible.
- Support or explicitly reject rotated raster in the editing UI before job time.
- Upgrade resampling and add realistic-scale perceptual output tests.
- Remove or quarantine dead fallback/Canny-era modules.
- Align PROJECT and WORKFLOW with the actual Potrace/centerline architecture.
- Decide whether current line-density ceilings are product policy or temporary budgets.

### Architecture and performance

- Replace broad store subscriptions and unstable object selectors with stable field selectors.
- Memoize 3D compile/removal work by output-affecting state.
- Replace streamer queue slice(1) with a cursor/deque.
- Cache decoded raster luma and bound preview caches.
- Move heavy shared work to cancellable workers where appropriate.
- Introduce one shared geometry Result contract or formally permit typed domain exceptions.
- Enforce or remove fictional soft line/export limits.
- Generate the ADR index and dead-module report.

### Tests and CI

- Add browser-level golden workflows.
- Add packaged Electron smoke coverage.
- Parameterize controller invariants over Marlin and Smoothieware.
- Add raster, camera, and relief end-to-end fidelity instruments.
- Add a no-partial-write Save regression.
- Decide whether snapshot acknowledgement and source-without-tests checks are automated policies or review conventions.
- Split CI signal where useful without weakening release:check parity.
- Record whether coverage thresholds and dependency-audit placement are deliberate.

## 11. Prioritized implementation roadmap

### Phase 0 — Machine and recovery integrity

1. M-01 tool-change barrier.
2. M-03 CNC resume plunge reconstruction.
3. M-02 unified profile-aware Connect.
4. M-04 tiled export readiness/provenance.
5. M-05 stable-Idle checkpoint retention.
6. M-06 modal/global-shortcut suppression.
7. M-07 camera frame/calibration compatibility.
8. M-10 CNC work-zero advisory.
9. M-17 safe CNC initial operation/tabs policy.
10. M-09 shared complexity/output budgets.

Each item should land as a narrow ticket with its own regression tests and release gate.

### Phase 1 — Operator trust and workflow clarity

1. Mount or replace the operator log channel.
2. Expose Frame validity and full-motion context.
3. Guard PWA reload across all incomplete/recovery states.
4. Add through-cut/no-tabs guidance.
5. Keep Layers and Material Library context visible.
6. Add universal Import, visible STL import, and a named Design Library entry.
7. Make job actions sticky and machine panels collapsible/resizable.

### Phase 2 — Performance and fidelity

1. Profile representative connected-preview, CNC 3D, and photo-raster projects.
2. Fix selector/recompute hot paths and the O(N²) queue.
3. Cache luma and bound raster/preview caches.
4. Upgrade raster resampling and realistic perceptual tests.
5. Add camera and relief synthetic fidelity instruments.
6. Move remaining large synchronous compilation stages behind workers/progress/cancellation.

### Phase 3 — Product-parity and governance decisions

1. Decide LightBurn/Easel parity policy feature by feature.
2. Reconcile WORKFLOW, PROJECT, help, and shortcuts with the shipped UI.
3. Generate the ADR index and mark superseded decisions clearly.
4. Formalize Result/error conventions and enforce only limits the team intends to maintain.
5. Add browser, packaged Electron, and controller-dialect CI coverage.
6. Resolve Open Recent, .lbrn, palette, Fire, advanced jog, and text/node scope explicitly.
7. Harden native-arc extrema scanning before production compilers begin emitting arc passes.

## 12. Validation record

### Completed by Claude

- Static source inspection across 14 sectors.
- 51 independent adversarial reviews of critical/major claims.
- Claimed totals in the source report: 47 confirmed, 4 partial, 0 refuted.
- Lint and typecheck reported clean.
- No live application, full test suite, hardware, or perceptual verification.

### Completed by the independent audit

- Canonical checkout, branch, commit, remote, and worktree identity verified.
- Three specialist source audits plus lead reconciliation.
- Live browser inspection at 1024×768, 1280×720, and 1366×768.
- Reversible representative drawing, Preview, menu, mode-switch, and responsive-layout workflows.
- Visible controls inspected for accessible naming/help metadata.
- No browser console errors in the exercised flow.
- Full release gate completed:
  - 651 passing test files, 12 skipped.
  - 4,107 passing tests, 17 skipped.
  - Typecheck, ESLint, Electron lint, Prettier, license check, dependency audit, web build, Electron-main build, and file-size policy all passed.
- Non-failing React act(...) and jsdom canvas/WebGL warning noise remains in several tests.

### Not verified

- Physical laser or CNC execution.
- Camera calibration against real stock.
- Real multi-bit tool changes or recovery after power loss.
- Perceptual comparison with LightBurn output.
- Packaged Electron installation/runtime.
- Production-scale performance profiling.
- The later origin/main rotary/emitter commit 40bd8194.

## 13. Reusable full-sweep audit prompt

~~~text
Perform a read-only, evidence-first, full-spectrum audit of the exact active
checkout. Resolve and record repository path, branch, commit, remote, and
working-tree state before analysis.

Run independent specialist tracks for:

1. Architecture, module boundaries, state ownership, and data flow
2. End-to-end workflows and feature placement
3. UI hierarchy, button layout, responsiveness, accessibility, and discoverability
4. CAM, emitted output, controller transport, and machine-safety behavior
5. Performance, heavy-job limits, persistence, recovery, and offline behavior
6. Testing, build gates, desktop security, documentation, and ADR drift
7. Reference-product parity, clearly separated from confirmed defects

Inspect the live application at representative laptop and desktop resolutions.
Exercise only reversible workflows. Trace high-risk actions from the UI through
state/actions, core logic, emitted output, transport, and recovery.

For every finding provide:

- Unique root-cause ID
- Severity and confidence
- Evidence type: source, test, live, hardware, reference, or documentation
- Exact file and line evidence
- User, data, or machine impact
- Concrete correction
- Required regression test

Deduplicate repeated cross-sector symptoms. Separate confirmed defects,
defense-in-depth gaps, documentation drift, product-scope choices, and
reference-product comparisons. Independently challenge every high-severity
claim and propagate verifier corrections into headings, totals, and priorities.

Finish with a direct code-quality verdict, unified scorecard, feature-flow map,
priority roadmap, strengths to preserve, validation record, and limitations.
Do not modify product files unless implementation is explicitly requested.
~~~

---

# Appendix A — Claude's original 14-sector audit

> Preserved from the untracked Claude worktree at the same audited commit. The original prose, findings, strengths, verifier notes, and completeness critique follow. Line endings were normalized. Several original headings/totals are superseded by Sections 5, 6, and 9 above.
