# Track GP — GRBL 1.1 protocol core (partial, in progress)
...
- upstream evidence: protocol.c:219-238 (`if ((rt_exec == EXEC_ALARM_HARD_LIMIT) || (rt_exec == EXEC_ALARM_SOFT_LIMIT)) { report_feedback_message(MESSAGE_CRITICAL_EVENT); ... do { } while (bit_isfalse(sys_rt_exec_state,EXEC_RESET)); }`)
  https://github.com/gnea/grbl/blob/bfb67f0c7963fe3ce4aaf8a97f9009ea5a8db36e/grbl/protocol.c#L219-L238 ; wiki Interface:
...
- upstream evidence: serial.c:148-155 (`case CMD_FEED_HOLD: system_set_exec_state_flag(EXEC_FEED_HOLD)`), config.h:51-54, protocol.c:252-290 (Idle → Hold, SUSPEND_HOLD_COMPLETE), protocol.c:546 `while (sys.suspend)`.
...
- kerfdesk evidence: `src/ui/state/grbl-settings-actions.ts:381-392` (only numeric/sign/$32 checks), `:297-302` (exact re-read equality);
...
- kerfdesk evidence: `src/ui/state/laser-home-action.ts:43-51,147-155` (comment + 'non-idle-status-activity' 120 s).
...
## Still to check
- WCO-less window right after connect without a board reset (zero-WCO default) — so far judged guarded by the Frame permit's wcoCache comparison.
- Report-units window after reset/connect before `$$` re-reads `$13` (positions read as mm) — judged narrow; confirm no jog-to-point path.
- Error/alarm text nits (alarm 4 wording omits G38.4/.5; error 11 "80 characters" is 79 after whitespace/comment stripping; error 14 never emitted by 1.1h/328p).
- Final "checked and correct" list.
