...
1. **T1-104** — `canFrame` exact-idle gate cluster (Frame, Frame Dot, Test Fire, Jog, Set Origin). ChatGPT items #2, #3, #6, #10. Currently `canFrame = isConnected && !isRunning`; should also require `machineState.status === 'idle'` (or simulator). Five surfaces touched, one umbrella ticket.
2. **T1-105** — Set Origin `await`s G10 success before saving to localStorage and updating UI. ChatGPT item #4. Burn-placement correctness; UI currently saves origin before machine confirms.
3. **Controller `safetyOff()` propagation to MachineService laser state.** ChatGPT item #11. Largest of the remaining items; high priority. May need scope-decision before ticket assignment.
