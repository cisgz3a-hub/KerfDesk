...
1. **T1-99 (NEW)** — Verify and fix savedOrigin → gcodeStale interaction. `useCompileManager` lists `savedOriginX/Y` as compile-invalidating deps, but `computeGcodeOffset` for savedOrigin mode emits `G10 L20 P1 X0 Y0` (zeroes WCS regardless). Suspected over-invalidation: jog → Set Origin → frame → Start may fail on the gcodeStale gate. Verify with code read on `computeGcodeOffset`; fix by removing savedOriginX/Y from compile invalidation OR by triggering a clean recompile on Set Origin. Estimate: 30 min including tests.
2. **T1-100 (NEW)** — `App.tsx machinePlanBounds={activeJobTransform?.plan.bounds ?? null}` is null pre-job-start, falling back to fragile g-code text scanning that mis-handles G91 relative moves in current/head startMode. Replace with `!gcodeStale && currentGcode && lastResult ? lastResult.machinePlanBounds : null`. Estimate: 30 min including tests.
3. **T1-101 (NEW)** — `frameSafe` defaults `withCrosshair: true`; in current/head startMode, the crosshair leaves the head at the design centroid, not the original jog point. Subsequent Start in current/head mode therefore burns from the wrong physical reference. Two options: default `withCrosshair: false` for `frameSafe` (reserve crosshair for Frame Dot / Mark Center buttons), OR have `buildFrameGcode` return to original frame origin after crosshair. **Safety-critical — hardware verification required.** Estimate: 1 hour including tests.
4. **T1-97 retire** — once T1-98 verified, revert `24e5dd46` + `96616f77`, OR demote to productionMode-only.
5. **T1-17 pass 4b** — JobCompiler consumes pre-processed `adjustedData` when present.
