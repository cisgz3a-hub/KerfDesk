...
| T1-43 | Reassert G90/G91 after customStart + block mode flips in templates | **Validator:** widen `TEMPLATE_G91_IN_HEADER` to `header` + `customStart`; add `TEMPLATE_G90_IN_HEADER` (kept legacy G91 code for test pin). **Output.encodeHeader:** reassert matching `G90`/`G91` with T1-43 marker after non-empty customStart. Pins: `tests/customstart-mode-reassertion.test.ts`, extended `tests/gcode-template-validator.test.ts`. Ship hash **`TBD`**. |
