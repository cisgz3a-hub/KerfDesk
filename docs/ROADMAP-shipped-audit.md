...
| T1-78 | Split `requireFeature` into `canUseFeature` + `assertFeature` | **Phase 1 shipped 2026-05-04 in `bad81ab`** — `src/entitlements/index.ts` now exports `canUseFeature(f)` (boolean), `assertFeature(f)` (throws `EntitlementError`), and the `EntitlementError` class itself. The previous `requireFeature` is preserved as a `@deprecated` alias delegating to the same `entitlementService.canUse` so the 8 existing call sites keep working unchanged. Pinned by `tests/entitlement-api-split.test.ts` (13/13: boolean delegation, throw/no-throw paths, error shape, deprecated alias still working). New test wired into `scripts/run-tests.mjs`. Pure additive surface — TS baseline unchanged at 43, npm test full suite green. | **Phase 2 open:** migrate the 8 caller files (`src/core/nesting/Nester.ts:65,275`, `src/geometry/BooleanOps.ts:193`, `src/core/job/JobCompiler.ts:76-81` × 6, `src/ui/hooks/useSceneOperations.ts:265`, `src/ui/hooks/useGeneratorHandlers.ts:123`, `src/ui/hooks/useMaterialTestHandlers.ts:28`, `src/ui/hooks/useKerfHandlers.ts:25,35,74`) from `requireFeature` to the new API. JobCompiler's six call sites are flag-builders → `canUseFeature`; the throw-on-not-entitled patterns in Nester/BooleanOps/hooks → `assertFeature`. Keep callers as separate ticket(s) so each one's intent is reviewed individually. |
