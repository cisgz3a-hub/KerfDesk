...
5. Keep all unrelated Labs features disabled. Rotary cases use the persisted Rotary Setup directly; this protocol qualifies physical results and does not unlock an output permission.
...
## Labs and physical qualification decisions

A remaining hardware-gated feature leaves Labs only in a dedicated PR containing:
...
Low-power Fire, print-and-cut, and camera alignment v2 remain independent Labs decisions. Rotary setup and G-code output are not Labs-gated (ADR-315): ROT evidence qualifies physical claims but never unlocks, changes, or suppresses generated bytes. A pass for one workflow does not establish another workflow's physical behavior.
