...
- Last shipped roadmap item: **T1-257** (installer workflows upload SHA256SUMS, shipped in `ed56b5a4`).
- Current audit-fix run completed: **T1-223 through T1-257**, with T1-237 still deferred as multi-week firmware-adapter wiring.
...
- `npx tsc --noEmit --pretty false` passed during the T1-257 close-out.
- Focused tests for T1-223 through T1-252 passed at their commits.
- Full `npm test` passed during T1-257.
- `npm run build`, `npx eslint . --max-warnings 0`, `npm run project-map:check`, `node scripts/exported-symbol-inventory.mjs --check`, and `git diff --check` passed during the T1-257 close-out.
