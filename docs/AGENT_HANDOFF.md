...
- Last shipped roadmap item: **T1-17 trace atomic-commit follow-up** (trace completion uses one scene transaction for scene + selection, shipped in `93b35b10`).
...
3. **Next in-repo candidate if hardware is unavailable** - continue with the lowest-numbered open non-hardware roadmap item.
