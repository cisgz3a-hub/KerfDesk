...
- Last shipped roadmap item: **T1-235** (core time/random per-site review, shipped in 49ed6bb2).
- Current audit-fix run completed: **T1-223 through T1-235**.
- Next audit-fix ticket: **T1-236** - route inline ID generators through `generateId()` where appropriate.
...
- `npx tsc --noEmit --pretty false` passed during the T1-223..T1-235 run.
- Focused tests for T1-223 through T1-235 passed at their commits.
...
1. **T1-236** - F-013: route inline ID generators through `generateId()` where appropriate.
2. **T1-237** - firmware adapter wiring remains deferred/multi-week.
3. **T1-238** - F-016: no-skip exported-symbol audit.
4. **T1-239** - F-017: hook dependency triage.
5. **T1-240** - F-018: regenerate/check `PROJECT_MAP.md`.
6. **T1-241** - F-019: diagnose/fix the full-suite test runner hang.
...
T1-235 closed the F-008 review ticket by adding `tests/core-time-random-audit.test.ts`. The test proves compile/output paths stay free of wall-clock/random input, confirms the critical deterministic-ID guards, and leaves the remaining inline random ID generators explicitly queued for T1-236/F-013.
