...
- Last shipped roadmap item: **T2-6 Phase 3aw** (startup autosave recovery prompt planning extracted from App.tsx, shipped in `f71604e2`).
- Latest local commit at last handoff refresh: **`a55c509d`** (`fix(ci): restore spool progress verification`), after the `bacf6ba1` external audit remediation batch and `a4d4f26d` hash-fill.
...
| T2-6 Phase 3al | App split | Exit flow decisions moved into `src/ui/components/app/appExitHelpers.ts`; App still owns prompts, safe-disconnect side effects, and navigation. |
| T2-6 Phase 3am | App split | Start-mode status label formatting moved into `src/ui/components/app/appStartModeSelectionHelpers.ts`; App still owns rendering and saved-origin state. |
| T2-6 Phase 3an | App split | Window-to-canvas resize sizing moved into `src/ui/components/app/appLayoutHelpers.ts`; App still owns the resize listener and viewport-store write. |
| T2-6 Phase 3ao | App split | Selected-text quick-action predicate moved into `src/ui/components/app/appSelectionHelpers.ts`; App still owns quick-action rendering. |
| T2-6 Phase 3ap | App split | Connection-panel bounds fallback props moved into `src/ui/components/appConnectionPanelProps.ts`; App still owns source bounds and panel rendering. |
| T2-6 Phase 3aq | App split | Connection-panel machine-plan bounds selection moved into `src/ui/components/appConnectionPanelProps.ts`; App still owns active-job and compile state. |
| T2-6 Phase 3ar | App split | Text-dialog open/edit decisions moved into `src/ui/components/app/appTextDialogOpenHelpers.ts`; App still owns dialog-store side effects. |
...
| T2-6 Phase 3aw | App split | Startup autosave recovery prompt eligibility and timestamp-label planning moved into `src/ui/components/app/appRecoveryHelpers.ts`; App still owns the autosave read and dialog-store writes. |
...
- `npx tsx tests/app-exit-helpers.test.ts`, `npx tsx tests/autosave-manual-save-separation.test.ts`, and `npx tsc --noEmit --pretty false` passed during the T2-6 Phase 3al close-out.
- `npx tsx tests/app-start-mode-selection-helpers.test.ts` and `npx tsc --noEmit --pretty false` passed during the T2-6 Phase 3am close-out.
- `npx tsx tests/app-layout-helpers.test.ts` and `npx tsc --noEmit --pretty false` passed during the T2-6 Phase 3an close-out.
- `npx tsx tests/app-selection-helpers.test.ts` and `npx tsc --noEmit --pretty false` passed during the T2-6 Phase 3ao close-out.
- `npx tsx tests/app-connection-panel-props-extracted.test.ts` and `npx tsc --noEmit --pretty false` passed during the T2-6 Phase 3ap close-out.
- `npx tsx tests/app-connection-panel-props-extracted.test.ts` and `npx tsc --noEmit --pretty false` passed during the T2-6 Phase 3aq close-out.
- `npx tsx tests/app-text-dialog-open-helpers.test.ts`, `npx tsx tests/text-operation-dialog-wiring.test.ts`, `npx tsx tests/app-text-commit-helpers.test.ts`, `npx tsx tests/app-size-audit.test.ts`, `npx tsc --noEmit --pretty false`, `npm run project-map:check`, `node scripts/exported-symbol-inventory.mjs --check`, and `git diff --check` passed during the T2-6 Phase 3ar close-out.
...
- `npx tsx tests/app-recovery-helpers.test.ts`, `npx tsx tests/recovery-eligibility-non-empty-cases.test.ts`, `npx tsc --noEmit --pretty false`, and `git diff --check` passed during the T2-6 Phase 3aw close-out.
