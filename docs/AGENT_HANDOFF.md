...
- Last shipped roadmap item: **T1-251** (service-level FrameTicket Start gate, shipped in `66e559f0`).
- Current audit-fix run completed: **T1-223 through T1-251**, with T1-237 still deferred as multi-week firmware-adapter wiring.
- Next active audit-fix ticket: continue the release-readiness audit sequence with the next runtime-enforcement cap after FrameTicket enforcement: structured pause/stop laser-off confirmation, support-bundle export, or signed entitlement authority.
...
- `npx tsc --noEmit --pretty false` passed during the T1-251 close-out.
- Focused tests for T1-223 through T1-251 passed at their commits.
- Full `npm test` passed during T1-251 after updating stale source-level pins and generated audit docs.
- `npm run build`, `npx eslint . --max-warnings 0`, `npm run project-map:check`, `node scripts/exported-symbol-inventory.mjs --check`, and `git diff --check` passed during the T1-251 close-out.
...
1. **Pause/stop/alarm laser-off confirmation** - no fire-and-forget laser-off safety paths may leave the app ready.
2. **Support bundle export** - wire the existing bundle assembler to a user-exportable ZIP.
3. **Signed entitlement authority** - replace local/cache-authoritative licensing with signed entitlement tokens.
