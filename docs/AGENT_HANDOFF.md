...
- Last shipped roadmap item: **T1-259** (signed release artifact attestations, shipped in `85ecae9c`).
- Current audit-fix run completed: **T1-223 through T1-259**, with T1-237 still deferred as multi-week firmware-adapter wiring.
...
- `npx tsc --noEmit --pretty false` passed during the T1-259 close-out.
- Focused tests for T1-223 through T1-252 passed at their commits.
- Full `npm test` passed during T1-259.
- `npm run build`, `npx eslint . --max-warnings 0`, `npm run project-map:check`, `node scripts/exported-symbol-inventory.mjs --check`, and `git diff --check` passed during the T1-259 close-out.
