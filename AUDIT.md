...
| ADR-116 — machine camera via the loopback bridge: /frame.jpg pixel proxy, server-side /discover, FrameSource union, restored+consolidated Camera panel (F-CAM6) | **CLAIMED** | bridge routes tested against a stub upstream (CORS, origin-gate-before-work, JPEG-magic, PNA); store source lifecycle + RTSP probe tests; rendered-frame detection over the capture-closure path. The machine was powered off for the entire build — zero frames from the real camera yet |
