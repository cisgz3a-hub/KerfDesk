...
| Interrupted-job checkpoint + resume (ADR-118, F-B16) | **CLAIMED** | 2026-07-07: core fingerprint/mapping property-tested against createStreamer (sendable-vs-raw numbering); storage round-trip + corrupt-discard; hook interval/transition/freeze/clear-on-done tests; flow tests pin checkpoint-on-start, fingerprint-mismatch refusal, resumeInFlight stamping; banner render/dismiss jsdom tests. NOT verified: a real kill-the-app-mid-burn crash + resume on the Falcon (maintainer) — and the underlying start-from-line replay itself is still hardware-CLAIMED (H.11/G7 row) |
...
| K.2/V1 panel cutouts (ADR-116) | **CLAIMED** | 2026-07-07: multi-ring fit invariants (slot widening c/2, hole-corner reliefs at full radius, severed-not-dropped); panels re-carriered to named imported-svg objects (holes real under even-odd fill; closes the v1 name gap). Benchmark 'cutouts' 100% |
| K.2/V2 divider grids (ADR-116) | **CLAIMED** | 2026-07-07: divider referee proves exact slot/tab/cross-lap complementarity (60-seed fuzz) + play contract (length play == c, thickness play == c/2), 4 negative controls; 0-divider output byte-identical. Benchmark 'dividers' 100%. **Pending hardware check: cut a 120×90×40 T=3 organizer with 2×1 dividers and assemble** |
| K.2/V3 slide-lid style (ADR-116) | **CLAIMED** | 2026-07-07: slide-lid referee (nominal exact + sliding contract on fitted panels: channel play c/2, lid clears front) + 4 negative controls; mandatory clearance validated; composes with dividers; sheet render eyeballed. Benchmark 'slide-lid' 100%. **Pending hardware check: cut an 80×50×30 T=3 slide-lid box and SLIDE the lid** |
| ADR-119 fit coupon + assembled preview | **CLAIMED** | 2026-07-07: per-rung law exact (notch − tab == cᵢ, laser + CNC-relieved); asymmetric-margin rung identification (index marks removed — any small notch/bump slivers under relief, caught in build); assembled-layout frames unit-pinned for every part kind; benchmark 'fit-coupon' 100% (1114/1114 overall). **Pending hardware check: cut a fit coupon, press the rungs, confirm the winning rung's clearance produces a good box** |
...
| ADR-121 — machine camera via the loopback bridge: /frame.jpg pixel proxy, server-side /discover, FrameSource union, restored+consolidated Camera panel (F-CAM6) | **PARTIALLY VERIFIED (2026-07-07 live)** | Verified against the real machine camera (192.168.10.1:8080, 1440×1080 fisheye) in the real browser: /discover finds it, the panel activates it, and Diagnostics "Test capture" reads pixels through the proxy ("Captured 1440×1080 — pixels readable"). Hardware pass also found+fixed: loopback-origin CORS gap and camera wedging under concurrent/aborted requests (bridge now serializes per host, probe timeout 5s). Still UNVERIFIED: lens calibration against the real lens, marker burn + auto-align on the bed, overlay registration accuracy |
| ADR-122 — click-to-position (F-CAM7), snapshot+monitoring (F-CAM8), burn-target alignment wizard (F-CAM9) | **CLAIMED** | click property test (any click, any origin → inside bed) + zero-axis jog fix pinned across GRBL/Smoothie/Marlin; wizard store/burn/detect unit tests; snapshot encode/save tests. No head moved, no marker burned, no frame captured on real hardware |
