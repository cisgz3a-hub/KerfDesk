# LaserForge Project Map

> AUTO-GENERATED by `npm run project-map:generate`.
> Do not edit this file by hand. Run `npm run project-map:check` to verify it is current.

## Summary

| Area | Files | Source of truth |
|---|---:|---|
| `src/` | 512 | Production code, UI, controller, app, geometry, import, storage, and worker modules |
| `tests/` | 820 | Executable regression, integration, source-pin, simulator, and E2E tests |

## Pipeline Overview

```text
Scene -> Job -> Plan -> Output -> Device
```

The source tree is the authority for detailed ownership. This generated map is a stable index for orientation and drift detection.

## Source Module Boundaries

| Boundary | Files | Direct child directories |
|---|---:|---|
| `src/` | 2 | - |
| `src/app` | 49 | - |
| `src/communication` | 5 | - |
| `src/controllers` | 29 | grbl |
| `src/core` | 116 | box, devices, geometry, grbl, image, job, logging, materials, nesting, output, plan, preflight, replay, scene, storage, tools |
| `src/debug` | 4 | - |
| `src/diagnostics` | 13 | - |
| `src/entitlements` | 12 | - |
| `src/fonts` | 6 | data |
| `src/geometry` | 10 | - |
| `src/import` | 19 | dxf, image, svg, trace |
| `src/integrity` | 1 | - |
| `src/io` | 14 | migrations, validation |
| `src/machine-control-v2` | 12 | grbl |
| `src/onboarding` | 1 | - |
| `src/runtime` | 2 | - |
| `src/security` | 3 | - |
| `src/storage` | 2 | - |
| `src/templates` | 1 | - |
| `src/transports` | 1 | - |
| `src/types` | 3 | - |
| `src/ui` | 205 | browser, components, features, history, hooks, pages, recovery, renderers, safety, scene, stores, styles, tools, utils |
| `src/workers` | 2 | - |

## Test Boundaries

| Boundary | Files | Direct child directories |
|---|---:|---|
| `tests/` | 743 | - |
| `tests/capability-regression` | 1 | - |
| `tests/connection-lifecycle` | 1 | - |
| `tests/controller-matrix` | 1 | - |
| `tests/e2e` | 41 | fixtures, helpers, snapshots |
| `tests/end-to-end-workflows` | 1 | - |
| `tests/entitlement-tamper-resistance` | 1 | - |
| `tests/fixtures` | 3 | projects |
| `tests/harness` | 2 | - |
| `tests/helpers` | 9 | - |
| `tests/ipc-fuzz` | 1 | - |
| `tests/malformed` | 5 | - |
| `tests/packaged-app-smoke` | 1 | - |
| `tests/perf` | 3 | - |
| `tests/persistence-stress` | 1 | - |
| `tests/safety-controller-matrix` | 1 | - |
| `tests/simulators` | 4 | - |
| `tests/undo-redo-correctness` | 1 | - |

## Source Files

### `src`

- `src/main.tsx`
- `src/vite-env.d.ts`

### `src/app`

- `src/app/ActiveJobCanvasContext.ts`
- `src/app/AutosaveBackupSlot.ts`
- `src/app/BurnState.ts`
- `src/app/CompiledJobState.ts`
- `src/app/ConnectionFailureGuidance.ts`
- `src/app/ConnectionManager.ts`
- `src/app/CriticalEventStore.ts`
- `src/app/CurrentFrameAnchor.ts`
- `src/app/DisconnectTransaction.ts`
- `src/app/DomainErrorState.ts`
- `src/app/ErrorReporter.ts`
- `src/app/ExecutionCoordinator.ts`
- `src/app/FrameResultMessages.ts`
- `src/app/FrameState.ts`
- `src/app/JobComplexitySummary.ts`
- `src/app/JobExecutionSession.ts`
- `src/app/JobLogCheckpoint.ts`
- `src/app/JobLogEventWindow.ts`
- `src/app/JobSession.ts`
- `src/app/LastMachinePosition.ts`
- `src/app/MachineCommandGateway.ts`
- `src/app/MachineEventLedger.ts`
- `src/app/MachineSafetyState.ts`
- `src/app/MachineService.ts`
- `src/app/OperationGate.ts`
- `src/app/OperationOrder.ts`
- `src/app/PipelineService.ts`
- `src/app/PositionTrust.ts`
- `src/app/ProjectRuntimeReset.ts`
- `src/app/RecoveryBypassToken.ts`
- `src/app/SafetyActionResult.ts`
- `src/app/SafetyStateMachine.ts`
- `src/app/StructuredMessageLog.ts`
- `src/app/StructuredRxTxEntry.ts`
- `src/app/UserModeGates.ts`
- `src/app/approvalNonceStore.ts`
- `src/app/autosavePersistence.ts`
- `src/app/computeCommandGates.ts`
- `src/app/computeFrameFreshnessKey.ts`
- `src/app/frameGcode.ts`
- `src/app/grblIdlePoll.ts`
- `src/app/machineServiceHelpers.ts`
- `src/app/recoveryEligibility.ts`
- `src/app/savedOriginVerify.ts`
- `src/app/sendResetWcsCommand.ts`
- `src/app/sendSetOriginWcsCommand.ts`
- `src/app/unsafeAtConnectMessages.ts`
- `src/app/unsafePriorState.ts`
- `src/app/validateJobTicket.ts`

### `src/communication`

- `src/communication/ConnectionGenerationGuard.ts`
- `src/communication/SerialPort.ts`
- `src/communication/SerialPortFactory.ts`
- `src/communication/TransportSubscription.ts`
- `src/communication/WebSerialPort.ts`

### `src/controllers`

- `src/controllers/CapabilityValue.ts`
- `src/controllers/ControllerCapabilities.ts`
- `src/controllers/ControllerInterface.ts`
- `src/controllers/ControllerRegistry.ts`
- `src/controllers/ControllerSafetyCapabilities.ts`
- `src/controllers/ControllerSafetyOps.ts`
- `src/controllers/ControllerStatus.ts`
- `src/controllers/FirmwareAdapter.ts`
- `src/controllers/GrblFirmwareAdapter.ts`
- `src/controllers/IdentityComparison.ts`
- `src/controllers/JobProgressMultiDomain.ts`
- `src/controllers/MarlinFirmwareAdapter.ts`
- `src/controllers/SafetyActionResult.ts`
- `src/controllers/grbl/CommandClassifier.ts`
- `src/controllers/grbl/ForceSafeState.ts`
- `src/controllers/grbl/GrblController.ts`
- `src/controllers/grbl/GrblFrameGcode.ts`
- `src/controllers/grbl/GrblIdentityParser.ts`
- `src/controllers/grbl/GrblJobBoundsChecker.ts`
- `src/controllers/grbl/GrblJobLineParser.ts`
- `src/controllers/grbl/GrblSafeStateClassifier.ts`
- `src/controllers/grbl/GrblSettingsParser.ts`
- `src/controllers/grbl/GrblStatusReportParser.ts`
- `src/controllers/grbl/GrblUnsafeAtConnect.ts`
- `src/controllers/grbl/GrblWcsConsentClassifier.ts`
- `src/controllers/grbl/GrblWcsParser.ts`
- `src/controllers/grbl/StopOnErrorOverrideToken.ts`
- `src/controllers/grbl/grblOperationValidators.ts`
- `src/controllers/grbl/streamingHealth.ts`

### `src/core`

- `src/core/box/boxGeometry.ts`
- `src/core/box/boxGeometryV2.ts`
- `src/core/box/boxJoineryEngine.ts`
- `src/core/box/boxLibrary.ts`
- `src/core/box/boxLibraryTypes.ts`
- `src/core/box/boxPreviewModel.ts`
- `src/core/box/joineryTypes.ts`
- `src/core/box/jointPattern.ts`
- `src/core/box/kerfPresets.ts`
- `src/core/box/panelModel.ts`
- `src/core/devices/ActiveProfileStore.ts`
- `src/core/devices/DeviceProfile.ts`
- `src/core/devices/SplitDeviceProfile.ts`
- `src/core/devices/homeCorner.ts`
- `src/core/devices/profileSnapshot.ts`
- `src/core/devices/validateProfile.ts`
- `src/core/geometry/CompoundPath.ts`
- `src/core/grbl/grblClassifierLines.ts`
- `src/core/image/ImageProcessing.ts`
- `src/core/job/CompoundPathOutput.ts`
- `src/core/job/FamilyAgnosticTicket.ts`
- `src/core/job/Job.ts`
- `src/core/job/JobCompiler.ts`
- `src/core/job/JobFingerprint.ts`
- `src/core/job/JobLog.ts`
- `src/core/job/OperationOrderer.ts`
- `src/core/job/ReconstructionGradeJobLog.ts`
- `src/core/job/ScanningOffset.ts`
- `src/core/job/SmartOverscan.ts`
- `src/core/job/TicketCapabilitySnapshot.ts`
- `src/core/job/ValidatedJobTicket.ts`
- `src/core/job/bezierFlatten.ts`
- `src/core/job/compileInputHashes.ts`
- `src/core/job/compoundRoles.ts`
- `src/core/job/index.ts`
- `src/core/job/jobCompilerHelpers.ts`
- `src/core/job/ticketHashing.ts`
- `src/core/logging/StructuredDiagnosticLog.ts`
- `src/core/materials/CalibrationAnalyzer.ts`
- `src/core/materials/CalibrationGrid.ts`
- `src/core/materials/CommunityResponseCurve.ts`
- `src/core/materials/MaterialFeedback.ts`
- `src/core/materials/MaterialLibrary.ts`
- `src/core/materials/MaterialPreset.ts`
- `src/core/materials/MaterialPresetSnapshot.ts`
- `src/core/materials/MaterialPresets.ts`
- `src/core/materials/MaterialSettingConfidence.ts`
- `src/core/materials/ResponseCurve.ts`
- `src/core/materials/defaultPresets.ts`
- `src/core/materials/presetOpsHelpers.ts`
- `src/core/nesting/Nester.ts`
- `src/core/output/GcodeOrigin.ts`
- `src/core/output/GcodeStreaming.ts`
- `src/core/output/GrblStrategy.ts`
- `src/core/output/Output.ts`
- `src/core/output/OutputFormat.ts`
- `src/core/output/TimeEstimator.ts`
- `src/core/output/burnEnvelopeDivergence.ts`
- `src/core/output/emittedBurnEnvelope.ts`
- `src/core/output/index.ts`
- `src/core/plan/ContainmentOrder.ts`
- `src/core/plan/FillGenerator.ts`
- `src/core/plan/GcodeTemplateValidator.ts`
- `src/core/plan/GcodeTemplates.ts`
- `src/core/plan/MachineBounds.ts`
- `src/core/plan/MachineTransform.ts`
- `src/core/plan/OperationOrderer.ts`
- `src/core/plan/PathOptimizer.ts`
- `src/core/plan/Plan.ts`
- `src/core/plan/PlanOptimizer.ts`
- `src/core/plan/RasterGenerator.ts`
- `src/core/plan/ScanningOffset.ts`
- `src/core/plan/Simulation.ts`
- `src/core/plan/SmartOverscan.ts`
- `src/core/plan/VelocityProfile.ts`
- `src/core/plan/fillGeometryHelpers.ts`
- `src/core/plan/index.ts`
- `src/core/plan/planOptimizerHelpers.ts`
- `src/core/preflight/GcodeTemplateValidator.ts`
- `src/core/preflight/Preflight.ts`
- `src/core/preflight/PreflightContext.ts`
- `src/core/preflight/confirmPreflightForJobStart.ts`
- `src/core/preflight/rules/CapabilityMismatchRules.ts`
- `src/core/preflight/rules/CompileComplexityPreflight.ts`
- `src/core/preflight/rules/DuplicateGeometryPreflight.ts`
- `src/core/preflight/rules/GeometryValidityPreflight.ts`
- `src/core/preflight/rules/LayerSettingsPreflight.ts`
- `src/core/preflight/rules/MachinePreflight.ts`
- `src/core/preflight/rules/MachineStatePreflight.ts`
- `src/core/preflight/rules/OptimizationPreflight.ts`
- `src/core/preflight/rules/OutputBoundsPreflight.ts`
- `src/core/preflight/rules/OutputValidator.ts`
- `src/core/preflight/rules/RasterPreflight.ts`
- `src/core/preflight/rules/ScenePreflight.ts`
- `src/core/preflight/rules/SelfIntersectionPreflight.ts`
- `src/core/preflight/rules/TemplatePreflight.ts`
- `src/core/preflight/rules/sharedHelpers.ts`
- `src/core/replay/JobReplay.ts`
- `src/core/scene/Layer.ts`
- `src/core/scene/Scene.ts`
- `src/core/scene/SceneObject.ts`
- `src/core/scene/SceneOps.ts`
- `src/core/scene/bounds.ts`
- `src/core/scene/index.ts`
- `src/core/scene/layerModeTransition.ts`
- `src/core/scene/sceneDirtyHash.ts`
- `src/core/storage/FilesystemStorageAdapter.ts`
- `src/core/storage/InMemoryStorageAdapter.ts`
- `src/core/storage/IndexedDbStorageAdapter.ts`
- `src/core/storage/StorageAdapter.ts`
- `src/core/storage/StorageNamespaces.ts`
- `src/core/storage/bootstrap.ts`
- `src/core/storage/index.ts`
- `src/core/storage/storage.ts`
- `src/core/tools/TestGridGenerator.ts`
- `src/core/types.ts`

### `src/debug`

- `src/debug/AppDebugState.ts`
- `src/debug/StateGraph.ts`
- `src/debug/TransitionLog.ts`
- `src/debug/traceStormProbe.ts`

### `src/diagnostics`

- `src/diagnostics/AppErrorBoundary.tsx`
- `src/diagnostics/ControllerSettingsSnapshot.ts`
- `src/diagnostics/CorrelationIds.ts`
- `src/diagnostics/CrashLoopDetector.ts`
- `src/diagnostics/CrashReporter.ts`
- `src/diagnostics/GrblDiagnostics.ts`
- `src/diagnostics/Redaction.ts`
- `src/diagnostics/RetentionPolicy.ts`
- `src/diagnostics/StorageHealth.ts`
- `src/diagnostics/SupportBundle.ts`
- `src/diagnostics/SupportBundleExport.ts`
- `src/diagnostics/TroubleshootingPanelContent.ts`
- `src/diagnostics/installGlobalErrorHandlers.ts`

### `src/entitlements`

- `src/entitlements/ClockTamperDetection.ts`
- `src/entitlements/EntitlementLifecycle.ts`
- `src/entitlements/EntitlementService.ts`
- `src/entitlements/FeatureMatrix.ts`
- `src/entitlements/LicenseStatus.ts`
- `src/entitlements/ServerEntitlementService.ts`
- `src/entitlements/ServerWebCryptoEntitlementSigner.ts`
- `src/entitlements/SignedEntitlementToken.ts`
- `src/entitlements/WebCryptoEntitlementVerifier.ts`
- `src/entitlements/index.ts`
- `src/entitlements/testerKey.ts`
- `src/entitlements/types.ts`

### `src/fonts`

- `src/fonts/data/hersheytext.min.json`
- `src/fonts/fontRegistry.ts`
- `src/fonts/injectFontFaces.ts`
- `src/fonts/loadFont.ts`
- `src/fonts/textToPathHershey.ts`
- `src/fonts/textToPathOpentype.ts`

### `src/geometry`

- `src/geometry/BooleanOps.ts`
- `src/geometry/CompoundPathPolygon.ts`
- `src/geometry/OffsetPath.ts`
- `src/geometry/TextToPath.ts`
- `src/geometry/bounds.ts`
- `src/geometry/expandTextForCompile.ts`
- `src/geometry/hit-test.ts`
- `src/geometry/textCanvasDraw.ts`
- `src/geometry/textLayout.ts`
- `src/geometry/textOutlineFingerprint.ts`

### `src/import`

- `src/import/Dithering.ts`
- `src/import/dxf/DxfParser.ts`
- `src/import/dxf/DxfToScene.ts`
- `src/import/dxf/DxfUnits.ts`
- `src/import/dxf/index.ts`
- `src/import/image/ImageImportLimits.ts`
- `src/import/image/ImageImportStorageStrategy.ts`
- `src/import/svg/PathParser.ts`
- `src/import/svg/SvgComplexityLimits.ts`
- `src/import/svg/SvgParser.ts`
- `src/import/svg/SvgToScene.ts`
- `src/import/svg/SvgUnitChoice.ts`
- `src/import/svg/TransformParser.ts`
- `src/import/svg/index.ts`
- `src/import/svg/svgParserHelpers.ts`
- `src/import/trace/ImageTracerAdapter.ts`
- `src/import/trace/PotraceTracer.ts`
- `src/import/trace/index.ts`
- `src/import/trace/trace.worker.ts`

### `src/integrity`

- `src/integrity/checksumFormat.ts`

### `src/io`

- `src/io/FileIO.ts`
- `src/io/ImageReferenceValidation.ts`
- `src/io/ImageStore.ts`
- `src/io/LargeProjectHandling.ts`
- `src/io/ProjectIntegrity.ts`
- `src/io/SceneParseWorker.ts`
- `src/io/SceneSerializer.ts`
- `src/io/SvgExporter.ts`
- `src/io/SvgImportPlacement.ts`
- `src/io/index.ts`
- `src/io/migrations/MigrationPipeline.ts`
- `src/io/migrations/projectMigrations.ts`
- `src/io/sceneSerializerHelpers.ts`
- `src/io/validation/geometryValidation.ts`

### `src/machine-control-v2`

- `src/machine-control-v2/ControllerCapabilitiesV2.ts`
- `src/machine-control-v2/ControllerPanelAdapter.ts`
- `src/machine-control-v2/ControllerProfile.ts`
- `src/machine-control-v2/MachineCommandLog.ts`
- `src/machine-control-v2/MachineIntent.ts`
- `src/machine-control-v2/MachineOperationService.ts`
- `src/machine-control-v2/MachineReadiness.ts`
- `src/machine-control-v2/MachineStateMachine.ts`
- `src/machine-control-v2/grbl/GrblJog.ts`
- `src/machine-control-v2/grbl/GrblRealtime.ts`
- `src/machine-control-v2/grbl/GrblSettingsModel.ts`
- `src/machine-control-v2/grbl/GrblTransportV2.ts`

### `src/onboarding`

- `src/onboarding/FirstRunGuide.ts`

### `src/runtime`

- `src/runtime/RecoveryState.ts`
- `src/runtime/RuntimeState.ts`

### `src/security`

- `src/security/CspPolicy.ts`
- `src/security/FalconWiFiTrust.ts`
- `src/security/TrustedSender.ts`

### `src/storage`

- `src/storage/StorageSizeLimits.ts`
- `src/storage/UserDataMigration.ts`

### `src/templates`

- `src/templates/TemplateLibrary.ts`

### `src/transports`

- `src/transports/Transport.ts`

### `src/types`

- `src/types/global.d.ts`
- `src/types/vendor-modules.d.ts`
- `src/types/web-serial.d.ts`

### `src/ui`

- `src/ui/browser/BrowserCompatibility.ts`
- `src/ui/components/AddTextDialog.tsx`
- `src/ui/components/App.tsx`
- `src/ui/components/AppDragDropOverlay.tsx`
- `src/ui/components/AppFirstRunGuideBridge.tsx`
- `src/ui/components/AppModal.tsx`
- `src/ui/components/AppRecoverySetup.tsx`
- `src/ui/components/AppSettingsModal.tsx`
- `src/ui/components/AppTextDialogs.tsx`
- `src/ui/components/BoxGenerator.tsx`
- `src/ui/components/BuildStamp.tsx`
- `src/ui/components/CameraDialog.tsx`
- `src/ui/components/CanvasViewport.tsx`
- `src/ui/components/ConnectionControls.tsx`
- `src/ui/components/ConnectionPanel.tsx`
- `src/ui/components/ConnectionPanelMain.tsx`
- `src/ui/components/ConsoleInput.tsx`
- `src/ui/components/ConsolePanel.tsx`
- `src/ui/components/ContextMenu.tsx`
- `src/ui/components/DeviceProfileSelector.tsx`
- `src/ui/components/FileToolbar.tsx`
- `src/ui/components/FirstRunGuide.tsx`
- `src/ui/components/FontCreditsDialog.tsx`
- `src/ui/components/GcodePreview.tsx`
- `src/ui/components/GcodeTemplateEditor.tsx`
- `src/ui/components/GridArrayDialog.tsx`
- `src/ui/components/JobControls.tsx`
- `src/ui/components/JobLogViewer.tsx`
- `src/ui/components/JobOutcomeDialog.tsx`
- `src/ui/components/KerfWizard.tsx`
- `src/ui/components/LaserModeBanner.tsx`
- `src/ui/components/LayerPanel.tsx`
- `src/ui/components/LearnedToast.tsx`
- `src/ui/components/MaterialBar.tsx`
- `src/ui/components/MaterialDialog.tsx`
- `src/ui/components/MaterialLibraryDialog.tsx`
- `src/ui/components/MaterialTestDialog.tsx`
- `src/ui/components/MoveControls.tsx`
- `src/ui/components/NestingDialog.tsx`
- `src/ui/components/NumberInput.tsx`
- `src/ui/components/PropertiesPanel.tsx`
- `src/ui/components/QuickActions.tsx`
- `src/ui/components/ScanningOffsetDialog.tsx`
- `src/ui/components/SettingsModal.tsx`
- `src/ui/components/ShortcutsPanel.tsx`
- `src/ui/components/SimulatorView.tsx`
- `src/ui/components/StatusFooter.tsx`
- `src/ui/components/TemplateBrowser.tsx`
- `src/ui/components/TestGridDialog.tsx`
- `src/ui/components/ToolBar.tsx`
- `src/ui/components/TrialGuard.tsx`
- `src/ui/components/UpdateNotice.tsx`
- `src/ui/components/VariableTextDialog.tsx`
- `src/ui/components/WelcomeWizard.tsx`
- `src/ui/components/app/appActivateLayerHelpers.ts`
- `src/ui/components/app/appAutosaveHelpers.ts`
- `src/ui/components/app/appCameraPositionHelpers.ts`
- `src/ui/components/app/appDeleteSelectionHelpers.ts`
- `src/ui/components/app/appExitHelpers.ts`
- `src/ui/components/app/appHistoryNavigationHelpers.ts`
- `src/ui/components/app/appLayerModeHelpers.ts`
- `src/ui/components/app/appLayoutHelpers.ts`
- `src/ui/components/app/appMaterialSuggestionHelpers.ts`
- `src/ui/components/app/appModePreferenceHelpers.ts`
- `src/ui/components/app/appModeTabHelpers.ts`
- `src/ui/components/app/appProjectLoadHelpers.ts`
- `src/ui/components/app/appRecoveryHelpers.ts`
- `src/ui/components/app/appSelectionHelpers.ts`
- `src/ui/components/app/appStartModeDecisions.ts`
- `src/ui/components/app/appStartModeSelectionHelpers.ts`
- `src/ui/components/app/appTextCommitHelpers.ts`
- `src/ui/components/app/appTextDialogOpenHelpers.ts`
- `src/ui/components/app/appTextPreviewFontHelpers.ts`
- `src/ui/components/app/appToolpathPreviewHelpers.ts`
- `src/ui/components/appCanvasViewportProps.ts`
- `src/ui/components/appConnectionPanelProps.ts`
- `src/ui/components/appFileToolbarProps.ts`
- `src/ui/components/box-library/BoxCategoryTabs.tsx`
- `src/ui/components/box-library/BoxFeatureChips.tsx`
- `src/ui/components/box-library/BoxGeneratorControls.tsx`
- `src/ui/components/box-library/BoxLibraryPanel.tsx`
- `src/ui/components/box-library/BoxLibrarySearch.tsx`
- `src/ui/components/box-library/BoxPresetCard.tsx`
- `src/ui/components/box-library/BoxPresetDetail.tsx`
- `src/ui/components/box-library/BoxPresetPreview.tsx`
- `src/ui/components/box-library/BoxStudioWorkspace.tsx`
- `src/ui/components/canvas/CanvasRenderer.tsx`
- `src/ui/components/canvas/ModeTabsOverlay.tsx`
- `src/ui/components/canvas/canvasSnapHelpers.ts`
- `src/ui/components/canvas/canvasViewportHelpers.ts`
- `src/ui/components/common/FontPicker.tsx`
- `src/ui/components/connection/ConnectWizard.tsx`
- `src/ui/components/connection/ConnectionDetailsPanel.tsx`
- `src/ui/components/connection/Controls.tsx`
- `src/ui/components/connection/Issues.tsx`
- `src/ui/components/connection/JobLayoutMiniMap.tsx`
- `src/ui/components/connection/Jog.tsx`
- `src/ui/components/connection/MachineControls.tsx`
- `src/ui/components/connection/Progress.tsx`
- `src/ui/components/connection/ReadyToRunPanel.tsx`
- `src/ui/components/connection/StartReadinessPanel.tsx`
- `src/ui/components/connection/StatusBar.tsx`
- `src/ui/components/connection/StructuredLogPanel.tsx`
- `src/ui/components/connection/UnsafeAtConnectBanner.tsx`
- `src/ui/components/connection/Workflow.tsx`
- `src/ui/components/connection/buildStartReadiness.ts`
- `src/ui/components/connection/connectionPanelEquality.ts`
- `src/ui/components/connection/connectionPanelLabels.ts`
- `src/ui/components/connection/jobModePlanSummary.ts`
- `src/ui/components/connection/playCompletionBeep.ts`
- `src/ui/components/connection/unsafeAtConnectMessages.ts`
- `src/ui/components/falcon-wifi/FalconAlarmToast.tsx`
- `src/ui/components/falcon-wifi/FalconWiFiConnectBlock.tsx`
- `src/ui/components/falcon-wifi/FalconWiFiStatusPanel.tsx`
- `src/ui/components/falcon-wifi/falconIpc.ts`
- `src/ui/components/falcon-wifi/index.ts`
- `src/ui/components/gcodePreviewModel.ts`
- `src/ui/components/layers/groupMaterialPresets.ts`
- `src/ui/components/layers/layerPanelStyles.ts`
- `src/ui/components/layers/layerTransforms.ts`
- `src/ui/components/materials/CalibrateMaterialDialog.tsx`
- `src/ui/components/properties/imageSettingsTransforms.ts`
- `src/ui/components/properties/propertiesPanelStyles.ts`
- `src/ui/components/properties/traceSceneTransforms.ts`
- `src/ui/components/settings/CalibrationSettingsTab.tsx`
- `src/ui/components/settings/GcodeSettingsTab.tsx`
- `src/ui/components/settings/MachineSettingsTab.tsx`
- `src/ui/components/settings/ProfilesSettingsTab.tsx`
- `src/ui/components/workflow/WorkflowPanel.tsx`
- `src/ui/components/workflow/derivePanelMode.ts`
- `src/ui/components/workflow/modes/ConnectingMode.tsx`
- `src/ui/components/workflow/modes/DisconnectedMode.tsx`
- `src/ui/components/workflow/modes/ModeStub.tsx`
- `src/ui/components/workflow/modes/PausedMode.tsx`
- `src/ui/components/workflow/modes/ReadyMode.tsx`
- `src/ui/components/workflow/modes/RecoveryMode.tsx`
- `src/ui/components/workflow/modes/RunningMode.tsx`
- `src/ui/components/workflow/modes/SetupMode.tsx`
- `src/ui/components/workflow/modes/setup/ConsoleTab.tsx`
- `src/ui/components/workflow/modes/setup/JobTab.tsx`
- `src/ui/components/workflow/modes/setup/MoveTab.tsx`
- `src/ui/components/workflow/modes/setup/TabBar.tsx`
- `src/ui/components/workflow/modes/setup/setupTabPersistence.ts`
- `src/ui/components/workflow/recoveryVariantFromState.ts`
- `src/ui/components/workflow/zones/PrimaryActionFooter.tsx`
- `src/ui/components/workflow/zones/TopBar.tsx`
- `src/ui/features/uiFeatureFlags.ts`
- `src/ui/history/HistoryManager.ts`
- `src/ui/history/SceneCommands.ts`
- `src/ui/history/estimateSceneBytes.ts`
- `src/ui/history/stripRegenerableCaches.ts`
- `src/ui/hooks/asyncSceneGuard.ts`
- `src/ui/hooks/useAppDeviceProfiles.ts`
- `src/ui/hooks/useAppGeneratorWorkflows.ts`
- `src/ui/hooks/useAppKeyboardWorkflow.ts`
- `src/ui/hooks/useAppMaterialWorkflows.ts`
- `src/ui/hooks/useAppNudgeWorkflow.ts`
- `src/ui/hooks/useBoxLibraryState.ts`
- `src/ui/hooks/useClipboard.ts`
- `src/ui/hooks/useCompileManager.ts`
- `src/ui/hooks/useConnectionHandlers.ts`
- `src/ui/hooks/useContextMenu.ts`
- `src/ui/hooks/useControllerConnection.ts`
- `src/ui/hooks/useDialogs.ts`
- `src/ui/hooks/useFileHandlers.ts`
- `src/ui/hooks/useGeneratorHandlers.ts`
- `src/ui/hooks/useGrblConnection.ts`
- `src/ui/hooks/useGrblDerivedMachineInfo.ts`
- `src/ui/hooks/useImageCacheWarmer.ts`
- `src/ui/hooks/useImport.ts`
- `src/ui/hooks/useKerfHandlers.ts`
- `src/ui/hooks/useKeyboardShortcuts.ts`
- `src/ui/hooks/useMachineService.ts`
- `src/ui/hooks/useMaterialHandlers.ts`
- `src/ui/hooks/useMaterialTestHandlers.ts`
- `src/ui/hooks/useModal.ts`
- `src/ui/hooks/usePersistentBoxPreferences.ts`
- `src/ui/hooks/useQuickActionHandlers.ts`
- `src/ui/hooks/useSceneOperations.ts`
- `src/ui/hooks/useSettingsLiveCapabilities.ts`
- `src/ui/hooks/useWizardHandlers.ts`
- `src/ui/pages/BoxStudioPage.tsx`
- `src/ui/recovery/RecoveryCard.tsx`
- `src/ui/recovery/RecoveryCardContent.ts`
- `src/ui/renderers/SceneRenderer.ts`
- `src/ui/renderers/SimulationRenderer.ts`
- `src/ui/renderers/sceneCornerHelpers.ts`
- `src/ui/renderers/sceneOverlayHelpers.ts`
- `src/ui/renderers/sceneRendererPredicates.ts`
- `src/ui/safety/SafetyOutcomeMessages.ts`
- `src/ui/scene/SceneCommitActions.ts`
- `src/ui/scene/SceneTransaction.ts`
- `src/ui/scene/TextOperationLayer.ts`
- `src/ui/stores/activeJobCanvasStore.ts`
- `src/ui/stores/appDialogsStore.ts`
- `src/ui/stores/appSettingsStore.ts`
- `src/ui/stores/editorStore.ts`
- `src/ui/stores/machineStartStore.ts`
- `src/ui/stores/sceneHistoryStore.ts`
- `src/ui/stores/sceneStore.ts`
- `src/ui/stores/viewportStore.ts`
- `src/ui/styles/theme.ts`
- `src/ui/tools/TextTool.ts`
- `src/ui/utils/proGate.ts`
- `src/ui/viewport.ts`

### `src/workers`

- `src/workers/ImagePrepWorker.ts`
- `src/workers/imagePrepClient.ts`

## Test Files

### `tests`

- `tests/acknowledge-fault-awaits-safety-off.test.ts`
- `tests/active-job-canvas-context-cleared.test.ts`
- `tests/active-job-canvas-context-pinned.test.ts`
- `tests/active-job-canvas-store.test.ts`
- `tests/active-layer-history-consistent.test.ts`
- `tests/active-profile-store.test.ts`
- `tests/add-text-dialog-font-load-failure.test.tsx`
- `tests/agent-handoff-current-state.test.ts`
- `tests/align-selection-object-to-object.test.ts`
- `tests/analyze-burn-bounds.test.ts`
- `tests/any-cast-cleanup.test.ts`
- `tests/app-activate-layer-helpers.test.ts`
- `tests/app-autosave-helpers.test.ts`
- `tests/app-camera-position-helpers.test.ts`
- `tests/app-canvas-viewport-props-extracted.test.ts`
- `tests/app-connection-panel-props-extracted.test.ts`
- `tests/app-delete-selection-helpers.test.ts`
- `tests/app-device-profiles-extracted.test.ts`
- `tests/app-drag-drop-overlay-extracted.test.ts`
- `tests/app-exit-helpers.test.ts`
- `tests/app-file-toolbar-props-extracted.test.ts`
- `tests/app-generator-workflows-extracted.test.ts`
- `tests/app-history-manager-owned-by-store.test.ts`
- `tests/app-history-navigation-helpers.test.ts`
- `tests/app-keyboard-workflow-extracted.test.ts`
- `tests/app-layer-mode-helpers.test.ts`
- `tests/app-layout-helpers.test.ts`
- `tests/app-material-suggestion-helpers.test.ts`
- `tests/app-material-workflows-extracted.test.ts`
- `tests/app-mode-preference-helpers.test.ts`
- `tests/app-mode-tab-helpers.test.ts`
- `tests/app-no-local-usestate.test.ts`
- `tests/app-nudge-workflow-extracted.test.ts`
- `tests/app-profile-apply-import.test.ts`
- `tests/app-project-load-helpers.test.ts`
- `tests/app-recovery-helpers.test.ts`
- `tests/app-recovery-setup-extracted.test.ts`
- `tests/app-selection-helpers.test.ts`
- `tests/app-settings-modal-extracted.test.ts`
- `tests/app-settings-store.test.ts`
- `tests/app-size-audit.test.ts`
- `tests/app-start-mode-decisions.test.ts`
- `tests/app-start-mode-selection-helpers.test.ts`
- `tests/app-text-commit-helpers.test.ts`
- `tests/app-text-dialog-open-helpers.test.ts`
- `tests/app-text-dialogs-extracted.test.ts`
- `tests/app-text-preview-font-helpers.test.ts`
- `tests/app-toolpath-preview-helpers.test.ts`
- `tests/app-version-from-package.test.ts`
- `tests/approval-nonce-store.test.ts`
- `tests/async-scene-producer-guards.test.ts`
- `tests/atomic-autosave-record.test.ts`
- `tests/auto-detect-includes-max-spindle.test.ts`
- `tests/auto-finalize-without-mounted-ui.test.ts`
- `tests/auto-m5-listener-cleared-on-disconnect.test.ts`
- `tests/auto-m5-routes-through-gateway.test.ts`
- `tests/auto-update-infrastructure.test.ts`
- `tests/autofocus-timeout-issues-safety-off.test.ts`
- `tests/autofocus.test.ts`
- `tests/autosave-dirty-flag-on-failure.test.ts`
- `tests/autosave-manual-save-separation.test.ts`
- `tests/autosave-pauses-during-active-job.test.ts`
- `tests/autosave-preserves-images.test.ts`
- `tests/autosave-previous-slot.test.ts`
- `tests/autosave-serialization.test.ts`
- `tests/autosave-storage.test.ts`
- `tests/backward-compat-project-fixtures.test.ts`
- `tests/bed-dimensions-known-blocks-start.test.ts`
- `tests/bed-height-resolver-parity.test.ts`
- `tests/bezier-flatten.test.ts`
- `tests/blue-noise-dither.test.ts`
- `tests/boolean-ops-throws-without-license.test.ts`
- `tests/box-dimension-modes.test.ts`
- `tests/box-generator-cross-field-validation.test.tsx`
- `tests/box-generator-cut-layer-assignment.test.tsx`
- `tests/box-generator-fit-warning.test.tsx`
- `tests/box-generator-library-integration.test.tsx`
- `tests/box-geometry.test.ts`
- `tests/box-joinery-v2.test.ts`
- `tests/box-library-filtering.test.tsx`
- `tests/box-library.test.ts`
- `tests/box-preset-generated-features.test.tsx`
- `tests/box-preset-preview-model.test.ts`
- `tests/browser-compatibility.test.ts`
- `tests/build-stamp.test.tsx`
- `tests/build-start-readiness.test.ts`
- `tests/burn-envelope-divergence.test.ts`
- `tests/burn-moves-2d.test.ts`
- `tests/burn-progress-ack-timing.test.ts`
- `tests/cable-yanked-mid-job.test.ts`
- `tests/calibrate-dialog.test.ts`
- `tests/calibration-analyzer.test.ts`
- `tests/calibration-grid.test.ts`
- `tests/canvas-snap-helpers.test.ts`
- `tests/canvas-toolpath-preview-travel-classification.test.ts`
- `tests/canvas-viewport-helpers.test.ts`
- `tests/capability-value-resolution.test.ts`
- `tests/checksum-validation.test.ts`
- `tests/ci-installer-builds.test.ts`
- `tests/clock-tamper.test.ts`
- `tests/code-signing-config.test.ts`
- `tests/command-classifier.test.ts`
- `tests/command-gates-honor-safety-state.test.ts`
- `tests/community-response-curve.test.ts`
- `tests/compile-cancellable.test.ts`
- `tests/compile-complexity-gate.test.ts`
- `tests/compile-race-guard.test.ts`
- `tests/compiled-job-state.test.ts`
- `tests/compiler-template-validation.test.ts`
- `tests/compound-boolean-ops.test.ts`
- `tests/compound-fill-generator.test.ts`
- `tests/compound-jobcompiler-metadata.test.ts`
- `tests/compound-offset-path.test.ts`
- `tests/compound-output-boundary.test.ts`
- `tests/compound-path-model.test.ts`
- `tests/compound-roles.test.ts`
- `tests/confirm-dialog-surfaces-divergence.test.ts`
- `tests/connect-abort-signal.test.ts`
- `tests/connect-browser-guidance.test.tsx`
- `tests/connect-button-mutex.test.tsx`
- `tests/connect-button-opens-panel-without-compile.test.ts`
- `tests/connect-cleanup-on-partial-failure.test.ts`
- `tests/connect-safe-state-clears-on-recovery.test.ts`
- `tests/connect-safe-state-handshake.test.ts`
- `tests/connection-details-panel.test.tsx`
- `tests/connection-failure-guidance.test.ts`
- `tests/connection-generation-guard.test.ts`
- `tests/connection-manager-fsm.test.ts`
- `tests/connection-panel-emergency-stop-service.test.ts`
- `tests/connection-panel-equality.test.ts`
- `tests/connection-panel-go-to-last-position.test.ts`
- `tests/connection-panel-jog-visible-without-scroll.test.ts`
- `tests/connection-panel-labels.test.ts`
- `tests/connection-panel-no-design-editing.test.ts`
- `tests/connection-panel-offset-button-disabled.test.ts`
- `tests/connection-panel-preflight-no-loop.test.tsx`
- `tests/connection-panel-unsafe-at-connect-banner.test.tsx`
- `tests/connection-panel-unsafe-at-connect-wiring.test.ts`
- `tests/conservative-unknown-capability-handling.test.ts`
- `tests/console-input.test.tsx`
- `tests/controller-bounds-checks-g91.test.ts`
- `tests/controller-bounds-full-scan.test.ts`
- `tests/controller-bounds-recheck.test.ts`
- `tests/controller-capabilities-enforced.test.ts`
- `tests/controller-family-profile-boundaries.test.ts`
- `tests/controller-fresh-status-recheck.test.ts`
- `tests/controller-interface-protocol-neutral.test.ts`
- `tests/controller-operations-api.test.ts`
- `tests/controller-safety-action-result-methods.test.ts`
- `tests/controller-safety-capabilities.test.ts`
- `tests/controller-safety-event-sink.test.ts`
- `tests/controller-settings-snapshot.test.ts`
- `tests/controller-shared-types-neutral-layer.test.ts`
- `tests/controller-stop-safety.test.ts`
- `tests/controller.test.ts`
- `tests/controls-reset-wcs-baseline.test.tsx`
- `tests/core-inline-id-generators.test.ts`
- `tests/core-time-random-audit.test.ts`
- `tests/correlation-ids.test.ts`
- `tests/crash-reporting-privacy.test.ts`
- `tests/critical-event-preservation.test.ts`
- `tests/csp-policy.test.ts`
- `tests/current-head-front-origin-orientation.test.ts`
- `tests/current-mode-frame-anchor.test.ts`
- `tests/customstart-mode-reassertion.test.ts`
- `tests/cut-open-paths-before-closed.test.ts`
- `tests/debug-state-graph-transition-log.test.ts`
- `tests/default-ci-release-confidence.test.ts`
- `tests/delete-marks-dirty.test.ts`
- `tests/dependency-security-scan.test.ts`
- `tests/deploy-pages-workflow.test.ts`
- `tests/determinism-gate.test.ts`
- `tests/deterministic-ids.test.ts`
- `tests/dev-build-self-check.test.ts`
- `tests/device-profile-basic-api.test.ts`
- `tests/device-profile-storage-migration.test.ts`
- `tests/dialog-open-file-size-limit.test.ts`
- `tests/dialog-open-no-full-path.test.ts`
- `tests/dialogs-store.test.ts`
- `tests/dirty-derivation.test.ts`
- `tests/dirty-state-app-migration.test.ts`
- `tests/disconnect-emergency-portref-compare-and-swap.test.ts`
- `tests/disconnect-laser-off-routes-through-notify.test.ts`
- `tests/disconnect-stops-job-gating.test.ts`
- `tests/disconnect-transaction.test.ts`
- `tests/disconnected-subpath-travel-safety.test.ts`
- `tests/distribute-selection-bounds-spacing.test.ts`
- `tests/dither-cache-key-content-hash.test.ts`
- `tests/dither-serpentine.test.ts`
- `tests/door-status-classification.test.ts`
- `tests/dxf-polyline-vertex-import.test.ts`
- `tests/dxf-unit-import.test.ts`
- `tests/e2e-semantic-assertions.test.ts`
- `tests/editor-store.test.ts`
- `tests/electron-csp-integration.test.ts`
- `tests/electron-job-lifecycle-token.test.ts`
- `tests/electron-navigation-blocked.test.ts`
- `tests/electron-renderer-sandbox.test.ts`
- `tests/electron-serial-permission-trust.test.ts`
- `tests/emitted-burn-envelope-arcs.test.ts`
- `tests/emitted-burn-envelope-r-mode-arcs.test.ts`
- `tests/emitted-burn-envelope-stream.test.ts`
- `tests/emitted-burn-envelope.test.ts`
- `tests/end-to-end-workflows-exits-naturally.test.ts`
- `tests/entitlement-api-migration-phase2a.test.ts`
- `tests/entitlement-api-migration-phase2b.test.ts`
- `tests/entitlement-api-no-deprecated-export.test.ts`
- `tests/entitlement-api-split.test.ts`
- `tests/entitlement-signed-cache-authority.test.ts`
- `tests/entitlement-storage-migration.test.ts`
- `tests/error-boundary.test.ts`
- `tests/error-handler-faults-active-job.test.ts`
- `tests/error-handler-sends-safety-off.test.ts`
- `tests/error-reporter.test.ts`
- `tests/eslint-cleanup-sweep.test.ts`
- `tests/exact-idle-gates.test.ts`
- `tests/execute-job-output-contract.test.ts`
- `tests/execution-coordinator-autofocus.test.ts`
- `tests/execution-coordinator-deadman.test.ts`
- `tests/execution-coordinator-disconnect.test.ts`
- `tests/execution-coordinator-testfire-setorigin.test.ts`
- `tests/execution-coordinator-unlock-home-frame.test.ts`
- `tests/execution-coordinator-uses-command-gateway.test.ts`
- `tests/execution-coordinator.test.ts`
- `tests/exported-symbol-inventory.test.ts`
- `tests/failed-start-persists-log.test.ts`
- `tests/failed-start-preserves-unsafe-state-when-streamed.test.ts`
- `tests/failed-start-uses-bytes-written-counter.test.ts`
- `tests/falcon-autofocus-firmware-gate.test.ts`
- `tests/falcon-autofocus-heal.test.ts`
- `tests/falcon-serial-profile.test.ts`
- `tests/falcon-wifi-fake-server.test.ts`
- `tests/falcon-wifi-ipc-target-validation.test.ts`
- `tests/falcon-wifi-trust-blocks-start.test.ts`
- `tests/falcon-wifi-trust.test.ts`
- `tests/falcon-ws-frame-cap.test.ts`
- `tests/family-agnostic-ticket.test.ts`
- `tests/fault-injecting-serial-port.test.ts`
- `tests/fault-injection-scenarios.test.ts`
- `tests/feature-matrix-enforcement.test.ts`
- `tests/file-toolbar-new-project-dirty-setup.test.tsx`
- `tests/fill-generator-active-edge-table.test.ts`
- `tests/fill-geometry-helpers.test.ts`
- `tests/fill-interval-cap-preflight.test.ts`
- `tests/fill-no-rows-throws-not-outline-fallback.test.ts`
- `tests/fill-rows-cache.test.ts`
- `tests/fill-with-holes.test.ts`
- `tests/firmware-adapter-type-contract.test.ts`
- `tests/first-run-guide-wiring.test.ts`
- `tests/first-run-guide.test.ts`
- `tests/flattening-tolerance-by-operation.test.ts`
- `tests/fonts.test.ts`
- `tests/footer-m5-appended-at-send.test.ts`
- `tests/force-safe-state-primitive.test.ts`
- `tests/frame-bounds-output-layer-filter.test.ts`
- `tests/frame-confirm-uses-machine-corners.test.ts`
- `tests/frame-current-mode-emits-first-move.test.ts`
- `tests/frame-dot-feed-rate.test.ts`
- `tests/frame-dot-finally-emits-m5.test.ts`
- `tests/frame-freshness-invalidation.test.ts`
- `tests/frame-gcode-crosshair.test.ts`
- `tests/frame-gcode-pure.test.ts`
- `tests/frame-idle-timeout-dynamic.test.ts`
- `tests/frame-line-delay-profile-driven.test.ts`
- `tests/frame-required-before-start.test.ts`
- `tests/frame-result-reasons.test.ts`
- `tests/frame-saved-origin-verify.test.ts`
- `tests/frame-state-transitions.test.ts`
- `tests/frame-ticket-start-gate.test.ts`
- `tests/frame-vs-burn-equivalence.test.ts`
- `tests/full-suite-expected-noise-captured.test.ts`
- `tests/gated-feature-per-feature.test.ts`
- `tests/gcode-emitter-purity-and-zero-distance.test.ts`
- `tests/gcode-encoder-state-isolation.test.ts`
- `tests/gcode-hard-off-zero-power-travel.test.ts`
- `tests/gcode-header-modal-baseline.test.ts`
- `tests/gcode-preview-arcs.test.ts`
- `tests/gcode-preview-large-job-sampling.test.ts`
- `tests/gcode-preview-laser-state.test.ts`
- `tests/gcode-preview-output-parity-fixtures.test.ts`
- `tests/gcode-preview-relative-mode.test.ts`
- `tests/gcode-relative-mode.test.ts`
- `tests/gcode-relative-return-template.test.ts`
- `tests/gcode-settings-custom-start-end-wiring.test.ts`
- `tests/gcode-streaming-foundation.test.ts`
- `tests/gcode-template-sanitization.test.ts`
- `tests/gcode-template-validator.test.ts`
- `tests/gcode-templates-safety.test.ts`
- `tests/gcode-templates.test.ts`
- `tests/geometry-deep-validation.test.ts`
- `tests/grbl-byte-buffer-accounting.test.ts`
- `tests/grbl-compatibility-settings-ui.test.tsx`
- `tests/grbl-connect-abort-signal.test.ts`
- `tests/grbl-connection-generation-guard.test.ts`
- `tests/grbl-derived-machine-info.test.ts`
- `tests/grbl-diagnostics-commands.test.ts`
- `tests/grbl-firmware-adapter.test.ts`
- `tests/grbl-handshake-rejects-bare-ok.test.ts`
- `tests/grbl-identity-parser.test.ts`
- `tests/grbl-identity-verification.test.ts`
- `tests/grbl-job-bounds-checker.test.ts`
- `tests/grbl-job-line-parser.test.ts`
- `tests/grbl-legacy-jog-mode.test.ts`
- `tests/grbl-marker-lifecycle.test.ts`
- `tests/grbl-operation-validators-reject-bad-numbers.test.ts`
- `tests/grbl-request-work-offsets.test.ts`
- `tests/grbl-safe-state-classifier.test.ts`
- `tests/grbl-settings-parser.test.ts`
- `tests/grbl-status-report-parser.test.ts`
- `tests/grbl-stream-fill-error-safety-off.test.ts`
- `tests/grbl-synchronous-transfer-mode.test.ts`
- `tests/grbl-system-line-tagging.test.ts`
- `tests/grbl-unsafe-at-connect-types.test.ts`
- `tests/grbl-wcs-consent-classifier.test.ts`
- `tests/grbl-wcs-parser.test.ts`
- `tests/grid-array-source-bounds.test.ts`
- `tests/group-material-presets.test.ts`
- `tests/group-ungroup-integrity.test.ts`
- `tests/history-entry-metadata.test.ts`
- `tests/history-memory-budget.test.ts`
- `tests/history.test.ts`
- `tests/home-corner-wizard-profile.test.ts`
- `tests/image-decompression-bomb.test.ts`
- `tests/image-import-dragdrop-supported-types.test.tsx`
- `tests/image-import-large-file-blob-store.test.ts`
- `tests/image-import-max-dim-cap.test.ts`
- `tests/image-prep-grayscale-equivalence.test.ts`
- `tests/image-processing-worker-equivalence.test.ts`
- `tests/image-processing.test.ts`
- `tests/image-reference-validation.test.ts`
- `tests/image-settings-transforms.test.ts`
- `tests/import-callback-identity-stable.test.tsx`
- `tests/import-limits.test.ts`
- `tests/import-operation-mapping.test.ts`
- `tests/installer-qa-matrix.test.ts`
- `tests/ipc-attack-surface.test.ts`
- `tests/ipc-sender-verification.test.ts`
- `tests/job-checkpoint.test.ts`
- `tests/job-compiler-curve-flatness.test.ts`
- `tests/job-compiler-helpers.test.ts`
- `tests/job-compiler-no-plan-runtime-imports.test.ts`
- `tests/job-complexity-summary.test.ts`
- `tests/job-execution-session.test.ts`
- `tests/job-fingerprint-start-validation.test.ts`
- `tests/job-fingerprint.test.ts`
- `tests/job-layout-minimap.test.tsx`
- `tests/job-log-quota.test.ts`
- `tests/job-mode-plan-summary.test.ts`
- `tests/job-outcome-enum-failed-to-start.test.ts`
- `tests/job-replay-capture-not-gated.test.ts`
- `tests/job-session-transitions.test.ts`
- `tests/job-start-deferred-simulator-fanout.test.ts`
- `tests/jobcompiler-keeps-pro-settings-with-license.test.ts`
- `tests/jobcompiler-progress-cancel.test.ts`
- `tests/jobcompiler-raster-preprocessing-cancel.test.ts`
- `tests/jobcompiler-strips-pro-settings-without-license.test.ts`
- `tests/jobcompiler-uses-processed-data-pass4b.test.ts`
- `tests/joblog-event-window-retention.test.ts`
- `tests/joblog-storage-migration.test.ts`
- `tests/jobreplay-storage-migration.test.ts`
- `tests/jog-and-setorigin-state-after-confirm.test.ts`
- `tests/kerf-presets.test.ts`
- `tests/kerf-wizard-bounds-block.test.tsx`
- `tests/kerf-wizard-coupon-geometry.test.tsx`
- `tests/kerf-wizard-requires-selection-apply.test.tsx`
- `tests/keyboard-boolean-shortcut-gate.test.ts`
- `tests/large-project-handling.test.ts`
- `tests/large-raster-plan-lazy-materialization.test.ts`
- `tests/laser-mode-banner.test.tsx`
- `tests/last-machine-position.test.ts`
- `tests/layer-panel-styles.test.ts`
- `tests/layer-panel-tabs-entitlement.test.ts`
- `tests/layer-transforms.test.ts`
- `tests/lead-in-off-contour.test.ts`
- `tests/lf004-spooled-compile-materialization.test.ts`
- `tests/license-status-machine.test.ts`
- `tests/license-status-states.test.ts`
- `tests/lifecycle-revocation.test.ts`
- `tests/lightburn-layout-reset-affordance.test.ts`
- `tests/lightburn-settings-compatibility.test.ts`
- `tests/load-repair-report.test.ts`
- `tests/local-origin-transform-preserves-orientation.test.ts`
- `tests/machine-command-gateway.test.ts`
- `tests/machine-control-v2-grbl-jog-realtime.test.ts`
- `tests/machine-control-v2-grbl-settings.test.ts`
- `tests/machine-control-v2-grbl-transport.test.ts`
- `tests/machine-control-v2-intents.test.ts`
- `tests/machine-control-v2-operations.test.ts`
- `tests/machine-control-v2-panel-adapter.test.ts`
- `tests/machine-control-v2-readiness.test.ts`
- `tests/machine-control-v2-state-machine.test.ts`
- `tests/machine-control-v2-wcs-cutover.test.ts`
- `tests/machine-event-ledger-job-lifecycle-wiring.test.ts`
- `tests/machine-event-ledger-pause-resume-wiring.test.ts`
- `tests/machine-event-ledger-production-wiring.test.ts`
- `tests/machine-event-ledger-recovery-cleared-wiring.test.ts`
- `tests/machine-event-ledger-safety-off-wiring.test.ts`
- `tests/machine-event-ledger.test.ts`
- `tests/machine-origin-overlay-marker.test.ts`
- `tests/machine-plan-bounds-source.test.ts`
- `tests/machine-safety-state.test.ts`
- `tests/machine-service-disconnect-safety-result.test.ts`
- `tests/machine-service-emergency-stop-safety-result.test.ts`
- `tests/machine-service-helpers.test.ts`
- `tests/machine-service-job-lifecycle-safety.test.ts`
- `tests/machine-service-job-log-checkpoint-wiring.test.ts`
- `tests/machine-service-jog-respects-mutex.test.ts`
- `tests/machine-service-known-port-reuse.test.ts`
- `tests/machine-service-laser-state-subscription.test.ts`
- `tests/machine-service-pause-resume.test.ts`
- `tests/machine-service-safety-state-machine.test.ts`
- `tests/machine-service-sendcommand-uses-gateway-policy.test.ts`
- `tests/machine-service-start-validated-job.test.ts`
- `tests/machine-service-user-sendcommand.test.ts`
- `tests/machine-settings-capability-indicators.test.ts`
- `tests/machine-settings-stop-on-error-toggle.test.ts`
- `tests/machine-setup-compatibility-contract.test.ts`
- `tests/machine-start-store.test.ts`
- `tests/machine-transform-fused-bounds.test.ts`
- `tests/macos-icon-size.test.ts`
- `tests/macos-signing-notarization-workflow.test.ts`
- `tests/manual-save-needs-acknowledgement.test.ts`
- `tests/marlin-firmware-adapter-stub.test.ts`
- `tests/material-feedback-storage.test.ts`
- `tests/material-first-workflow.test.ts`
- `tests/material-library-storage.test.ts`
- `tests/material-preset-kerf-compile.test.ts`
- `tests/material-preset-schema.test.ts`
- `tests/material-preset-snapshot.test.ts`
- `tests/material-presets-storage.test.ts`
- `tests/material-test-bounds-block.test.tsx`
- `tests/material-test-safe-ordering.test.tsx`
- `tests/materials.test.ts`
- `tests/maxspindle-precedence-and-mismatch.test.ts`
- `tests/migration-pipeline-wired-into-loader.test.ts`
- `tests/migration-pipeline.test.ts`
- `tests/modal-confirm-with-checkbox.test.tsx`
- `tests/namespace-isolation.test.ts`
- `tests/native-deps-prebuild-check.test.ts`
- `tests/nesting-group-unit.test.ts`
- `tests/nesting-partial-apply-block.test.ts`
- `tests/nesting-rotation-apply.test.ts`
- `tests/nesting-throws-without-license.test.ts`
- `tests/no-direct-sendcommand-outside-gateway.test.ts`
- `tests/no-electron-sendgcode-export.test.ts`
- `tests/no-gcode-in-ui.test.ts`
- `tests/no-live-pro-flag-writes.test.ts`
- `tests/no-localstorage-in-core.test.ts`
- `tests/no-scene-canvas-in-machine-coord.test.ts`
- `tests/notify-simulator-chunked-warns-on-broken-listener.test.ts`
- `tests/numberinput-focused-not-overridden.test.tsx`
- `tests/numberinput-no-loop-on-rapid-prop-change.test.tsx`
- `tests/numberinput-tolerance-still-works.test.tsx`
- `tests/offset-preserves-holes.test.ts`
- `tests/open-path-boolean-offset-rejects.test.ts`
- `tests/openbuilds-boundary-comparison.test.ts`
- `tests/operation-gate-decisions.test.ts`
- `tests/operation-mutex-lease-tokens.test.ts`
- `tests/operation-mutex-prevents-overlap.test.ts`
- `tests/operation-order-warning.test.ts`
- `tests/operation-ordering.test.ts`
- `tests/origin-mode-wcs-zero.test.ts`
- `tests/output-deterministic-with-clock-injection.test.ts`
- `tests/output-gcode-semantic-preflight.test.ts`
- `tests/output-marker-encoding.test.ts`
- `tests/output-progress-cancel.test.ts`
- `tests/output-safety-wrapper-non-removable.test.ts`
- `tests/output-target-resolution.test.ts`
- `tests/overcut-final-pass-only.test.ts`
- `tests/packaged-app-no-dev-arg.test.ts`
- `tests/parse-gcode-helper.test.ts`
- `tests/path-optimizer-deterministic-budget.test.ts`
- `tests/pause-button-immediate-feedback.test.ts`
- `tests/pause-button-safety-result-refusal.test.ts`
- `tests/pause-emits-m5-after-feed-hold.test.ts`
- `tests/pause-laser-off-confirmation.test.ts`
- `tests/pause-resume-stop-surface-errors.test.ts`
- `tests/per-feature-granularity.test.ts`
- `tests/pipeline-compile-accepts-profile-snapshot.test.ts`
- `tests/pipeline.test.ts`
- `tests/placement.test.ts`
- `tests/plan-accel-sanity.test.ts`
- `tests/plan-marker-emission.test.ts`
- `tests/plan-optimizer-helpers.test.ts`
- `tests/plan-optimizer-large-raster.test.ts`
- `tests/plan-optimizer-mid-operation-cancel.test.ts`
- `tests/plan-optimizer-progress-cancel.test.ts`
- `tests/plan-stats-time-estimation-defaults.test.ts`
- `tests/play-completion-beep.test.ts`
- `tests/poll-status-failure-normalized.test.ts`
- `tests/position-trust-transitions.test.ts`
- `tests/preflight-blocks-when-maxspindle-unknown.test.ts`
- `tests/preflight-bounds.test.ts`
- `tests/preflight-capability-mismatches.test.ts`
- `tests/preflight-confirm-surfaces-info.test.ts`
- `tests/preflight-duplicate-geometry.test.ts`
- `tests/preflight-lightburn-policy.test.ts`
- `tests/preflight-negative-coords.test.ts`
- `tests/preflight-output-layer-filter.test.ts`
- `tests/preflight-rejects-m4-without-laser-mode.test.ts`
- `tests/preflight-rules-no-orchestrator-import.test.ts`
- `tests/preflight-self-intersection.test.ts`
- `tests/preflight-template-validation.test.ts`
- `tests/preflight-validator-dialect-routing.test.ts`
- `tests/preflight-warning-confirm-includes-detail.test.ts`
- `tests/preflight-z-axis-bounds.test.ts`
- `tests/preflight.test.ts`
- `tests/preset-ops-helpers.test.ts`
- `tests/pro-feature-ui-scoped-gates.test.ts`
- `tests/production-bundle-smoke.test.ts`
- `tests/production-console-log-structured.test.ts`
- `tests/production-security-source-checks.test.ts`
- `tests/profile-auto-m5-on-connect.test.ts`
- `tests/profile-change-blocks-start.test.ts`
- `tests/profile-overrides-wired-to-pipeline.test.ts`
- `tests/profile-schema-validation.test.ts`
- `tests/profile-selector-pinned-to-header.test.ts`
- `tests/profile-validation.test.ts`
- `tests/progress-model-multi-domain.test.ts`
- `tests/project-map-generated.test.ts`
- `tests/project-profile-snapshot.test.ts`
- `tests/project-runtime-reset.test.ts`
- `tests/properties-panel-styles.test.ts`
- `tests/property-geometry.test.ts`
- `tests/prt4040-connection-home-gate.test.ts`
- `tests/prt4040-router-laser-profile.test.ts`
- `tests/prt4040-wizard-and-start-mode.test.ts`
- `tests/raster-bidirectional-row-parity.test.ts`
- `tests/raster-buffers-out-of-history.test.ts`
- `tests/raster-dither-selection-commit.test.ts`
- `tests/raster-gcode-streaming.test.ts`
- `tests/raster-grayscale-merge-tolerance.test.ts`
- `tests/raster-m4-no-software-splitting.test.ts`
- `tests/raster-move-iterator.test.ts`
- `tests/raster-output-uses-modal-m4.test.ts`
- `tests/raster-overscan-as-s0-travel.test.ts`
- `tests/raster-pixel-fixtures.test.ts`
- `tests/raster-power-min-preflight.test.ts`
- `tests/raster-with-curve.test.ts`
- `tests/react-hooks-clean.test.ts`
- `tests/ready-to-run-panel.test.tsx`
- `tests/reconnect-same-machine-verification.test.ts`
- `tests/reconstruction-grade-joblog.test.ts`
- `tests/recovery-bypass-requires-token.test.ts`
- `tests/recovery-card-actions-ack-after-work.test.ts`
- `tests/recovery-card-actions-advance-state.test.ts`
- `tests/recovery-card-ui-wiring.test.ts`
- `tests/recovery-cards.test.ts`
- `tests/recovery-eligibility-non-empty-cases.test.ts`
- `tests/recovery-failure-alerts.test.tsx`
- `tests/recovery-state-blocks-start.test.ts`
- `tests/recovery-state-transitions.test.ts`
- `tests/redaction.test.ts`
- `tests/release-artifact-attestations.test.ts`
- `tests/release-artifact-integrity.test.ts`
- `tests/release-github-publish-workflows.test.ts`
- `tests/release-openbuilds-antipatterns.test.ts`
- `tests/release-sbom-workflows.test.ts`
- `tests/renderer-lifecycle-safety.test.ts`
- `tests/resolve-frame-transform-bounds.test.ts`
- `tests/response-curve.test.ts`
- `tests/resume-awaits-modal-restore.test.ts`
- `tests/resume-modal-tokenization.test.ts`
- `tests/retention-policy.test.ts`
- `tests/right-origin-x-flip.test.ts`
- `tests/roadmap-backfill-t1-209-222.test.ts`
- `tests/roadmap-current-counts.test.ts`
- `tests/rollback-crash-loop-detection.test.ts`
- `tests/rotated-raster-fails-closed.test.ts`
- `tests/ruida-safety-design-doc.test.ts`
- `tests/run-frame-fail-fast-on-blocked-command.test.ts`
- `tests/runner-auto-discovery.test.ts`
- `tests/runtime-state-meta.test.ts`
- `tests/safety-action-result.test.ts`
- `tests/safety-controls-bypass-entitlement.test.ts`
- `tests/safety-off-outcome-routing.test.ts`
- `tests/safety-off-two-stage.test.ts`
- `tests/safety-operations-controller-routing.test.ts`
- `tests/safety-ops-contract.test.ts`
- `tests/safety-outcome-messages.test.ts`
- `tests/safety-state-transitions.test.ts`
- `tests/safety-write-failure-surfaces.test.ts`
- `tests/saved-origin-production-tip.test.ts`
- `tests/saved-origin-verifies-wcs.test.ts`
- `tests/saved-origin-wcs-lifecycle.test.ts`
- `tests/savedorigin-not-compile-invalidating.test.ts`
- `tests/scanning-offset.test.ts`
- `tests/scene-bounds-selectors.test.ts`
- `tests/scene-canvas-machine-coord-check.test.ts`
- `tests/scene-commit-actions-wired.test.ts`
- `tests/scene-corner-helpers.test.ts`
- `tests/scene-dirty-hash-core-layer.test.ts`
- `tests/scene-history-store.test.ts`
- `tests/scene-io.test.ts`
- `tests/scene-overlay-helpers.test.ts`
- `tests/scene-renderer-predicates.test.ts`
- `tests/scene-serializer-helpers.test.ts`
- `tests/scene-serializer-typed-boundaries.test.ts`
- `tests/scene-store.test.ts`
- `tests/scene-transaction-app-wired.test.ts`
- `tests/scene-transaction-unified.test.ts`
- `tests/security-deps-pinned.test.ts`
- `tests/selection-restore-on-history.test.ts`
- `tests/serial-known-port-reuse.test.ts`
- `tests/serial-navigator-disconnect.test.ts`
- `tests/serial-port-close-async.test.ts`
- `tests/serial-port-subscription-wiring.test.ts`
- `tests/server-entitlement-service.test.ts`
- `tests/server-webcrypto-entitlement-signer.test.ts`
- `tests/service-layer-pro-gate-coverage.test.ts`
- `tests/service-start-blocks-unknown-bed.test.ts`
- `tests/signed-entitlement-token.test.ts`
- `tests/simulation.test.ts`
- `tests/simulator-view-ymirror.test.ts`
- `tests/smart-overscan.test.ts`
- `tests/source-maps-hidden-mode.test.ts`
- `tests/source-maps-not-shipped.test.ts`
- `tests/source-text-migration.test.ts`
- `tests/stale-followup-doc-sweep.test.ts`
- `tests/stale-gcode-blocks-start.test.ts`
- `tests/start-mode-labels.test.ts`
- `tests/start-mode-wcs-reset.test.ts`
- `tests/start-readiness-auto-expand.test.tsx`
- `tests/start-readiness-frame-control-gate.test.ts`
- `tests/start-readiness-panel.test.tsx`
- `tests/start-validated-job-passes-context.test.ts`
- `tests/start-validated-job-then-immediately-tryfinalize.test.ts`
- `tests/startup-diagnostics.test.ts`
- `tests/stop-on-error-no-bypass-source-guard.test.ts`
- `tests/stop-on-error-override.test.ts`
- `tests/stop-on-error-token-in-interface.test.ts`
- `tests/storage-adapter-contract.test.ts`
- `tests/storage-filesystem-unit.test.ts`
- `tests/storage-health.test.ts`
- `tests/storage-ipc-no-broad-clear.test.ts`
- `tests/storage-singleton.test.ts`
- `tests/storage-size-limits.test.ts`
- `tests/streaming-health-saturation.test.ts`
- `tests/streaming-health.test.ts`
- `tests/structured-log-events.test.ts`
- `tests/structured-rx-tx-entries.test.ts`
- `tests/support-bundle-ui-export.test.ts`
- `tests/support-bundle-zip-export.test.ts`
- `tests/support-bundle.test.ts`
- `tests/svg-complexity-limits.test.ts`
- `tests/svg-file-size-preflight.test.ts`
- `tests/svg-import-placement.test.ts`
- `tests/svg-import.test.ts`
- `tests/svg-inherited-group-styles.test.ts`
- `tests/svg-parser-helpers.test.ts`
- `tests/svg-text-import-warning.test.ts`
- `tests/svg-unsupported-feature-warning.test.ts`
- `tests/svg-use-defs-import.test.ts`
- `tests/svg-viewbox-unit-choice.test.ts`
- `tests/t1-26-defense-in-depth-strips-comments.test.ts`
- `tests/t2-26-operation-routing-static-guard.test.ts`
- `tests/tab-gap-uses-feed-not-rapid.test.ts`
- `tests/template-safety-enforcement.test.ts`
- `tests/template-validation-aggregates-errors.test.ts`
- `tests/template-validator.test.ts`
- `tests/temporary-pro-access-unlocks-all-features.test.ts`
- `tests/test-grid-dialog-preview-flow.test.ts`
- `tests/test-grid-dialog-validation.test.tsx`
- `tests/test-grid-generator.test.ts`
- `tests/test-runner-lanes.test.ts`
- `tests/test-runner-timeout-diagnostics.test.ts`
- `tests/tester-secret-not-in-source.test.ts`
- `tests/tester-verification-no-secret.test.ts`
- `tests/text-layout-to-path.test.ts`
- `tests/text-operation-dialog-wiring.test.ts`
- `tests/text-operation-layer.test.ts`
- `tests/text-outline-cache.test.ts`
- `tests/text-property-edits-undoable.test.ts`
- `tests/ticket-capability-snapshot-validation.test.ts`
- `tests/ticket-determinism-entitlement-and-presets.test.ts`
- `tests/ticket-ledger-format-required.test.ts`
- `tests/ticket-validation-message-translation.test.ts`
- `tests/time-estimator-stream.test.ts`
- `tests/trace-contour-safety.test.ts`
- `tests/trace-scene-transforms.test.ts`
- `tests/trace-selection-atomic-commit.test.ts`
- `tests/trace-storm-probe.test.ts`
- `tests/transport-abstraction.test.ts`
- `tests/transport-multi-subscribe.test.ts`
- `tests/troubleshooting-panel.test.ts`
- `tests/trusted-command-boundary-contract.test.ts`
- `tests/try-finalize-after-observed-running.test.ts`
- `tests/try-finalize-respects-observed-running.test.ts`
- `tests/typed-error-domains.test.ts`
- `tests/typed-storage-ipc.test.ts`
- `tests/ui-integration.test.ts`
- `tests/ui-start-frame-ticket-proof.test.ts`
- `tests/ui-start-job-end-to-end.test.ts`
- `tests/ui-start-job-preserves-markers.test.ts`
- `tests/ui-start-job-uses-ticket.test.tsx`
- `tests/undo-during-job.test.ts`
- `tests/undo-redo-invalidation.test.ts`
- `tests/unsafe-prior-state-persistence.test.ts`
- `tests/unsafe-state-survives-disconnect-and-estop.test.ts`
- `tests/update-notice-ui.test.tsx`
- `tests/usecompilemanager-cancel-progress.test.tsx`
- `tests/usecompilemanager-stale-no-loop.test.tsx`
- `tests/user-data-migration.test.ts`
- `tests/user-mode-gates.test.ts`
- `tests/user-mode-policy-production-wiring.test.ts`
- `tests/validate-job-ticket.test.ts`
- `tests/validated-job-ticket-mismatch.test.ts`
- `tests/validated-job-ticket-phase1.test.ts`
- `tests/variable-text-expanded-spacing.test.tsx`
- `tests/variable-text-transform-preservation.test.tsx`
- `tests/velocity-profile.test.ts`
- `tests/viewport-store.test.ts`
- `tests/viewport.test.ts`
- `tests/virtual-scheduler.test.ts`
- `tests/wake-lock.test.ts`
- `tests/warm-processed-image-cache-pass4c.test.ts`
- `tests/wcs-command-helpers-use-gateway.test.ts`
- `tests/wcs-consent-suppression-policy.test.ts`
- `tests/wcs-fail-closed-integration.test.ts`
- `tests/wcs-fail-closed-on-unknown.test.ts`
- `tests/wcs-mutation-consent.test.ts`
- `tests/wcs-no-listener-blocks-job.test.ts`
- `tests/wcs-no-listener-headless-flag.test.ts`
- `tests/wcs-normalization-operation-gate.test.ts`
- `tests/wcs-profile-gate-contract.test.ts`
- `tests/wcs-query-error-fails-closed.test.ts`
- `tests/web-serial-byte-stream-harness.test.ts`
- `tests/web-serial-control-signals.test.ts`
- `tests/web-serial-partial-open-cleanup.test.ts`
- `tests/webcrypto-entitlement-verifier.test.ts`
- `tests/webserial-cable-pull-heartbeat.test.ts`
- `tests/webserial-unknown-catch.test.ts`
- `tests/windows-signing-release-workflow.test.ts`
- `tests/wizard-complete-does-not-enable-material.test.tsx`
- `tests/wizard-grbl-support-scope.test.ts`
- `tests/workflow-panel-derive-mode.test.ts`
- `tests/workflow-panel-feature-flag.test.ts`
- `tests/workflow-panel-frame-ticket-start-gate.test.ts`
- `tests/workflow-panel-phase2-modes.test.ts`
- `tests/workflow-panel-phase3-setup.test.ts`
- `tests/workflow-panel-phase4-live-job.test.ts`
- `tests/workflow-panel-scaffold.test.ts`
- `tests/workflow-panel-v2-beta-lock.test.ts`

### `tests/capability-regression`

- `tests/capability-regression/capability-regression-coverage.test.ts`

### `tests/connection-lifecycle`

- `tests/connection-lifecycle/connection-lifecycle-coverage.test.ts`

### `tests/controller-matrix`

- `tests/controller-matrix/operation-routing-by-family.test.ts`

### `tests/e2e`

- `tests/e2e/README.md`
- `tests/e2e/circle-cut.test.ts`
- `tests/e2e/engrave-fill.test.ts`
- `tests/e2e/fixtures/circleCut.ts`
- `tests/e2e/fixtures/engraveFill.ts`
- `tests/e2e/fixtures/largeScene.ts`
- `tests/e2e/fixtures/mixedScene.ts`
- `tests/e2e/fixtures/multiPassCut.ts`
- `tests/e2e/fixtures/originAbsolute.ts`
- `tests/e2e/fixtures/originSaved.ts`
- `tests/e2e/fixtures/rectangleCut.ts`
- `tests/e2e/fixtures/scoreLine.ts`
- `tests/e2e/fixtures/textBundledInter.ts`
- `tests/e2e/fixtures/textHersheySans.ts`
- `tests/e2e/helpers/compileToGcode.ts`
- `tests/e2e/helpers/e2eDeterministicIds.ts`
- `tests/e2e/helpers/prepareSceneForCompile.ts`
- `tests/e2e/helpers/semanticGcodeAssertions.ts`
- `tests/e2e/helpers/snapshot.ts`
- `tests/e2e/helpers/textFixtureObject.ts`
- `tests/e2e/large-scene.test.ts`
- `tests/e2e/mixed-scene.test.ts`
- `tests/e2e/multi-pass-cut.test.ts`
- `tests/e2e/origin-absolute.test.ts`
- `tests/e2e/origin-saved.test.ts`
- `tests/e2e/rectangle-cut.test.ts`
- `tests/e2e/score-line.test.ts`
- `tests/e2e/snapshots/.gitattributes`
- `tests/e2e/snapshots/circle-cut.gcode`
- `tests/e2e/snapshots/engrave-fill.gcode`
- `tests/e2e/snapshots/mixed-scene.gcode`
- `tests/e2e/snapshots/multi-pass-cut.gcode`
- `tests/e2e/snapshots/origin-absolute.gcode`
- `tests/e2e/snapshots/origin-saved.gcode`
- `tests/e2e/snapshots/rectangle-cut.gcode`
- `tests/e2e/snapshots/score-line.gcode`
- `tests/e2e/snapshots/test-grid-default.gcode`
- `tests/e2e/snapshots/text-bundled-inter.gcode`
- `tests/e2e/snapshots/text-hershey-sans.gcode`
- `tests/e2e/text-bundled-inter.test.ts`
- `tests/e2e/text-hershey-sans.test.ts`

### `tests/end-to-end-workflows`

- `tests/end-to-end-workflows/end-to-end-workflows.test.ts`

### `tests/entitlement-tamper-resistance`

- `tests/entitlement-tamper-resistance/tamper-resistance.test.ts`

### `tests/fixtures`

- `tests/fixtures/projects/v1-basic-cut.lfproj`
- `tests/fixtures/projects/v1-legacy-minimal-score.lfproj`
- `tests/fixtures/projects/v1_1-multilayer-vector.lfproj`

### `tests/harness`

- `tests/harness/fakeFalconServer.ts`
- `tests/harness/fakeWebSerial.ts`

### `tests/helpers`

- `tests/helpers/ControllerFault.ts`
- `tests/helpers/FaultInjectingSerialPort.ts`
- `tests/helpers/VirtualScheduler.ts`
- `tests/helpers/analyzeBurnBounds.ts`
- `tests/helpers/imageFixtures.ts`
- `tests/helpers/parseGcode.ts`
- `tests/helpers/propertyTesting.ts`
- `tests/helpers/testFrameTicket.ts`
- `tests/helpers/testJobFingerprint.ts`

### `tests/ipc-fuzz`

- `tests/ipc-fuzz/ipc-fuzz.test.ts`

### `tests/malformed`

- `tests/malformed/bad-image.test.ts`
- `tests/malformed/bad-settings.test.ts`
- `tests/malformed/bad-svg.test.ts`
- `tests/malformed/bad-template.test.ts`
- `tests/malformed/extreme-coordinates.test.ts`

### `tests/packaged-app-smoke`

- `tests/packaged-app-smoke/packaged-app-build-config.test.ts`

### `tests/perf`

- `tests/perf/large-job-stress.test.ts`
- `tests/perf/streaming-expected-blockers.test.ts`
- `tests/perf/text-outline-stress.test.ts`

### `tests/persistence-stress`

- `tests/persistence-stress/persistence-stress.test.ts`

### `tests/safety-controller-matrix`

- `tests/safety-controller-matrix/unknown-controller-safety.test.ts`

### `tests/simulators`

- `tests/simulators/GrblSimulator.ts`
- `tests/simulators/SimulatedControllerDevice.ts`
- `tests/simulators/grbl-simulator.test.ts`
- `tests/simulators/simulated-controller-device.test.ts`

### `tests/undo-redo-correctness`

- `tests/undo-redo-correctness/undo-redo-correctness.test.ts`
