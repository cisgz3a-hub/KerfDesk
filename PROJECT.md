...
> **Status:** v4.1 — KerfDesk Desktop Preview governance, packaging, and notify-only update discovery are implemented under ADR-247/248/249; immutable public prereleases are published through `v0.2.0-preview.13`, while installed-package and real-OS qualification remain separate. Repository release immutability is enabled. The existing web/PWA stays first-class; exact-version Windows x64 and macOS x64/arm64 Electron Previews do not change machining workflows, core, toolpaths, runtime trust, or the current MIT/public/free posture. Phase L "Image Studio" remains in progress under ADR-242. ADR-292 implements the bounded P2R.1a substrate: project schema v4, a canonical U16LE heightfield, exact legacy migration, worker-backed qualified grayscale import with simple transparency masks, and existing relief CAM/preview integration. ADR-292 Amendment 2 keeps canonical Width and bounds synchronized through binary64 ratio extremes, uses one source authority, and discloses the resolved aspect policy without adding a guard. ADR-292 Amendment 3 makes that canonical Width the nonzero-scale authority for selected properties, canvas-preview budgeting/cache identity, and Relief 3D title/resolution while preserving exact-zero compatibility and legacy meshes. ADR-292 Amendment 4 keeps an oversized but exactly common-factor-rebaseable heightfield Width edit inside project-v4 coordinate and scale domains without changing its machine geometry; exact-zero, common-factor scale exhaustion, non-reversible, and independent-axis-only encodings remain outside the bounded repair, while fully durable positive-finite intent remains an explicit future-format representation gap. The remaining P2R.1 product surface and P2R.2-P2R.6 stay planned. ADR-293 adds focused one-command KerfDesk user macros through the existing Console path. ADR-294 gives non-divisible relief grids an exact logical physical boundary without rewriting the requested interior spacing and discloses preview/emitter representation limits. ADR-295 qualifies exact non-interlaced grayscale PNG at 8 or 16 bits, ADR-296 preserves exact 8-bit grayscale-alpha samples as the same canonical field plus U8 mask, ADR-297 exposes the persisted-threshold outside-mask meaning, ADR-298 exposes exact U16 input endpoints, ADR-299 exposes exact inclusion-threshold editing, ADR-300 exposes the persisted declared source meaning, and ADR-301 exposes recorded source details in Relief properties without treating that metadata as authenticated. These controls and disclosures do not rewrite source bytes or change CAM algorithms. ADR-302 removes the obsolete preflight warning that falsely described supported raster rotation as unemittable; compile and emitted-byte behavior are unchanged. ADR-303 corrects current Frame/Job Review workflow and idle control copy to match the implemented dialog-free Frame, review-pending permit, and Start-time review sequence; runtime behavior is unchanged. ADR-304 adds a heightfield-only, read-only **Field geometry** disclosure to selected-relief properties: relief-axis physical span after absolute object scale and crop-aware nominal full source-cell pitch, with explicit zero-scale and evidence limits. It changes no CAM, schema, output, or machine policy; creation-surface geometry controls remain planned. ADR-305 adds atomic whole-project IndexedDB autosave/recovery for large canonical reliefs while preserving self-contained manual files; current runtime evidence covers committed Chrome state across reload, not abrupt process loss or packaged Electron restart. ADR-306 consolidates CNC settings ownership: Startup Setup is the sole writer for machine limits, current-job material/bit/stock, and the operation material/tool plan; Artwork owns operation cutting values; CNC Job Review is read-only. Software/browser verification is complete, while controller and hardware qualification remain pending. ADR-307 adds uncapped Grid/Circular placement and transient Point Rotation arrays around the combined selection center. ADR-308 restores positive-finite Gamma editing for canonical heightfields without rewriting source data. ADR-309 preserves finite legacy-mesh coordinates through ASCII import, persistence, and materialization instead of adding a refusal. ADR-310 preserves global clearing-before-profile order across multi-tool CNC output. ADR-311 binds machine commands, asynchronous document work, packaged-window visibility, deploy freshness, and release evidence to their exact owners. ADR-312 preserves terminal stream ownership during MPG takeover, guarantees reset fail-off cleanup, keeps canonical curves authoritative at worker scale, and applies physical-frame direction/placement/bounds consistently across CNC and jog planning. ADR-313 reconciles exact-current-main document, output, transport, preview, interaction, and bounded-scalability findings without adding a guard or refusal. ADR-314 binds concurrent Save, Image Studio dialog, and controller-represented CNC Z owners while retaining exact emitted-depth text and adding no guard or refusal. A completed Frame for the exact current job remains the sole ordinary Start guard on laser and CNC (ADR-228), and Job Review remains the single warning surface. ADR-316 reconciles the September 5 feature audit through exact geometry/settings ownership, canonical edited pixels, cancellable surfacing export, and controller/document request ownership. ADR-317 adds operation-owned artwork settings and project schema v5 while preserving existing CNC compound identities. ADR-318 repairs the September 12 CNC settings, output and preparation findings, gives tile registration an explicit schema-v6 cutting plan, and measures conservative clearing-aware V-carve finishing and source corners. ADR-319 preserves V-carve path conversion semantics in schema v7, clearing dependencies and final-coordinate containment. ADR tail at 319. The conservative dependency policy remains in force (ADR-017). Changes from here require a `DECISIONS.md` entry.
>
> **Relief Width update:** ADR-292 Amendment 5 extends the exact common-power-of-two project-v4
> re-expression to representable legacy-mesh Width edits. It preserves stored-bounds aspect,
> intrinsic mesh CAM geometry, transformed corners, prepared job, and the Frame signature while
> keeping validator and guard policy unchanged. Shared displayed-Width division that rounds to `0`
> or `Infinity`, unavailable exact v4 encodings, and durable all-positive-finite intent remain the
> next representation gap.
>
...
> **Current project format:** schema v7 preserves converted text winding and transformed stroke
> pens (ADR-319), following operation-owned overrides in v5 and tile registration plans in v6.
> The v4→v5→v6→v7 migrations preserve existing operation identities, settings and geometry.
> These extensions do not change the relief coordinate/scale representation or close
> its remaining positive-finite Width durability gap.

---

## Product goal

LaserForge 2.0 is the internal repository/project name for KerfDesk, a focused LightBurn-style CAM application for **GRBL** laser cutters and engravers, delivered today as a **web/PWA** from a codebase whose checked-in workflows also package **Windows and macOS Electron Previews**. Immutable Preview packages are published through `v0.2.0-preview.13`; installed-package, native-picker, and real-OS qualification remain separate. KerfDesk takes vector designs, text, traced artwork, raster images, and generated shapes; assigns named cut, fill, image, or CNC operations to artwork; previews the toolpath; generates correct G-code; and streams it to the connected machine.

Preview naming contract: **KerfDesk** is the user-facing product and release name. Its visible app name, installer, downloadable artifacts, download surfaces, and release titles use **KerfDesk**. **LaserForge 2.0** remains the repo/package/internal architecture name, and existing persistence identifiers may remain LaserForge where changing them would break identity or stored user data. The Cloudflare Pages API project is still named `laserforge` for historical reasons.
...
- **F.5 - Material calibration workflow** [Shipped; hardware calibration pending]. Minimal LightBurn-style Material Test and Interval Test generators, native `.lfml.json` libraries, multi-library UI, bounded LightBurn `.clb` import, and revisioned native preset-to-layer bindings with explicit refresh are shipped. Linked-current, linked-stale, legacy-untracked, device mismatch, and unsupported qualification are disclosed without refusing Apply/Link or adding a confirmation. Image recipes carry their effective raster direction and scan-offset override fields. Manufacturer profile packs, `.clb` export, and LightBurn `LinkPath` synchronization remain deferred. ADR-044, ADR-045, ADR-093, ADR-164.
...
| P2R.1 | Schema-v4 canonical U16 field; lossless legacy migration; 8/16-bit grayscale, alpha/mask, tonal mapping, external-relative input; self-contained manual save and atomic autosave | In progress — P2R.1a built under ADR-292/294/295/296/297/298/299/300/301/304/305/308/309 plus ADR-292 Amendments 2-5: schema v4/U16LE, v3 migration, manual-file persistence, worker-backed exact 8/16-bit grayscale plus simple `tRNS` masks and exact 8-bit grayscale-alpha masks, positive-finite gamma mapping, exact U16 input-endpoint mapping, exact inclusion-threshold and outside-mask-meaning controls, read-only persisted declared-source-meaning, recorded-source-detail, selected-heightfield physical-span/nominal-source-cell-pitch and resolved-aspect-policy disclosures, canonical Width/bounds synchronization without intermediate binary64 ratio loss, canonical preview/planning Width authority, exact common-factor project-v4 Width rebasing for representable heightfield and legacy-mesh edits, finite-preserving legacy-mesh ASCII import/persistence/materialization, logical partial-edge CAM with disclosed preview/emitter precision, existing CAM/previews, and atomic whole-project IndexedDB autosave/recovery qualified for committed Chrome state across reload. Abrupt-process and packaged-Electron recovery, shared displayed-Width division that rounds to `0` or `Infinity`, all-positive-finite Width durability beyond the v4 coordinate/scale representation, 16-bit alpha, mask authoring, histogram/clipping disclosure, creation-surface geometry controls and pre-create physical/effective-cell disclosure, editable crop/aspect controls, source-mode creation, output/Job Review provenance, and physical qualification remain planned |
...
- **Laser area-only second pass** (requested 2026-09-21): after a completed engraving,
  select a region in a zoomable canvas and repeat only that region to darken it. Keep its
  placement tied to the original run and preview the actual emitted region. The interruption
  restart picker replays the selected movement and the remainder; it does not yet clip output
  to a selected area.
...
- **Lint/format:**
  ...
  - Prettier.
- **CI:** GitHub Actions keeps the existing `ubuntu-latest` release/deploy gate for lint, typecheck, license-check, unit, property, snapshot, web/PWA build, and Electron main-process compile. Playwright remains a separate pull-request, main-push, and manually dispatched browser-smoke workflow; it covers both the Vite development runtime and a narrow production `dist/web` bundle smoke without becoming a Pages deploy dependency (ADR-158/311). The checked-in Desktop Preview workflow has Windows and macOS packaging jobs and publishes only authorized exact-version, same-repository, draft-to-immutable GitHub prerelease assets with checksums, a manifest, an SBOM, and provenance. It does not publish R2 assets, a mutable `latest`, or updater metadata. Real-OS install, serial, and hardware verification remain separate and CLAIMED until recorded; the report-only packaged native smoke adds launch/import/save/visible-window evidence without qualifying those lanes (ADR-247/248/311; S02-001/003).
...
Project
  ...
  scene
    objects: SceneObject[]
      ...
      shape
      name, operationIds[]
      operationOverride: legacy settings plus optional byOperation[id]: settings | null
...
Schema v5 resolves `operationOverride.byOperation` by the exact operation ID, then its parent
binding ID for a materialized sub-operation. A settings entry replaces the legacy artwork-wide
override; absent fields inherit that operation's base settings. An explicit `null` inherits the
base without the legacy override, and a missing key retains legacy behavior. Scope keys reference
existing operations or their sub-operations, travel with copied bindings, and are removed with
deleted operations. Existing operation identities and CNC compound geometry remain unchanged
when **Add operation** creates a separately editable copy (ADR-317).
