...
4. Do not publish Preview artifacts to R2, a mutable `latest` path, or an update feed. Updater trust remains false. A packaged Preview may make one anonymous, metadata-only request per app launch to the fixed public GitHub Releases API to surface a newer immutable prerelease; it never downloads, executes, installs, or silently navigates to an update (ADR-249).
...
- **No auto-update from arbitrary or unsigned channels.** The production desktop `electron-updater` feed is pinned at build time to our own `https://dl.kerfdesk.com/desktop/` origin and remains inert until production code signing is operational (ADR-024/135). Preview updater trust stays false and no unsigned executable is downloaded or run. Its separate checker accepts only a newer immutable, strict Preview release with the complete six-asset integrity set, then shows a passive status-bar link to that exact version's public `https://github.com/cisgz3a-hub/KerfDesk/releases/tag/v<version>` page. API-provided URLs are ignored. No `quitAndInstall`; future trusted updates apply on quit only, never mid-burn.
