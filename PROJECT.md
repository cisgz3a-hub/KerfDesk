...
> **Status:** v3.6 — **frame-first exact-artifact authorization (ADR-228/ADR-230, 2026-07-19): Job Review prepares one exact artifact and only its fully acknowledged, cleanly completed Frame issues the sole Start permit on laser and CNC.** Every prior Start gate's disposition (kept as transport/compile/handoff, demoted to a warning, or deleted) is tabulated in `docs/audits/2026-07-18-guard-inventory-frame-first.md`; see non-negotiable #21. Also recent: machine-rail control order and go-green coaching (ADR-225), OFL CNC stroke fonts (ADR-226), status-bar PWA Update control (ADR-227), and the read-only Super console (ADR-229). ADR tail at 230. MIT license, open-source release (ADR-120 supersedes ADR-018); MIT-compatible dependency policy preserved (ADR-017); DOMPurify pinned for Phase A SVG sanitization. Changes from here require a `DECISIONS.md` entry.
>
