# PROJECT.md — CurveDesk

> **Status:** v4.1 — CurveDesk Desktop Preview governance, packaging, and notify-only update discovery are implemented under ADR-247/248/249, pending merge and real-OS qualification before the first Preview tag. Repository release immutability is enabled. The existing web/PWA stays first-class; exact-version Windows x64 and macOS x64/arm64 Electron Previews do not change machining workflows, core, toolpaths, runtime trust, or the current MIT/public/free posture. Phase L "Image Studio" remains in progress under ADR-242. ADR-292 now carries the bounded P2R.1a substrate through project schema v5: a canonical U16LE heightfield, exact legacy migration, persisted intrinsic Float32 legacy-mesh bounds, worker-backed qualified grayscale import with simple transparency masks, and existing relief CAM/preview integration. ADR-292 Amendments 2-7 keep canonical Width and bounds coherent through binary64 ratio extremes, make the canonical field authoritative for preview planning, preserve exactly re-factorable heightfield and legacy-mesh Width edits inside the unchanged coordinate/scale domains, reject only factual non-finite Float32 legacy materialization under the maintainer's explicit authorization, commit reproduced displayed-Width divide underflow/overflow drafts without a lying local value, and remove repeated runtime intrinsic-mesh scans. Durable all-positive-finite Width intent beyond the unchanged coordinate/scale representation remains planned. ADR-293 adds atomic whole-project IndexedDB autosave/recovery for large canonical reliefs while preserving self-contained manual files; current runtime evidence covers committed Chrome state across reload, not abrupt process loss or packaged Electron restart. ADR-294 gives non-divisible relief grids an exact logical physical boundary without rewriting the requested interior spacing and discloses preview/emitter representation limits. ADR-295 qualifies exact non-interlaced grayscale PNG at 8 or 16 bits, ADR-296 preserves exact 8-bit grayscale-alpha samples as the same canonical field plus U8 mask, ADR-297 exposes the existing non-destructive gamma mapping, ADR-298 exposes the existing persisted-threshold outside-mask meaning, ADR-299 exposes exact U16 input endpoints, ADR-300 exposes exact inclusion-threshold editing, ADR-301 exposes the persisted declared source meaning, and ADR-302 exposes recorded source details in Relief properties without treating that metadata as authenticated. These controls and disclosures do not rewrite source bytes or change CAM algorithms. ADR-303 removes the obsolete preflight warning that falsely described supported raster rotation as unemittable; compile and emitted-byte behavior are unchanged. ADR-304 corrects current Frame/Job Review workflow and idle control copy to match the implemented dialog-free Frame, review-pending permit, and Start-time review sequence; runtime behavior is unchanged. ADR-305 adds a heightfield-only, read-only **Field geometry** disclosure to the current selected-relief properties: relief-axis physical span after absolute object scale and crop-aware nominal full source-cell pitch, with explicit zero-scale and evidence limits. It changes no CAM, output, or machine policy; creation-surface geometry controls remain planned. The remaining P2R.1 product surface and P2R.2-P2R.6 stay planned. A completed Frame for the exact current job remains the sole ordinary Start guard on laser and CNC (ADR-228), and Job Review remains the single warning surface. ADR tail at 305. The conservative dependency policy remains in force (ADR-017). Changes from here require a `DECISIONS.md` entry.
>
> **Relief representation update:** ADR-292 Amendments 5 and 7 preserve exactly representable
> legacy-mesh Width edits, persist the mesh's authoritative Float32 intrinsic bounds and explicit
> target Height/aspect policy in schema v5, and remove factorization/materialization rescans of
> immutable mesh positions. Width now commits in machine space; when native division by nonzero X
> scale rounds to `0` or `Infinity`, the reproduced finite drafts rebase to unit X scale and remain
> synchronized with project, undo, preview, CAM, and persistence. Coordinate/scale domains remain
> unchanged, so all-positive-finite intent beyond an exact bounded representation stays planned.
>
> **Legacy mesh materialization update:** ADR-292 Amendment 6 keeps import, project-open,
> manual-save, and autosave validation unchanged while requiring the actual Float32 legacy-CAM
> X/Y/Z bounds to be finite. An invalid materialization now returns the existing named
> `relief-materialization-failed` compile-integrity result before any partial program, Frame
> candidate, or Start permit exists. The maintainer explicitly authorized only this factual refusal
> widening; no schema, stored bytes, valid-mesh CAM, or policy guard changes.
>
...
| P2R.1 | Schema-v5 canonical U16 field plus persisted legacy Float32 geometry authority; lossless legacy migration; 8/16-bit grayscale, alpha/mask, tonal mapping, external-relative input; self-contained manual save and atomic autosave | In progress — P2R.1a built under ADR-292/293/294/295/296/297/298/299/300/301/302/305 plus ADR-292 Amendments 2-7: schema v5/U16LE, v3-v5 migration, manual-file persistence, worker-backed exact 8/16-bit grayscale plus simple `tRNS` masks and exact 8-bit grayscale-alpha masks, non-destructive manual gamma and exact U16 input-endpoint mapping, exact inclusion-threshold and outside-mask-meaning controls, read-only persisted declared-source-meaning, recorded-source-detail, selected-heightfield physical-span/nominal-source-cell-pitch and resolved-aspect-policy disclosures, canonical Width/bounds synchronization without intermediate binary64 ratio loss, canonical preview/planning Width authority, exact common-factor Width rebasing for representable heightfield and legacy-mesh edits, persisted intrinsic Float32 legacy bounds and explicit target Height/aspect, machine-space Width commits across reproduced native divide underflow/overflow, legacy Float32 X/Y/Z materialization integrity, logical partial-edge CAM with disclosed preview/emitter precision, and atomic whole-project IndexedDB autosave/recovery qualified for committed Chrome state across reload. Abrupt-process and packaged-Electron recovery, all-positive-finite Width durability beyond the unchanged coordinate/scale representation, 16-bit alpha, mask authoring, histogram/clipping disclosure, creation-surface geometry controls and pre-create physical/effective-cell disclosure, editable crop/aspect controls, source-mode creation, output/Job Review provenance, and physical qualification remain planned |
