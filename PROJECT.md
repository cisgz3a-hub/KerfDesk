...
| P2R.1 | Schema-v4 canonical U16 field; lossless legacy migration; 8/16-bit grayscale, alpha/mask, tonal mapping, external-relative input; self-contained manual save and atomic autosave | In progress — P2R.1a built under ADR-292/293/294/295/296: schema v4/U16LE, v3 migration, manual-file persistence, worker-backed exact 8/16-bit grayscale plus simple `tRNS` masks and exact 8-bit grayscale-alpha masks, logical partial-edge CAM with disclosed preview/emitter precision, and atomic whole-project IndexedDB autosave/recovery qualified for committed Chrome state across reload. Abrupt-process and packaged-Electron recovery, 16-bit alpha, full mask controls, and tonal/source-mode UI remain planned |
