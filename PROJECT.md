...
> **Status:** v4.1 — KerfDesk Desktop Preview governance adopted (ADR-247/248), but its packaging workflow and artifacts are still planned and no Preview tag is permitted yet. The existing web/PWA stays first-class; the planned exact-version Windows x64 and macOS x64/arm64 Electron Previews will not change machining workflows, core, toolpaths, runtime trust, or the current MIT/public/free posture. Their only planned visible changes are KerfDesk branding and the desktop-download surface. Phase L "Image Studio" remains in progress under ADR-242. A completed Frame for the exact current job remains the sole ordinary Start guard on laser and CNC (ADR-228), and Job Review remains the single warning surface. ADR tail at 248. The conservative dependency policy remains in force (ADR-017). Changes from here require a `DECISIONS.md` entry.
>
