...
4. App compiles the project to G-code via `emitGcode`, then opens the **Job Review** dialog
   ...
   **Artwork settings** table with laser core numbers editable in place (power / speed / passes /
   air) and CNC core numbers shown read-only from the exact prepared job (depth, depth-per-pass,
   feed, plunge, artwork spindle RPM), plus a muted per-row detail line carrying the mode-specific
   settings (kerf/tabs/hatch/dither; CNC passes,
...
4. Editing a laser value inside the review commits through the normal layer store actions and
   re-runs the full prepare pipeline (debounced). The stat tiles dim behind "Recomputing…" until
   the fresh program replaces the shown one. The laser-only **Approve settings** status identifies
   an editable value changed since the review opened or was last approved; clicking it marks the
   synchronized main Artwork / Operations values approved and requests an immediate refresh when
   needed. It never gates Start: **Start job** still approves the complete review in one click.
   CNC Job Review accepts no setting edits. Its **Edit Artwork settings** and **Edit Startup Setup**
   routes close the review and return to the one writable owner. Any project change that alters the
   exact framed artifact invalidates the existing permit through the handoff-consistency contract,
   so the operator reviews and frames the changed job again; Frame remains the only ordinary Start
   guard.
...
1. **Cancel** or Escape sends no Frame/job bytes, stages no candidate, and issues no permit. Laser
   edits already made in the review are ordinary project edits and are kept (undo applies as usual);
   CNC review has no writable settings to retain.
...
laser-only setup remains six (ADR-294 supersedes ADR-240's fixed six-page composition):
...
1. The read-only Artwork reference and pre-Frame Job Review identify the mismatch and direct the
...
### F-CNC31. Auto-fill operation feeds from a material — ADR-111 #1, amended by ADR-294
...
### F-CNC32. Edit operation values beside read-only setup references — ADR-111 #4 / ADR-294
...
### F-CNC35. Set the project material once — ADR-112/264, amended by ADR-294
