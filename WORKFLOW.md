...
- **Rulers**: top and left, mm units. Tick and label spacing follow the zoom (ADR-348): the labelled step is the smallest 1-2-5 number whose labels stay clear of each other, minor ticks subdivide it while they remain separable, and a taller half-step tick marks the middle of a division. The pointer position is marked on both strips while it is over the canvas.
- **Grid**: light dotted, 10mm spacing, behind bed.
- **Origin marker**: small cross at the device-profile origin.
- **Hover size**: with the select tool and no drag in progress, resting the pointer on artwork shows its name, `W × H mm`, and the X/Y of its top-left beside the cursor. Display only — it changes no selection, no geometry, and no output.
...
- An Undo / Redo pair sits in the menu bar, immediately right of Help (ADR-348). Each button is disabled when its history is empty, and both run the same commands as the shortcuts and the Edit menu.
...
### F-B7a. CNC Pause and lift (ADR-401)
...
5. Width, total depth, polarity, and exact **Input low**/**Input high** U16 codes are editable. A
   ...
   authorization rules are unchanged. **Gamma** accepts every positive finite exponent without a
   minimum, maximum, cap, or clamp; `1` is linear. Materialization raises normalized source samples
   to that exponent before polarity. A gamma edit changes only persisted curve and revision, not
   samples, masks, digest, provenance, geometry, or output policy.
...
> **Planned - not current UI.** P2R.1a plus ADR-292 Amendments 2-5 and ADR-294/295/296/297/298/299/300/301/304/305/308
...
> autosave/recovery, and the existing CAM/preview substrate. The creation modes and the
...
3. Qualification sends one `M221`. With no `Laser power` report the Laser module is not loaded:
   jog, Frame, Home and the job program leave out `fire off` (nothing would answer it), and the
   Console `fire` and Fire are refused. A laser job's Frame and Start are not refused: Job
   Review warns that the job will run with the laser off. A report without `disable auto
   power` is a build older than edge 971eb8cf (2021-06-15); Job Review warns that its
...
4. Abort uses the controller's Ctrl-X halt path; a halted controller answers `!!` to normal
   ...
   is unhalted and Idle, so job travel keeps the configured seek rate. The board halts rather
   than reboots, so the controller checks run again on the first Idle after Unlock. This is not
   the GRBL soft-reset lifecycle or a physical E-stop.
...
6. `$H` answers `ok` whether or not anything homed, so after the cycle KerfDesk asks `G28.6`
   which axes are homed. Home is confirmed only when X and Y report homed; otherwise it ends
   with "Home was not confirmed" and the reason (no homing switches configured, or the cycle
   did not home an axis).
...
1. The user selects **Learn**, **Help > Visual tutorials**, or **Tutorial** within a tool panel — Registration jig, Camera, Place board, Trace, Image Studio and Design Studio. Since ADR-348 no other surface carries a tutorial button: not menu rows, dialog headers, rail section headers or individual settings rows. The current tool and its draft stay in place.
...
Tutorials never open automatically, submit forms, invoke tool commands, change artwork or send controller commands. Every lesson remains reachable from Learn, including those for controls that no longer carry their own button. Reduced motion keeps explicit static example-stage controls. The learning surface registers as a modal to isolate application shortcuts, appears above studio dialogs, and stays below the live-motion control bar. Completion is a reading aid, not a qualification or prerequisite for using a tool or starting a job.
