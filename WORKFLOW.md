...
- **Top command toolbar**: Open, Import, Save and Preview lead with readable labels. Selecting an image brings Trace image and Image Studio into the row when space permits. Other commands remain in **More** and the application menus, with their existing shortcuts, disabled reasons, tooltips and actions. The row measures available width and moves commands into More instead of scrolling horizontally. Utility controls share the row down to 520 px. The project name includes an unsaved-change indicator.
