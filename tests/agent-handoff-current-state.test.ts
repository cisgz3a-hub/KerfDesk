...
test('handoff names the current audit-fix state and next roadmap queue', () => {
  assert.match(handoff, /Last shipped roadmap item: \*\*T2-6 Phase 3aw\*\*/);
...
test('handoff preserves known verification caveats', () => {
  ...
  assert.match(handoff, /T2-6 Phase 3au extracts manual-save clean-baseline planning/);
  assert.match(handoff, /T2-6 Phase 3av \| App split \| Unsafe-prior-state recovery alert/);
  assert.match(handoff, /T2-6 Phase 3aw \| App split \| Startup autosave recovery prompt/);
  assert.match(handoff, /T2-6 Phase 3aw extracts startup autosave recovery prompt planning/);
