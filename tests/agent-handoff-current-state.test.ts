...
test('handoff names the current audit-fix state and next ticket', () => {
  assert.match(handoff, /Last shipped roadmap item: \*\*T1-233\*\*/);
  assert.match(handoff, /Current audit-fix run completed: \*\*T1-223 through T1-233\*\*/);
  assert.match(handoff, /Next audit-fix ticket: \*\*T1-234\*\*/);
});
