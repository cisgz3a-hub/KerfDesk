...
test('handoff names the current audit-fix state and next ticket', () => {
  assert.match(handoff, /Last shipped roadmap item: \*\*T1-259\*\*/);
  assert.match(handoff, /Current audit-fix run completed: \*\*T1-223 through T1-259\*\*/);
...
test('handoff preserves known verification caveats', () => {
  assert.match(handoff, /Full `npm test` passed during T1-259/);
  assert.match(handoff, /git diff --check` passed during the T1-259 close-out/);
  assert.match(handoff, /npx tsc --noEmit --pretty false` passed during the T1-259 close-out/);
