...

{
  ...
  assert(/connectionRecoveryVisible/.test(source), 'ConnectionPanelMain tracks unexpected disconnect recovery visibility');
  assert(/wasConnectedRef/.test(source), 'ConnectionPanelMain remembers prior connected state for cable-pull detection');
  assert(/buildRecoveryCard\(\{\s*variant:\s*'disconnect'/.test(source), 'ConnectionPanelMain builds disconnect recovery content');
  assert(/frameRecoveryTimeoutSec/.test(source), 'ConnectionPanelMain tracks frame-failed recovery timeout');
  assert(/setFrameRecoveryTimeoutSec/.test(source), 'ConnectionPanelMain marks failed frame attempts for recovery');
  assert(/buildRecoveryCard\(\{\s*variant:\s*'frame-failed'/.test(source), 'ConnectionPanelMain builds frame-failed recovery content');
  assert(/jobFailedRecoveryMessage/.test(source), 'ConnectionPanelMain tracks job-failed recovery copy');
