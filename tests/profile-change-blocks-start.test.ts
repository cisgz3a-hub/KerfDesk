/**
 * T3-2 case 4: profile changes invalidate compiled G-code before start.
...
async function run(): Promise<void> {
  console.log('\n=== profile-change-blocks-start ===\n');

  installMockLocalStorage();
  for (const key of Object.keys(memoryStore)) delete memoryStore[key];

  const profile400 = createBlankProfile('T3-2 400x400');
  profile400.bedWidth = 400;
  profile400.bedHeight = 400;
  saveDeviceProfile(profile400);
  setActiveProfileId(profile400.id);

  const scene0 = createScene(400, 400, 'profile changed');
  const scene = addObject(
    scene0,
    createRect(scene0.layers[0].id, 20, 20, 40, 30, 'profile-rect'),
  );
  ...
  const profile300 = createBlankProfile('T3-2 300x300');
  profile300.bedWidth = 300;
  profile300.bedHeight = 300;
  saveDeviceProfile(profile300);
  setActiveProfileId(profile300.id);
  ...
  assert(
    errorMessage.includes('The device profile changed after this G-code was created'),
    'profile switch is rejected with user-facing stale-profile message',
  );
  assert(!errorMessage.includes('hash'), 'stale-profile message does not expose hash internals');
  assert(executeCalls.length === 0, 'profile-mismatched ticket is not sent to executeJob');
  assert(service.getActiveTicket() === null, 'profile-mismatched ticket does not become active');
  assert(service.getActiveJobCanvasContext() === null, 'profile-mismatched ticket does not pin canvas context');
