...
void (async () => {
...

  {
    ...
    assert(!warns.some(w => w.includes('[TestFire] stop blocked')), 'Not connected → no TestFire stop warn');
  }

  {
    ...
    assert(warns.some(w => w.includes('[TestFire] stop blocked')), 'other error → TestFire stop warn');
  }
