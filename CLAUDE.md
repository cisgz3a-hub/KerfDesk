...
7. **No new guards without the maintainer's explicit prior permission — hard rule.** A *guard* is any new behavior that blocks, refuses, gates, caps, clamps, delays, hides, disables, rewrites, or adds confirmation before an otherwise available action, input, output, machine command, job start, preview, save, import, export, or G-code emission. **Do not implement one, test-drive one into the design, or expand an existing guard's refusal surface unless the maintainer first approves that specific guard in chat.** Before asking, provide a written necessity case that identifies the concrete failure being prevented, current-tree evidence that the failure is real, why existing guards do not cover it, every valid workflow or input the guard could block, the exact operator-facing message and recovery path, and why a warning, measurement, scalable implementation, or other non-blocking alternative is insufficient. The bar is **absolutely necessary**, not “defense in depth,” convention, hypothetical risk, or “safer by default.” The PR must link or quote both the necessity case and the maintainer's approval. Approval for one guard never authorizes similar guards elsewhere; tests and an ADR do not substitute for permission. Narrowing, correcting, or removing an existing guard is normal bug-fix work, but making it block more requires fresh permission. If approval is absent, stop and report the proposed guard instead of adding it (ADR-206; PROJECT.md non-negotiable #21).
