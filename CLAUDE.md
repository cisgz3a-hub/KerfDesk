...
9. **When you don't know — go research it. Never reason your way to an answer.** (Rule 8 is reserved by an open PR.) An unverified belief is not a deliverable. The moment you catch yourself about to write "I believe," "typically," "should be," "probably," "I think GRBL/LightBurn/that library does X," or "I have not verified this, but" — **stop and go find out before you answer.** Research, in order of preference: read the code in this tree → run the actual command → read the vendor's own documentation or firmware source → search the web for the authoritative source. Then **cite it** — `file:line`, the command output, or the URL — so the maintainer can check your work. Never substitute plausible reasoning about how something *probably* works for the documented fact, and never hand the maintainer a belief for them to go verify when a search would have settled it in a minute. If research genuinely fails, say precisely what you consulted and what is still open — "I read X and searched Y; the answer is in neither" — never a bare "I don't know," and never a guess dressed up as a finding. This rule binds hardest **outside** this repo — GRBL/grblHAL behavior, controller firmware, LightBurn semantics, third-party APIs — because that is exactly where nothing in the tree will catch the mistake for you.
...
**Do not stop at "I don't know" and hand the question back.** Collaboration rule 9 applies: go research it — this tree, the command, the vendor's documentation or firmware source, then the web — and come back with the answer and its citation. Asking the maintainer to look something up that a search would have settled is the same failure as inventing it.

Only when research actually fails do you report the gap, and then with your working shown:

> I don't know X. I read [file:line] and searched [source]; the answer is in neither. To settle it I need [the specific command, or a fact only you have — e.g. what firmware the machine runs]. Meanwhile I've assumed Y, and here is what changes if that's wrong.
