...
- **Wired-into-product gate (T1-120):** a ticket may only be marked **Shipped** when the code change is *consumed by a production code path* and *covered by an integration / end-to-end test that exercises that path*. Unit-only tests of an isolated helper, a type that nothing imports, or a function that no live caller invokes do **not** satisfy the contract. The audit ledger has repeatedly accumulated "type-only" tickets where a foundation was built but never wired (T1-119 had to retrofit MigrationPipeline; the audit ledger flagged similar gaps for `RecoveryState`, Falcon WiFi trust, server entitlement tokens, controller abstraction, streaming output). Per-ticket status format in [docs/TICKET-LEDGER-FORMAT.md](docs/TICKET-LEDGER-FORMAT.md). Before writing `**Status:** Shipped` confirm: (a) at least one non-test file under `src/` or `electron/` calls/imports the new code, and (b) at least one test file exercises the live wiring (i.e. mounts the real production path, not a mock-only fixture).
