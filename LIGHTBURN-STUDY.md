...
- **DIVERGE**
  - ✓ **Preamble pre-arms the laser with `M3 S0`** (`grbl-strategy.ts` `preamble()`), vs LightBurn's stock GRBL header which is units/positioning only with M3/M4 issued per cut layer. Deliberate: primes `$32=0` controllers so S-driven moves fire; `M3 S0` is laser-off in laser mode. Hardware-verified. (Ledgered per audit M14.)
  ...
  - ✓ **Path optimization is nearest-neighbor within a layer only** (`optimize-paths.ts`) — cut-inner-first now ships (containment-depth order); no cross-layer reorder, no 2-opt; LightBurn exposes Optimization Settings (order inner-first / by layer / by group, reduce travel, etc.; §5.6). → **redesign:** cut-inner-first ordering shipped; cross-layer reorder + 2-opt still open.
...
- **SHIPPED since this row:** **Interval Test** (§7.1), **Material Test** (§7.8), **Dot Width Correction** (§7.2), **Convert to Bitmap** (§7.4, ADR-029), **Apply Mask to Image** (§7.5), and **Material Library** core (§7.7; `.clb` compat still deferred). **Still GAP:** per-object Shape-Properties image controls (§7.6).
